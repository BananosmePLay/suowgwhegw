...
public class CustomColors {
   ...
   private static void readColorProperties(String fileName) {
      try {
         ...
         xpOrbTime = Config.parseInt(((Properties)properties).getProperty("xporb.time"), -1);
   ...
   private static void addToList(CustomColormap cm, List list, int id) {
      ...
      ArrayList<CustomColormap> subList = (ArrayList)list.get(id);
      if (subList == null) {
   ...
   private static CustomColormap[][] blockListToArray(List list) {
      ...
      for(int i2 = 0; i2 < list.size(); ++i2) {
         List<CustomColormap> subList = (List)list.get(i2);
         if (subList != null) {
   ...
   public static int getFluidColor(IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, RenderEnv renderEnv) {
      ...
      if (customcolors$icolorizer == null) {
         ...
         return Config.isSmoothBiomes() && !((IColorizer)customcolors$icolorizer).isColorConstant() ? getSmoothColorMultiplier(blockState, blockAccess, blockPos, (IColorizer)customcolors$icolorizer, renderEnv.getColorizerBlockPosM()) : ((IColorizer)customcolors$icolorizer).getColor(blockState, blockAccess, blockPos);
      }
   }
   ...
   private static int getMaxPotionId() {
      int i = 0;
      Iterator var1 = Potion.REGISTRY.getKeys().iterator();
