   ...
   public class AIWololoSpell extends EntitySpellcasterIllager.AIUseSpell {
      final Predicate<EntitySheep> wololoSelector = new Predicate<EntitySheep>() {
         ...
         public boolean apply(Object var1) {
            return this.apply((EntitySheep)var1);
         }
      };
      ...
      public boolean shouldExecute() {
         if (EntityEvoker.this.getAttackTarget() != null) {
            ...
            List<EntitySheep> list = EntityEvoker.this.world.getEntitiesWithinAABB(EntitySheep.class, EntityEvoker.this.getEntityBoundingBox().grow(16.0, 4.0, 16.0), this.wololoSelector);
            if (list.isEmpty()) {
               return false;
            } else {
      ...
      protected void castSpell() {
         ...
         if (EntityEvoker.this.getDistanceSq(entitylivingbase) < 9.0) {
            ...
            for(k = 0; k < 5; ++k) {
               ...
               this.spawnFangs(EntityEvoker.this.posX + (double)MathHelper.cos(f2) * 1.5, EntityEvoker.this.posZ + (double)MathHelper.sin(f2) * 1.5, d0, d1, f2, 0);
            }

            for(k = 0; k < 8; ++k) {
               f2 = f + (float)k * 3.1415927F * 2.0F / 8.0F + 1.2566371F;
               this.spawnFangs(EntityEvoker.this.posX + (double)MathHelper.cos(f2) * 2.5, EntityEvoker.this.posZ + (double)MathHelper.sin(f2) * 2.5, d0, d1, f2, 3);
            }
         } else {
            for(k = 0; k < 16; ++k) {
