...
public class BlockDoublePlant extends BlockBush implements IGrowable {
   ...
   public void getSubBlocks(CreativeTabs itemIn, NonNullList<ItemStack> items) {
      EnumPlantType[] var3 = BlockDoublePlant.EnumPlantType.values();
      int var4 = var3.length;

      for(int var5 = 0; var5 < var4; ++var5) {
         EnumPlantType blockdoubleplant$enumplanttype = var3[var5];
   ...
   public int getMetaFromState(IBlockState state) {
      return state.getValue(HALF) == BlockDoublePlant.EnumBlockHalf.UPPER ? 8 | ((EnumFacing)state.getValue(FACING)).getHorizontalIndex() : ((EnumPlantType)state.getValue(VARIANT)).getMeta();
   }
