   ...
   public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
      if (args.length <= 0) {
         ...
         if (sender.getEntityWorld().getGameRules().getBoolean("sendCommandFeedback")) {
            ((EntityPlayer)entityplayer).sendMessage(new TextComponentTranslation("gameMode.changed", new Object[]{itextcomponent}));
         }

         if (entityplayer == sender) {
            notifyCommandListener(sender, this, 1, "commands.gamemode.success.self", new Object[]{itextcomponent});
         } else {
            notifyCommandListener(sender, this, 1, "commands.gamemode.success.other", new Object[]{((EntityPlayer)entityplayer).getName(), itextcomponent});
         }
