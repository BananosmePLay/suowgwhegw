...
public class EntityParrot extends EntityShoulderRiding implements EntityFlying {
   ...
   private static boolean playMimicSound(World worldIn, Entity parrotIn) {
      if (!parrotIn.isSilent() && worldIn.rand.nextInt(50) == 0) {
         ...
         if (!list.isEmpty()) {
            EntityLiving entityliving = (EntityLiving)list.get(worldIn.rand.nextInt(list.size()));
   ...
   static {
      ...
      CAN_MIMIC = new Predicate<EntityLiving>() {
      ...
      TAME_ITEMS = Sets.newHashSet((Object[])(Items.WHEAT_SEEDS, Items.MELON_SEEDS, Items.PUMPKIN_SEEDS, Items.BEETROOT_SEEDS));
