...
public class DynamicLights {
   ...
   private static void loadModConfiguration(InputStream in, String path, String modId) {
      if (in != null) {
         try {
            ...
            loadModLightLevels(((Properties)properties).getProperty("entities"), mapEntityLightLevels, new EntityClassLocator(), connectedparser, path, modId);
            loadModLightLevels(((Properties)properties).getProperty("items"), mapItemLightLevels, new ItemLocator(), connectedparser, path, modId);
         } catch (IOException var5) {
   ...
   private static void updateMapDynamicLights(RenderGlobal renderGlobal) {
      ...
      if (world != null) {
         Iterator var2 = ((World)world).getLoadedEntityList().iterator();
   ...
   static {
      PARAMETER_ITEM_STACK = new DataParameter(6, DataSerializers.ITEM_STACK);
   }
}
