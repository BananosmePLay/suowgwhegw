...
public class TextComponentScore extends TextComponentBase {
   ...
   public void resolve(DB sender) {
      Xx minecraftserver = sender.getServer();
      if (minecraftserver != null && minecraftserver.isAnvilFileSet() && StringUtils.isNullOrEmpty(this.value)) {
         Ws scoreboard = minecraftserver.getWorld(0).getScoreboard();
         Wz scoreobjective = scoreboard.getObjective(this.objective);
         if (scoreboard.entityHasObjective(this.name, scoreobjective)) {
            Wr score = scoreboard.getOrCreateScore(this.name, scoreobjective);
