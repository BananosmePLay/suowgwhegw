...
public class MathHelper {
   ...
   public static final float PI = bqD.roundToFloat(Math.PI);
   public static final float PI2 = bqD.roundToFloat(6.283185307179586);
   public static final float PId2 = bqD.roundToFloat(1.5707963267948966);
   private static final float radToIndex = bqD.roundToFloat(651.8986469044033);
   public static final float deg2Rad = bqD.roundToFloat(0.017453292519943295);
   ...
   public static boolean isMouseHoveringOnRect(double x, double y, double width, double height, int mouseX, int mouseY) {
      return (double)mouseX >= x && (double)mouseY >= y && (double)mouseX <= x + width && (double)mouseY <= y + height;
   }
   ...
   public static float round(float num, float increment) {
      double v = (double)Math.round(num / increment) * (double)increment;
      BigDecimal bd = new BigDecimal(v);
      bd = bd.setScale(2, RoundingMode.HALF_UP);
   ...
   static {
      ...
      for(k = 0; k < SIN_TABLE_FAST.length; ++k) {
         SIN_TABLE_FAST[k] = bqD.roundToFloat(Math.sin((double)k * Math.PI * 2.0 / 4096.0));
      }
