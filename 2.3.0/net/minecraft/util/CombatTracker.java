...
public class CombatTracker {
   ...
   public CombatTracker(Iw fighterIn) {
   ...
   public void calculateFallSuffix() {
      ...
      if (this.fighter.isOnLadder()) {
         co block = this.fighter.world.getBlockState(new BlockPos(this.fighter.posX, this.fighter.getEntityBoundingBox().minY, this.fighter.posZ)).getBlock();
         if (block == Nk.LADDER) {
            this.fallSuffix = "ladder";
         } else if (block == Nk.VINE) {
   ...
   public ITextComponent getDeathMessage() {
      if (this.combatEntries.isEmpty()) {
         ...
         Ig entity = combatentry1.getDamageSrc().getTrueSource();
            ...
            if (combatentry.getDamageSrc() != DamageSource.FALL && combatentry.getDamageSrc() != DamageSource.OUT_OF_WORLD) {
               if (itextcomponent2 == null || itextcomponent1 != null && itextcomponent2.equals(itextcomponent1)) {
                  if (itextcomponent1 != null) {
                     Qy itemstack = entity instanceof Iw ? ((Iw)entity).getHeldItemMainhand() : Qy.EMPTY;
                  ...
                  Ig entity1 = combatentry.getDamageSrc().getTrueSource();
                  Qy itemstack1 = entity1 instanceof Iw ? ((Iw)entity1).getHeldItemMainhand() : Qy.EMPTY;
   ...
   @Nullable
   public Iw getBestAttacker() {
      Iw entitylivingbase = null;
      ...
      while(true) {
         ...
         do {
            do {
               ...
               if (combatentry.getDamageSrc().getTrueSource() instanceof ME && (entityplayer == null || combatentry.getDamage() > f1)) {
                  f1 = combatentry.getDamage();
                  entityplayer = (ME)combatentry.getDamageSrc().getTrueSource();
               }
            } while(!(combatentry.getDamageSrc().getTrueSource() instanceof Iw));
         } while(entitylivingbase != null && !(combatentry.getDamage() > f));

         f = combatentry.getDamage();
         entitylivingbase = (Iw)combatentry.getDamageSrc().getTrueSource();
      }
   }
