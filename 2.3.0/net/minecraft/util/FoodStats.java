...
public class FoodStats {
   ...
   public void addStats(PI foodItem, Qy stack) {
      this.addStats(foodItem.getHealAmount(stack), foodItem.getSaturationModifier(stack));
   }

   public void onUpdate(ME player) {
      baV enumdifficulty = player.world.getDifficulty();
      this.prevFoodLevel = this.foodLevel;
      if (this.foodExhaustionLevel > 4.0F) {
         this.foodExhaustionLevel -= 4.0F;
         if (this.foodSaturationLevel > 0.0F) {
            this.foodSaturationLevel = Math.max(this.foodSaturationLevel - 1.0F, 0.0F);
         } else if (enumdifficulty != baV.PEACEFUL) {
      ...
      if (flag && this.foodSaturationLevel > 0.0F && player.shouldHeal() && this.foodLevel >= 20) {
         ...
         ++this.foodTimer;
         if (this.foodTimer >= 80) {
            if (player.getHealth() > 10.0F || enumdifficulty == baV.HARD || player.getHealth() > 1.0F && enumdifficulty == baV.NORMAL) {
   ...
   public void readNBT(QQ compound) {
   ...
   public void writeNBT(QQ compound) {
