...
public abstract class TabCompleter {
   ...
   public TabCompleter(lE textFieldIn, boolean hasTargetBlockIn) {
   ...
   private void requestCompletions(String prefix) {
      if (prefix.length() >= 1) {
         nC.getMinecraft();
         nC.player.connection.sendPacket(new Tm(prefix, this.getTargetBlockPos(), this.hasTargetBlock));
