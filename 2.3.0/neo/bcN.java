...
public class bcN extends bcM {
   protected static final in BLK_LAVA;
   protected static final in BLK_AIR;
   protected static final in BLK_SANDSTONE;
   protected static final in BLK_RED_SANDSTONE;

   public bcN() {
   }

   protected void addRoom(long p_180703_1_, int p_180703_3_, int p_180703_4_, ban p_180703_5_, double p_180703_6_, double p_180703_8_, double p_180703_10_) {
   ...
   protected void addTunnel(long p_180702_1_, int p_180702_3_, int p_180702_4_, ban p_180702_5_, double p_180702_6_, double p_180702_8_, double p_180702_10_, float p_180702_12_, float p_180702_13_, float p_180702_14_, int p_180702_15_, int p_180702_16_, double p_180702_17_) {
               ...
               for(int j1 = k2; !flag3 && j1 < k; ++j1) {
                  for(j3 = i3; !flag3 && j3 < i1; ++j3) {
                     for(int l1 = l + 1; !flag3 && l1 >= l2 - 1; --l1) {
                        if (l1 >= 0 && l1 < 256) {
                           in iblockstate = p_180702_5_.getBlockState(j1, l1, j3);
                           if (iblockstate.getBlock() == Nk.FLOWING_WATER || iblockstate.getBlock() == Nk.WATER) {
                              flag3 = true;
                           }
                        ...
                        if (d10 * d10 + d8 * d8 < 1.0) {
                           for(int j2 = l; j2 > l2; --j2) {
                              ...
                              if (d9 > -0.7 && d10 * d10 + d9 * d9 + d8 * d8 < 1.0) {
                                 in iblockstate1 = p_180702_5_.getBlockState(j3, j2, i2);
                                 in iblockstate2 = (in)MoreObjects.firstNonNull(p_180702_5_.getBlockState(j3, j2 + 1, i2), BLK_AIR);
                                 if (iblockstate1.getBlock() == Nk.GRASS || iblockstate1.getBlock() == Nk.MYCELIUM) {
                                    flag1 = true;
                                 }
                                 ...
                                 if (this.canReplaceBlock(iblockstate1, iblockstate2)) {
                                    if (j2 - 1 < 10) {
                                       ...
                                       if (flag1 && p_180702_5_.getBlockState(j3, j2 - 1, i2).getBlock() == Nk.DIRT) {
   ...
   protected boolean canReplaceBlock(in p_175793_1_, in p_175793_2_) {
      if (p_175793_1_.getBlock() == Nk.STONE) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.DIRT) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.GRASS) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.HARDENED_CLAY) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.STAINED_HARDENED_CLAY) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.SANDSTONE) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.RED_SANDSTONE) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.MYCELIUM) {
         return true;
      } else if (p_175793_1_.getBlock() == Nk.SNOW_LAYER) {
         return true;
      } else {
         return (p_175793_1_.getBlock() == Nk.SAND || p_175793_1_.getBlock() == Nk.GRAVEL) && p_175793_2_.getMaterial() != hM.WATER;
      }
   }

   protected void recursiveGenerate(bij worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ban chunkPrimerIn) {
   ...
   static {
      BLK_LAVA = Nk.LAVA.getDefaultState();
      BLK_AIR = Nk.AIR.getDefaultState();
      BLK_SANDSTONE = Nk.SANDSTONE.getDefaultState();
      BLK_RED_SANDSTONE = Nk.RED_SANDSTONE.getDefaultState();
   }
}
