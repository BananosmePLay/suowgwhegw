...
public class IN extends Iw {
   ...
   public static final Rd<Byte> STATUS;
   public static final Rd<Rotations> HEAD_ROTATION;
   public static final Rd<Rotations> BODY_ROTATION;
   public static final Rd<Rotations> LEFT_ARM_ROTATION;
   public static final Rd<Rotations> RIGHT_ARM_ROTATION;
   public static final Rd<Rotations> LEFT_LEG_ROTATION;
   public static final Rd<Rotations> RIGHT_LEG_ROTATION;
   private static final Predicate<Ig> IS_RIDEABLE_MINECART;
   private final NonNullList<Qy> handItems;
   private final NonNullList<Qy> armorItems;
   ...
   public IN(bij worldIn) {
      super(worldIn);
      this.handItems = NonNullList.withSize(2, Qy.EMPTY);
      this.armorItems = NonNullList.withSize(4, Qy.EMPTY);
   ...
   public IN(bij worldIn, double posX, double posY, double posZ) {
      this(worldIn);
   ...
   public Qy getItemStackFromSlot(EntityEquipmentSlot slotIn) {
      switch (slotIn.getSlotType()) {
         case HAND:
            return (Qy)this.handItems.get(slotIn.getIndex());
         case ARMOR:
            return (Qy)this.armorItems.get(slotIn.getIndex());
         default:
            return Qy.EMPTY;
      }
   }
   ...
   public boolean replaceItemInInventory(int inventorySlot, Qy itemStackIn) {
      ...
      if (!itemStackIn.isEmpty() && !Iu.isItemStackInSlot(entityequipmentslot, itemStackIn) && entityequipmentslot != EntityEquipmentSlot.HEAD) {
         return false;
      } else {
   ...
   public static void registerFixesArmorStand(DataFixer fixer) {
      fixer.registerWalker(FixTypes.ENTITY, new ItemStackDataLists(IN.class, new String[]{"ArmorItems", "HandItems"}));
   }
   ...
   public void readEntityFromNBT(QQ compound) {
      ...
      if (compound.hasKey("ArmorItems", 9)) {
         ...
         for(j = 0; j < this.armorItems.size(); ++j) {
            this.armorItems.set(j, new Qy(nbttaglist1.getCompoundTagAt(j)));
         }
      }
      ...
      if (compound.hasKey("HandItems", 9)) {
         ...
         for(j = 0; j < this.handItems.size(); ++j) {
            this.handItems.set(j, new Qy(nbttaglist1.getCompoundTagAt(j)));
         }
      }
   ...
   protected void collideWithNearbyEntities() {
      List<Ig> list = this.world.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox(), IS_RIDEABLE_MINECART);
   ...
   public EnumActionResult applyPlayerInteraction(ME player, Vec3d vec, EnumHand hand) {
      ...
      if (!this.hasMarker() && itemstack.getItem() != NK.NAME_TAG) {
   ...
   private void swapItem(ME player, EntityEquipmentSlot p_184795_2_, Qy p_184795_3_, EnumHand hand) {
      Qy itemstack = this.getItemStackFromSlot(p_184795_2_);
         ...
         if (DamageSource.OUT_OF_WORLD.equals(source)) {
            ...
            if (source.isExplosion()) {
               ...
               if (!flag1 && !flag) {
                  ...
                  if (source.getImmediateSource() instanceof MO) {
                     source.getImmediateSource().setDead();
                  }

                  if (source.getTrueSource() instanceof ME && !((ME)source.getTrueSource()).capabilities.allowEdit) {
                     return false;
   ...
   public void handleStatusUpdate(byte id) {
      if (id == 32) {
         if (this.world.isRemote) {
            this.world.playSound(this.posX, this.posY, this.posZ, NO.ENTITY_ARMORSTAND_HIT, this.getSoundCategory(), 0.3F, 1.0F, false);
   ...
   private void playParticles() {
      if (this.world instanceof bis) {
         ((bis)this.world).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY + (double)this.height / 1.5, this.posZ, 10, (double)(this.width / 4.0F), (double)(this.height / 4.0F), (double)(this.width / 4.0F), 0.05, co.getStateId(Nk.PLANKS.getDefaultState()));
      }
   ...
   private void dropBlock() {
      co.spawnAsEntity(this.world, new BlockPos(this), new Qy(NK.ARMOR_STAND));
      this.dropContents();
   }
   ...
   private void dropContents() {
      ...
      Qy itemstack1;
      for(j = 0; j < this.handItems.size(); ++j) {
         itemstack1 = (Qy)this.handItems.get(j);
         if (!itemstack1.isEmpty()) {
            co.spawnAsEntity(this.world, (new BlockPos(this)).up(), itemstack1);
            this.handItems.set(j, Qy.EMPTY);
         }
      }

      for(j = 0; j < this.armorItems.size(); ++j) {
         itemstack1 = (Qy)this.armorItems.get(j);
         if (!itemstack1.isEmpty()) {
            co.spawnAsEntity(this.world, (new BlockPos(this)).up(), itemstack1);
            this.armorItems.set(j, Qy.EMPTY);
         }
      }
   ...
   private void playBrokenSound() {
      this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_ARMORSTAND_BREAK, this.getSoundCategory(), 1.0F, 1.0F);
   }
   ...
   public hJ getPushReaction() {
      return this.hasMarker() ? hJ.IGNORE : super.getPushReaction();
   }
   ...
   protected SoundEvent getFallSound(int heightIn) {
      return NO.ENTITY_ARMORSTAND_FALL;
   }

   @Nullable
   protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
      return NO.ENTITY_ARMORSTAND_HIT;
   }

   @Nullable
   protected SoundEvent getDeathSound() {
      return NO.ENTITY_ARMORSTAND_BREAK;
   }
   ...
   static {
      STATUS = Rv.createKey(IN.class, Rt.BYTE);
      HEAD_ROTATION = Rv.createKey(IN.class, Rt.ROTATIONS);
      BODY_ROTATION = Rv.createKey(IN.class, Rt.ROTATIONS);
      LEFT_ARM_ROTATION = Rv.createKey(IN.class, Rt.ROTATIONS);
      RIGHT_ARM_ROTATION = Rv.createKey(IN.class, Rt.ROTATIONS);
      LEFT_LEG_ROTATION = Rv.createKey(IN.class, Rt.ROTATIONS);
      RIGHT_LEG_ROTATION = Rv.createKey(IN.class, Rt.ROTATIONS);
      IS_RIDEABLE_MINECART = new Predicate<Ig>() {
         public boolean apply(@Nullable Ig p_apply_1_) {
            return p_apply_1_ instanceof Jc && ((Jc)p_apply_1_).getType() == Jb.RIDEABLE;
         }
