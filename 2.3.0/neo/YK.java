...
public class YK extends Yg implements ITickable {
   private in pistonState;
   ...
   public YK(in pistonStateIn, EnumFacing pistonFacingIn, boolean extendingIn, boolean shouldHeadBeRenderedIn) {
   ...
   public AxisAlignedBB getAABB(bfZ p_184321_1_, BlockPos p_184321_2_) {
      return this.getAABB(p_184321_1_, p_184321_2_, this.progress).union(this.getAABB(p_184321_1_, p_184321_2_, this.lastProgress));
   }

   public AxisAlignedBB getAABB(bfZ p_184319_1_, BlockPos p_184319_2_, float p_184319_3_) {
      p_184319_3_ = this.getExtendedProgress(p_184319_3_);
      in iblockstate = this.getCollisionRelatedBlockState();
   ...
   private in getCollisionRelatedBlockState() {
      return !this.isExtending() && this.shouldPistonHeadBeRendered() ? Nk.PISTON_HEAD.getDefaultState().withProperty(fi.TYPE, this.pistonState.getBlock() == Nk.STICKY_PISTON ? fh.STICKY : fh.DEFAULT).withProperty(fi.FACING, this.pistonState.getValue(ff.FACING)) : this.pistonState;
   }
   ...
   private void moveCollidedEntities(float p_184322_1_) {
      ...
      this.getCollisionRelatedBlockState().addCollisionBoxToList(this.world, BlockPos.ORIGIN, new AxisAlignedBB(BlockPos.ORIGIN), list, (Ig)null, true);
      if (!list.isEmpty()) {
         AxisAlignedBB axisalignedbb = this.moveByPositionAndProgress(this.getMinMaxPiecesAABB(list));
         List<Ig> list1 = this.world.getEntitiesWithinAABBExcludingEntity((Ig)null, this.getMovementArea(axisalignedbb, enumfacing, d0).union(axisalignedbb));
         if (!list1.isEmpty()) {
            boolean flag = this.pistonState.getBlock() == Nk.SLIME_BLOCK;

            for(int i = 0; i < list1.size(); ++i) {
               Ig entity = (Ig)list1.get(i);
               if (entity.getPushReaction() != hJ.IGNORE) {
                  if (flag) {
                  ...
                  if (d1 > 0.0) {
                     ...
                     entity.move(Lq.PISTON, d1 * (double)enumfacing.getXOffset(), d1 * (double)enumfacing.getYOffset(), d1 * (double)enumfacing.getZOffset());
   ...
   private void fixEntityWithinPistonBase(Ig p_190605_1_, EnumFacing p_190605_2_, double p_190605_3_) {
      AxisAlignedBB axisalignedbb = p_190605_1_.getEntityBoundingBox();
      AxisAlignedBB axisalignedbb1 = co.FULL_BLOCK_AABB.offset(this.pos);
      if (axisalignedbb.intersects(axisalignedbb1)) {
         ...
         if (Math.abs(d0 - d1) < 0.01) {
            ...
            p_190605_1_.move(Lq.PISTON, d0 * (double)enumfacing.getXOffset(), d0 * (double)enumfacing.getYOffset(), d0 * (double)enumfacing.getZOffset());
            MOVING_ENTITY.set((Object)null);
         }
      }
   ...
   public void clearPistonTileEntity() {
      if (this.lastProgress < 1.0F && this.world != null) {
         ...
         this.invalidate();
         if (this.world.getBlockState(this.pos).getBlock() == Nk.PISTON_EXTENSION) {
   ...
   public void readFromNBT(QQ compound) {
      ...
      this.pistonState = co.getBlockById(compound.getInteger("blockId")).getStateFromMeta(compound.getInteger("blockData"));
   ...
   public QQ writeToNBT(QQ compound) {
      ...
      compound.setInteger("blockId", co.getIdFromBlock(this.pistonState.getBlock()));
   ...
   public void addCollissionAABBs(bij p_190609_1_, BlockPos p_190609_2_, AxisAlignedBB p_190609_3_, List<AxisAlignedBB> p_190609_4_, @Nullable Ig p_190609_5_) {
      if (!this.extending && this.shouldHeadBeRendered) {
         this.pistonState.withProperty(ff.EXTENDED, true).addCollisionBoxToList(p_190609_1_, p_190609_2_, p_190609_3_, p_190609_4_, p_190609_5_, false);
      }
      ...
      if ((double)this.progress >= 1.0 || enumfacing != (this.extending ? this.pistonFacing : this.pistonFacing.getOpposite())) {
         ...
         if (this.shouldPistonHeadBeRendered()) {
            iblockstate = Nk.PISTON_HEAD.getDefaultState().withProperty(fi.FACING, this.pistonFacing).withProperty(fi.SHORT, this.extending != 1.0F - this.progress < 0.25F);
         } else {
