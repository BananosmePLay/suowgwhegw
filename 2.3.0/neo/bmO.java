...
public class bmO {
   ...
   public static byte[] get(String urlStr) throws IOException {
      ...
      try {
         ...
         httpurlconnection = (HttpURLConnection)url.openConnection(nC.getMinecraft().getProxy());
   ...
   public static synchronized String getPlayerItemsUrl() {
      if (playerItemsUrl == null) {
         try {
            boolean flag = XH.parseBoolean(System.getProperty("player.models.local"), false);
            if (flag) {
               File file1 = nC.getMinecraft().gameDir;
