...
public class Cn extends Ch {
   ...
   public void execute(Xx server, DB sender, String[] args) throws Ct {
      if (args.length < 1) {
         throw new Ej("commands.debug.usage", new Object[0]);
      } else {
         if ("start".equals(args[0])) {
            ...
            this.profileStartTime = Xx.getCurrentTimeMillis();
            ...
            if (!server.profiler.profilingEnabled) {
               throw new Ct("commands.debug.notStarted", new Object[0]);
            }
   ...
   private void saveProfilerResults(long timeSpan, int tickSpan, Xx server) {
      ...
      try {
         ...
         ((Writer)writer).write(this.getProfilerResults(timeSpan, tickSpan, server));
   ...
   private String getProfilerResults(long timeSpan, int tickSpan, Xx server) {
   ...
   private void appendProfilerResults(int depth, String sectionName, StringBuilder builder, Xx server) {
      List<Wj> list = server.profiler.getProfilingData(sectionName);
      if (list != null && list.size() >= 3) {
         for(int i = 1; i < list.size(); ++i) {
            Wj profiler$result = (Wj)list.get(i);
