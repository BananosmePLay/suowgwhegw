...
public class bjb {
   ...
   public bjb(String tag, String value) {
      String[] astring = XH.tokenize(tag, ".");
      this.parents = (String[])((String[])Arrays.copyOfRange(astring, 0, astring.length - 1));
   ...
   public boolean matchesCompound(QQ nbt) {
      if (nbt == null) {
         return false;
      } else {
         QH nbtbase = nbt;
         ...
         for(int i = 0; i < this.parents.length; ++i) {
            ...
            nbtbase = getChildTag((QH)nbtbase, s);
            if (nbtbase == null) {
               return false;
            }
         }
         ...
         if (this.name.equals("*")) {
            return this.matchesAnyChild((QH)nbtbase);
         } else {
            QH nbtbase = getChildTag((QH)nbtbase, this.name);
            if (nbtbase == null) {
               return false;
   ...
   private boolean matchesAnyChild(QH tagBase) {
      QH nbtbase1;
      if (tagBase instanceof QQ) {
         QQ nbttagcompound = (QQ)tagBase;
   ...
   private static QH getChildTag(QH tagBase, String tag) {
      if (tagBase instanceof QQ) {
         QQ nbttagcompound = (QQ)tagBase;
         return nbttagcompound.getTag(tag);
      } else if (tagBase instanceof QW) {
         QW nbttaglist = (QW)tagBase;
         if (tag.equals("count")) {
            return new QU(nbttaglist.tagCount());
         } else {
            int i = XH.parseInt(tag, -1);
   ...
   public boolean matchesBase(QH nbtBase) {
      if (nbtBase == null) {
         return false;
      } else {
   ...
   private boolean matchesPattern(String str, String pattern) {
      return bqP.equalsMask(str, pattern, '*', '?');
   }
   ...
   private static String getNbtString(QH nbtBase, int format) {
      if (nbtBase == null) {
         return null;
      } else if (nbtBase instanceof Ra) {
         Ra nbttagstring = (Ra)nbtBase;
         return nbttagstring.getString();
      } else if (nbtBase instanceof QU) {
         QU nbttagint = (QU)nbtBase;
         return format == 1 ? "#" + bqP.fillLeft(Integer.toHexString(nbttagint.getInt()), 6, '0') : Integer.toString(nbttagint.getInt());
      } else if (nbtBase instanceof QM) {
         QM nbttagbyte = (QM)nbtBase;
         return Byte.toString(nbttagbyte.getByte());
      } else if (nbtBase instanceof QZ) {
         QZ nbttagshort = (QZ)nbtBase;
         return Short.toString(nbttagshort.getShort());
      } else if (nbtBase instanceof QX) {
         QX nbttaglong = (QX)nbtBase;
         return Long.toString(nbttaglong.getLong());
      } else if (nbtBase instanceof QT) {
         QT nbttagfloat = (QT)nbtBase;
         return Float.toString(nbttagfloat.getFloat());
      } else if (nbtBase instanceof QR) {
         QR nbttagdouble = (QR)nbtBase;
