...
public final class Nh {
   ...
   public static RayTraceResult forwardsRaycast(Ig projectile, boolean includeEntities, boolean ignoreExcludedEntity, Ig excludedEntity) {
      ...
      if (includeEntities) {
         ...
         Ig entity = null;
         List<Ig> list = world.getEntitiesWithinAABBExcludingEntity(projectile, projectile.getEntityBoundingBox().expand(d3, d4, d5).grow(1.0));
         double d6 = 0.0;
   ...
   public static final void rotateTowardsMovement(Ig projectile, float rotationSpeed) {
      double d0 = projectile.motionX;
      double d1 = projectile.motionY;
      double d2 = projectile.motionZ;
      float f = MathHelper.sqrt(d0 * d0 + d2 * d2);
      projectile.rotationYaw = (float)(MathHelper.atan2(d2, d0) * 57.29577951308232) + 90.0F;

      for(projectile.rotationPitch = (float)(MathHelper.atan2((double)f, d1) * 57.29577951308232) - 90.0F; projectile.rotationPitch - projectile.prevRotationPitch < -180.0F; projectile.prevRotationPitch -= 360.0F) {
      }
