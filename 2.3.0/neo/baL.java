...
@Immutable
public class baL {
   private final baV worldDifficulty;
   private final float additionalDifficulty;

   public baL(baV worldDifficulty, long worldTime, long chunkInhabitedTime, float moonPhaseFactor) {
   ...
   private float calculateAdditionalDifficulty(baV difficulty, long worldTime, long chunkInhabitedTime, float moonPhaseFactor) {
      if (difficulty == baV.PEACEFUL) {
         return 0.0F;
      } else {
         boolean flag = difficulty == baV.HARD;
         float f = 0.75F;
