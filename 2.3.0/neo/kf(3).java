...
public class kf extends lU {
   private static final ResourceLocation ENCHANTMENT_TABLE_GUI_TEXTURE = new ResourceLocation("textures/gui/container/enchanting_table.png");
   private static final ResourceLocation ENCHANTMENT_TABLE_BOOK_TEXTURE = new ResourceLocation("textures/entity/enchanting_table_book.png");
   private static final nP MODEL_BOOK = new nP();
   ...
   public float oOpen;
   private Qy last;
   private final bgd nameable;

   public kf(MJ inventory, bij worldIn, bgd nameable) {
   ...
   protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
      ...
      for(int k = 0; k < 3; ++k) {
         ...
         if (l >= 0 && i1 >= 0 && l < 108 && i1 < 19) {
            nC var10001 = this.mc;
            if (this.container.enchantItem(nC.player, k)) {
   ...
   protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
      ...
      yh.viewport((scaledresolution.getScaledWidth() - 320) / 2 * scaledresolution.getScaleFactor(), (scaledresolution.getScaledHeight() - 240) / 2 * scaledresolution.getScaleFactor(), 320 * scaledresolution.getScaleFactor(), 240 * scaledresolution.getScaleFactor());
      yh.translate(-0.34F, 0.23F, 0.0F);
      Project.gluPerspective(90.0F, 1.3333334F, 9.0F, 80.0F);
      float f = 1.0F;
      yh.matrixMode(5888);
      yh.loadIdentity();
      yz.enableStandardItemLighting();
      yh.translate(0.0F, 3.3F, -16.0F);
      yh.scale(1.0F, 1.0F, 1.0F);
      float f1 = 5.0F;
      yh.scale(5.0F, 5.0F, 5.0F);
      ...
      yh.translate((1.0F - f2) * 0.2F, (1.0F - f2) * 0.1F, (1.0F - f2) * 0.25F);
      yh.rotate(-(1.0F - f2) * 90.0F - 90.0F, 0.0F, 1.0F, 0.0F);
      ...
      MODEL_BOOK.render((Ig)null, 0.0F, f3, f4, f2, 0.0F, 0.0625F);
      ...
      for(int l = 0; l < 3; ++l) {
         ...
         if (k1 == 0) {
            ...
            label66: {
               ...
               label54: {
                  s = "" + k1;
                  l1 = 86 - this.fontRenderer.getStringWidth(s);
                  s1 = EnchantmentNameParts.getInstance().generateNewRandomName(this.fontRenderer, l1);
                  fontrenderer = this.mc.standardGalacticFontRenderer;
                  i2 = 6839882;
                  nC var10000;
                  if (k >= l + 1) {
                     var10000 = this.mc;
                     if (nC.player.experienceLevel >= k1) {
                        break label54;
                     }
                  }

                  var10000 = this.mc;
                  if (!nC.player.capabilities.isCreativeMode) {
                     this.drawTexturedModalRect(i1, j + 14 + 19 * l, 0, 185, 108, 19);
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      ...
      nC var10000 = this.mc;
      boolean flag = nC.player.capabilities.isCreativeMode;
      int i = this.container.getLapisAmount();

      for(int j = 0; j < 3; ++j) {
         int k = this.container.enchantLevels[j];
         Fa enchantment = Fa.getEnchantmentByID(this.container.enchantClue[j]);
         int l = this.container.worldClue[j];
         int i1 = j + 1;
         if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0 && l >= 0 && enchantment != null) {
            List<String> list = Lists.newArrayList();
            list.add("" + TextFormatting.WHITE + TextFormatting.ITALIC + Ax.format("container.enchant.clue", enchantment.getTranslatedName(l)));
            if (!flag) {
               list.add("");
               var10000 = this.mc;
               if (nC.player.experienceLevel < k) {
                  list.add(TextFormatting.RED + Ax.format("container.enchant.level.requirement", this.container.enchantLevels[j]));
               } else {
                  String s;
                  if (i1 == 1) {
                     s = Ax.format("container.enchant.lapis.one");
                  } else {
                     s = Ax.format("container.enchant.lapis.many", i1);
                  }

                  TextFormatting textformatting = i >= i1 ? TextFormatting.GRAY : TextFormatting.RED;
                  list.add(textformatting + "" + s);
                  if (i1 == 1) {
                     s = Ax.format("container.enchant.level.one");
                  } else {
                     s = Ax.format("container.enchant.level.many", i1);
                  }
   ...
   public void tickBook() {
      Qy itemstack = this.inventorySlots.getSlot(0).getStack();
      if (!Qy.areItemStacksEqual(itemstack, this.last)) {
         this.last = itemstack;
