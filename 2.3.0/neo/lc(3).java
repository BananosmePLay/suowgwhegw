...
public class lc extends lU implements IContainerListener {
   private static final ResourceLocation ANVIL_RESOURCE = new ResourceLocation("textures/gui/container/anvil.png");
   private final ContainerRepair anvil;
   private lE nameField;
   private final MJ playerInventory;

   public lc(MJ inventoryIn, bij worldIn) {
      nC.getMinecraft();
      super(new ContainerRepair(inventoryIn, worldIn, nC.player));
   ...
   public void initGui() {
      ...
      this.nameField = new lE(0, this.fontRenderer, i + 62, j + 24, 103, 12);
   ...
   protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
      yh.disableLighting();
      yh.disableBlend();
      this.fontRenderer.drawString(Ax.format("container.repair"), 60, 6, 4210752);
      if (this.anvil.maximumCost > 0) {
         ...
         label26: {
            i = 8453920;
            flag = true;
            s = Ax.format("container.repair.cost", this.anvil.maximumCost);
            if (this.anvil.maximumCost >= 40) {
               nC var10000 = this.mc;
               if (!nC.player.capabilities.isCreativeMode) {
                  s = Ax.format("container.repair.expensive");
                  i = 16736352;
                  break label26;
               }
            }
   ...
   private void renameItem() {
      ...
      nC var10000 = this.mc;
      nC.player.connection.sendPacket(new SN("MC|ItemName", (new SA(Unpooled.buffer())).writeString(s)));
   }
