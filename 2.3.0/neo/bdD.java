...
public abstract class bdD extends bdB {
   private static final bfD DEFAULT_PLACE_SETTINGS = new bfD();
   protected bfK template;
   protected bfD placeSettings;
   protected BlockPos templatePosition;

   public bdD() {
      this.placeSettings = DEFAULT_PLACE_SETTINGS.setIgnoreEntities(true).setReplacedBlock(Nk.AIR);
   }
   ...
   protected void setup(bfK templateIn, BlockPos pos, bfD settings) {
   ...
   public boolean addComponentParts(bij worldIn, Random randomIn, bdy structureBoundingBoxIn) {
      ...
      while(var5.hasNext()) {
         Map.Entry<BlockPos, String> entry = (Map.Entry)var5.next();
   ...
   protected abstract void handleDataMarker(String var1, BlockPos var2, bij var3, Random var4, bdy var5);
   ...
   private void setBoundingBoxFromTemplate() {
      ...
      this.boundingBox = new bdy(0, 0, 0, blockpos.getX(), blockpos.getY() - 1, blockpos.getZ());
      switch (rotation) {
         case NONE:
         default:
            break;
