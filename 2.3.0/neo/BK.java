...
public class BK {
   ...
   private BK(boolean separateBlendIn, boolean opaqueIn, int srcColorFactorIn, int destColorFactorIn, int srcAlphaFactorIn, int destAlphaFactorIn, int blendFunctionIn) {
   ...
   public BK(int srcFactor, int dstFactor, int blendFunctionIn) {
      this(false, false, srcFactor, dstFactor, srcFactor, dstFactor, blendFunctionIn);
   }

   public BK(int srcColorFactorIn, int destColorFactorIn, int srcAlphaFactorIn, int destAlphaFactorIn, int blendFunctionIn) {
   ...
   public void apply() {
      if (!this.equals(lastApplied)) {
         ...
         yh.glBlendEquation(this.blendFunction);
         if (this.separateBlend) {
            yh.tryBlendFuncSeparate(this.srcColorFactor, this.destColorFactor, this.srcAlphaFactor, this.destAlphaFactor);
         } else {
            yh.blendFunc(this.srcColorFactor, this.destColorFactor);
         }
      }
   ...
   public boolean equals(Object p_equals_1_) {
      if (this == p_equals_1_) {
         return true;
      } else if (!(p_equals_1_ instanceof BK)) {
         return false;
      } else {
         BK jsonblendingmode = (BK)p_equals_1_;
   ...
   public static BK parseBlendNode(JsonObject json) {
      if (json == null) {
         return new BK();
      } else {
         int i = 32774;
         ...
         if (flag) {
            ...
            return flag1 ? new BK(j, k, l, i1, i) : new BK(j, k, i);
         }
      }
   }
