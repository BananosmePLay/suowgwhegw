...
public class zT {
   protected final yy renderGlobal;
   protected final bij world;
   protected int countChunksY;
   protected int countChunksX;
   protected int countChunksZ;
   public ug[] renderChunks;
   private Map<ChunkPos, boc[]> mapVboRegions = new HashMap();

   public zT(bij worldIn, int renderDistanceChunks, yy renderGlobalIn, uc renderChunkFactory) {
   ...
   protected void createRenderChunks(uc renderChunkFactory) {
      int i = this.countChunksX * this.countChunksY * this.countChunksZ;
      this.renderChunks = new ug[i];
      int j = 0;
      ...
      for(k = 0; k < this.countChunksX; ++k) {
         for(int l = 0; l < this.countChunksY; ++l) {
            for(l1 = 0; l1 < this.countChunksZ; ++l1) {
               ...
               this.renderChunks[j1].setPosition(k * 16, l * 16, l1 * 16);
               if (XH.isVbo() && XH.isRenderRegions()) {
      ...
      for(k = 0; k < this.renderChunks.length; ++k) {
         ug renderchunk1 = this.renderChunks[k];
         ...
         for(l1 = 0; l1 < EnumFacing.VALUES.length; ++l1) {
            ...
            ug renderchunk = this.getRenderChunk(blockpos);
   ...
   public void deleteGlResources() {
      ug[] var1 = this.renderChunks;
      int var2 = var1.length;

      for(int var3 = 0; var3 < var2; ++var3) {
         ug renderchunk = var1[var3];
      ...
      for(int l = 0; l < this.countChunksX; ++l) {
         ...
         for(int j1 = 0; j1 < this.countChunksZ; ++j1) {
            ...
            for(int l1 = 0; l1 < this.countChunksY; ++l1) {
               int i2 = l1 * 16;
               ug renderchunk = this.renderChunks[(j1 * this.countChunksY + l1) * this.countChunksX + l];
      ...
      for(int k1 = i; k1 <= l; ++k1) {
         ...
         for(int i2 = j; i2 <= i1; ++i2) {
            ...
            for(int k2 = k; k2 <= j1; ++k2) {
               ...
               ug renderchunk = this.renderChunks[i3];
   ...
   @Nullable
   public ug getRenderChunk(BlockPos pos) {
      int i = pos.getX() >> 4;
      int j = pos.getY() >> 4;
      int k = pos.getZ() >> 4;
   ...
   private void updateVboRegion(ug p_updateVboRegion_1_) {
      ...
      boc[] avboregion = (boc[])this.mapVboRegions.get(chunkpos);
      int l;
      if (avboregion == null) {
         avboregion = new boc[ablockrenderlayer.length];

         for(l = 0; l < ablockrenderlayer.length; ++l) {
            avboregion[l] = new boc(ablockrenderlayer[l]);
         }
   ...
   public void deleteVboRegions() {
      ...
      while(var1.hasNext()) {
         ChunkPos chunkpos = (ChunkPos)var1.next();
         boc[] avboregion = (boc[])this.mapVboRegions.get(chunkpos);

         for(int i = 0; i < avboregion.length; ++i) {
            boc vboregion = avboregion[i];
            if (vboregion != null) {
