...
public class Cj extends Ch {
   ...
   public void execute(Xx server, DB sender, String[] args) throws Ct {
      MG entityplayermp = args.length == 0 ? getCommandSenderAsPlayer(sender) : getPlayer(server, sender, args[0]);
      OL item = args.length >= 2 ? getItemByText(sender, args[1]) : null;
      int i = args.length >= 3 ? parseInt(args[2], -1) : -1;
      int j = args.length >= 4 ? parseInt(args[3], -1) : -1;
      QQ nbttagcompound = null;
      if (args.length >= 5) {
         try {
            nbttagcompound = QG.getTagFromJson(buildString(args, 4));
         } catch (QI var10) {
            QI nbtexception = var10;
            throw new Ct("commands.clear.tagError", new Object[]{nbtexception.getMessage()});
         }
      }

      if (args.length >= 2 && item == null) {
         throw new Ct("commands.clear.failure", new Object[]{entityplayermp.getName()});
      } else {
