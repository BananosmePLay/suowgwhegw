...
public abstract class vI<T extends Ig> implements bkm {
   ...
   public boolean shouldRender(T livingEntity, uO camera, double camX, double camY, double camZ) {
   ...
   protected int getTeamColor(T entityIn) {
      int i = 16777215;
      WA scoreplayerteam = (WA)entityIn.getTeam();
      if (scoreplayerteam != null) {
         String s = jH.getFormatFromString(scoreplayerteam.getPrefix());
         if (s.length() >= 2) {
   ...
   private void renderEntityOnFire(Ig entity, double x, double y, double z, float partialTicks) {
      yh.disableLighting();
      zj texturemap = nC.getMinecraft().getTextureMapBlocks();
      zd textureatlassprite = texturemap.getAtlasSprite("minecraft:blocks/fire_layer_0");
      zd textureatlassprite1 = texturemap.getAtlasSprite("minecraft:blocks/fire_layer_1");
      yh.pushMatrix();
      ...
      yh.translate(0.0F, 0.0F, -0.3F + (float)((int)f3) * 0.02F);
      yh.color(1.0F, 1.0F, 1.0F, 1.0F);
      float f5 = 0.0F;
      int i = 0;
      boolean flag = XH.isMultiTexture();
      if (flag) {
      ...
      while(f3 > 0.0F) {
         zd textureatlassprite2 = i % 2 == 0 ? textureatlassprite : textureatlassprite1;
   ...
   private void renderShadow(Ig entityIn, double x, double y, double z, float shadowAlpha, float partialTicks) {
      if (!XH.isShaders() || !bpq.shouldSkipDefaultShadow) {
         yh.enableBlend();
         yh.blendFunc(ya.SRC_ALPHA, xR.ONE_MINUS_SRC_ALPHA);
         this.renderManager.renderEngine.bindTexture(SHADOW_TEXTURES);
         bij world = this.getWorldFromRenderManager();
         yh.depthMask(false);
         float f = this.shadowSize;
         if (entityIn instanceof Iu) {
            Iu entityliving = (Iu)entityIn;
         ...
         while(var32.hasNext()) {
            ...
            in iblockstate = world.getBlockState(blockpos.down());
   ...
   private bij getWorldFromRenderManager() {
      return this.renderManager.world;
   }

   private void renderShadowSingle(in state, double p_188299_2_, double p_188299_4_, double p_188299_6_, BlockPos p_188299_8_, float p_188299_9_, float p_188299_10_, double p_188299_11_, double p_188299_13_, double p_188299_15_) {
      if (state.isFullCube()) {
         yN tessellator = yN.getInstance();
         tN bufferbuilder = tessellator.getBuffer();
   ...
   public static void renderOffsetAABB(AxisAlignedBB boundingBox, double x, double y, double z) {
      ...
      bufferbuilder.begin(7, zK.POSITION_NORMAL);
   ...
   public void doRenderShadowAndFire(Ig entityIn, double x, double y, double z, float yaw, float partialTicks) {
      if (this.renderManager.options != null) {
         ...
         if (entityIn.canRenderOnFire() && (!(entityIn instanceof ME) || !((ME)entityIn).isSpectator())) {
   ...
   public jH getFontRendererFromRenderManager() {
   ...
   protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance) {
      ...
      if (d0 <= (double)(maxDistance * maxDistance)) {
         ...
         xz.drawNameplate(this.getFontRendererFromRenderManager(), str, (float)x, (float)y + f2, (float)z, i, f, f1, flag1, flag);
      }
