...
public class Ds {
   ...
   private static final Predicate<String> IS_VALID_ARGUMENT = new Predicate<String>() {
      public boolean apply(@Nullable String p_apply_1_) {
         return p_apply_1_ != null && (Ds.VALID_ARGUMENTS.contains(p_apply_1_) || p_apply_1_.length() > "score_".length() && p_apply_1_.startsWith("score_"));
      }
   ...
   @Nullable
   public static MG matchOnePlayer(DB sender, String token) throws Ct {
      return (MG)matchOneEntity(sender, token, MG.class);
   }

   public static List<MG> getPlayers(DB sender, String token) throws Ct {
      return matchEntities(sender, token, MG.class);
   }

   @Nullable
   public static <T extends Ig> T matchOneEntity(DB sender, String token, Class<? extends T> targetClass) throws Ct {
      List<T> list = matchEntities(sender, token, targetClass);
      return list.size() == 1 ? (Ig)list.get(0) : null;
   }

   @Nullable
   public static ITextComponent matchEntitiesToTextComponent(DB sender, String token) throws Ct {
      List<Ig> list = matchEntities(sender, token, Ig.class);
      if (list.isEmpty()) {
         return null;
      } else {
         ...
         return Ch.join(list1);
      }
   }

   public static <T extends Ig> List<T> matchEntities(DB sender, String token, Class<? extends T> targetClass) throws Ct {
      ...
      if (matcher.matches() && sender.canUseCommand(1, "@")) {
         ...
         if (!isEntityTypeValid(sender, map)) {
            ...
            List<bij> list = getWorlds(sender, map);
            List<T> list1 = Lists.newArrayList();
            Iterator var10 = list.iterator();

            while(var10.hasNext()) {
               bij world = (bij)var10.next();
               if (world != null) {
                  List<Predicate<Ig>> list2 = Lists.newArrayList();
                  ...
                  if ("s".equalsIgnoreCase(s)) {
                     ...
                     if (entity != null && targetClass.isAssignableFrom(entity.getClass())) {
                        ...
                        do {
                           if (!var18.hasNext()) {
                              return Lists.newArrayList(new Ig[]{entity});
                           }
   ...
   private static List<bij> getWorlds(DB sender, Map<String, String> argumentMap) {
      List<bij> list = Lists.newArrayList();
   ...
   private static <T extends Ig> boolean isEntityTypeValid(DB commandSender, Map<String, String> params) {
   ...
   private static List<Predicate<Ig>> getTypePredicates(Map<String, String> params, String type) {
      String s = getArgument(params, ARGUMENT_ENTITY_TYPE);
      if (s == null || !type.equals("e") && !type.equals("r") && !type.equals("s")) {
         return !type.equals("e") && !type.equals("s") ? Collections.singletonList(new Predicate<Ig>() {
            public boolean apply(@Nullable Ig p_apply_1_) {
               return p_apply_1_ instanceof ME;
            }
         ...
         return Collections.singletonList(new Predicate<Ig>() {
            public boolean apply(@Nullable Ig p_apply_1_) {
               return Ir.isMatchingName(p_apply_1_, resourcelocation) != flag;
            }
   ...
   private static List<Predicate<Ig>> getXpLevelPredicates(Map<String, String> params) {
      List<Predicate<Ig>> list = Lists.newArrayList();
      final int i = getInt(params, ARGUMENT_LEVEL_MIN, -1);
      final int j = getInt(params, ARGUMENT_LEVEL_MAX, -1);
      if (i > -1 || j > -1) {
         list.add(new Predicate<Ig>() {
            public boolean apply(@Nullable Ig p_apply_1_) {
               if (!(p_apply_1_ instanceof MG)) {
                  return false;
               } else {
                  MG entityplayermp = (MG)p_apply_1_;
   ...
   private static List<Predicate<Ig>> getGamemodePredicates(Map<String, String> params) {
      List<Predicate<Ig>> list = Lists.newArrayList();
      ...
      if (s == null) {
         ...
         try {
            ...
            gametype = bbb.parseGameTypeWithDefault(i, bbb.NOT_SET);
         } catch (Throwable var6) {
            gametype = bbb.parseGameTypeWithDefault(s, bbb.NOT_SET);
         }

         final bbb type = gametype;
         list.add(new Predicate<Ig>() {
            public boolean apply(@Nullable Ig p_apply_1_) {
               if (!(p_apply_1_ instanceof MG)) {
                  return false;
                  ...
                  MG entityplayermp = (MG)p_apply_1_;
                  bbb gametype1 = entityplayermp.interactionManager.getGameType();
   ...
   private static List<Predicate<Ig>> getTeamPredicates(Map<String, String> params) {
      List<Predicate<Ig>> list = Lists.newArrayList();
      ...
      if (s != null) {
         ...
         list.add(new Predicate<Ig>() {
            public boolean apply(@Nullable Ig p_apply_1_) {
               if (!(p_apply_1_ instanceof Iw)) {
                  return false;
               } else {
                  Iw entitylivingbase = (Iw)p_apply_1_;
                  WE team = entitylivingbase.getTeam();
   ...
   private static List<Predicate<Ig>> getScorePredicates(final DB sender, Map<String, String> params) {
      final Map<String, Integer> map = getScoreMap(params);
      return (List)(map.isEmpty() ? Collections.emptyList() : Lists.newArrayList(new Predicate[]{new Predicate<Ig>() {
         public boolean apply(@Nullable Ig p_apply_1_) {
            if (p_apply_1_ == null) {
               return false;
            } else {
               Ws scoreboard = sender.getServer().getWorld(0).getScoreboard();
               ...
               do {
                  ...
                  Wz scoreobjective = scoreboard.getObjective(s);
                  if (scoreobjective == null) {
                     return false;
                  }

                  String s1 = p_apply_1_ instanceof MG ? p_apply_1_.getName() : p_apply_1_.getCachedUniqueIdString();
   ...
   private static List<Predicate<Ig>> getNamePredicates(Map<String, String> params) {
      List<Predicate<Ig>> list = Lists.newArrayList();
   ...
   private static List<Predicate<Ig>> getTagPredicates(Map<String, String> params) {
      List<Predicate<Ig>> list = Lists.newArrayList();
   ...
   private static List<Predicate<Ig>> getRadiusPredicates(Map<String, String> params, final Vec3d pos) {
      ...
      if (flag && flag1) {
         ...
         return Lists.newArrayList(new Predicate[]{new Predicate<Ig>() {
            public boolean apply(@Nullable Ig p_apply_1_) {
               if (p_apply_1_ == null) {
                  return false;
               } else {
   ...
   private static List<Predicate<Ig>> getRotationsPredicates(Map<String, String> params) {
      List<Predicate<Ig>> list = Lists.newArrayList();
      final int k;
      final int l;
   ...
   private static <T extends Ig> List<T> filterResults(Map<String, String> params, Class<? extends T> entityClass, List<Predicate<Ig>> inputList, String type, bij worldIn, BlockPos position) {
      ...
      Predicate<Ig> predicate = Predicates.and(inputList);
      Predicate<Ig> predicate1 = Predicates.and(EntitySelectors.IS_ALIVE, predicate);
      ...
      if (!params.containsKey(ARGUMENT_DELTA_X) && !params.containsKey(ARGUMENT_DELTA_Y) && !params.containsKey(ARGUMENT_DELTA_Z)) {
         ...
         if (flag && !flag1) {
            Predicate<Ig> predicate2 = new Predicate<Ig>() {
               public boolean apply(@Nullable Ig p_apply_1_) {
   ...
   private static <T extends Ig> List<T> getEntitiesFromPredicates(List<T> matchingEntities, Map<String, String> params, DB sender, Class<? extends T> targetClass, String type, final Vec3d pos) {
      ...
      if (!type.equals("p") && !type.equals("a") && !type.equals("e")) {
         ...
         Collections.sort((List)matchingEntities, new Comparator<Ig>() {
      ...
      if (entity != null && targetClass.isAssignableFrom(entity.getClass()) && i == 1 && ((List)matchingEntities).contains(entity) && !"r".equals(type)) {
         matchingEntities = Lists.newArrayList(new Ig[]{entity});
      }
   ...
   private static double getCoordinate(Map<String, String> params, String key, double defaultD, boolean offset) {
      return params.containsKey(key) ? (double)MathHelper.getInt((String)params.get(key), MathHelper.floor(defaultD)) + (offset ? 0.5 : 0.0) : defaultD;
   }
   ...
   public static boolean matchesMultiplePlayers(String selectorStr) throws Ct {
   ...
   private static Map<String, String> getArgumentMap(@Nullable String argumentString) throws Ct {
      Map<String, String> map = Maps.newHashMap();
      if (argumentString == null) {
         return map;
      } else {
         Iterator var2 = COMMA_SPLITTER.split(argumentString).iterator();
         ...
         while(var2.hasNext()) {
            ...
            if (!IS_VALID_ARGUMENT.apply(s1)) {
               throw new Ct("commands.generic.selector_argument", new Object[]{s});
            }
   ...
   static {
      WORLD_BINDING_ARGS = Sets.newHashSet(new String[]{ARGUMENT_COORDINATE_X, ARGUMENT_COORDINATE_Y, ARGUMENT_COORDINATE_Z, ARGUMENT_DELTA_X, ARGUMENT_DELTA_Y, ARGUMENT_DELTA_Z, ARGUMENT_RANGE_MIN, ARGUMENT_RANGE_MAX});
   }
}
