...
public class bpz {
   ...
   public static void initVertexBuilder(tN wrr) {
      wrr.sVertexBuilder = new bpz();
   }
   ...
   public static void pushEntity(in blockState, BlockPos blockPos, bfZ blockAccess, tN wrr) {
      ...
      if (blockState instanceof ie) {
         ...
         i = co.getIdFromBlock(block);
         j = block.getMetaFromState(blockState);
      }

      int j1 = boe.getBlockAliasId(i, j);
      if (j1 >= 0) {
         i = j1;
      }
   ...
   public static void popEntity(tN wrr) {
      wrr.sVertexBuilder.popEntity();
   }

   public static boolean popEntity(boolean value, tN wrr) {
      wrr.sVertexBuilder.popEntity();
      return value;
   }

   public static void endSetVertexFormat(tN wrr) {
      bpz svertexbuilder = wrr.sVertexBuilder;
      zO vertexformat = wrr.getVertexFormat();
   ...
   public static void beginAddVertex(tN wrr) {
   ...
   public static void endAddVertex(tN wrr) {
      bpz svertexbuilder = wrr.sVertexBuilder;
   ...
   public static void beginAddVertexData(tN wrr, int[] data) {
      if (wrr.vertexCount == 0) {
         endSetVertexFormat(wrr);
      }
   ...
   public static void beginAddVertexData(tN wrr, ByteBuffer byteBuffer) {
      if (wrr.vertexCount == 0) {
         endSetVertexFormat(wrr);
      }
   ...
   public static void endAddVertexData(tN wrr) {
      bpz svertexbuilder = wrr.sVertexBuilder;
   ...
   public void calcNormal(tN wrr, int baseIndex) {
   ...
   public static void calcNormalChunkLayer(tN wrr) {
   ...
   public static void drawArrays(int drawMode, int first, int count, tN wrr) {
      if (count != 0) {
         zO vertexformat = wrr.getVertexFormat();
         ...
         if (i == 56) {
            ...
            GL20.glVertexAttribPointer(bpq.midTexCoordAttrib, 2, 5126, false, i, bytebuffer);
            bytebuffer.position(40);
            GL20.glVertexAttribPointer(bpq.tangentAttrib, 4, 5122, false, i, bytebuffer);
            bytebuffer.position(48);
            GL20.glVertexAttribPointer(bpq.entityAttrib, 3, 5122, false, i, bytebuffer);
            ...
            yh.glDrawArrays(drawMode, first, count);
