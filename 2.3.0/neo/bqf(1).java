...
public class bqf {
   private static bqd[] textureAnimations = null;
   ...
   public static void updateAnimations() {
      if (textureAnimations != null && XH.isAnimatedTextures()) {
         int i = 0;

         int k;
         for(k = 0; k < textureAnimations.length; ++k) {
            bqd textureanimation = textureAnimations[k];
         ...
         k = XH.getMinecraft().entityRenderer.frameCount;
   ...
   private static bqd[] getTextureAnimations(AC[] rps) {
      List list = new ArrayList();

      for(int i = 0; i < rps.length; ++i) {
         AC iresourcepack = rps[i];
         bqd[] atextureanimation = getTextureAnimations(iresourcepack);
         if (atextureanimation != null) {
            list.addAll(Arrays.asList(atextureanimation));
         }
      }

      bqd[] atextureanimation1 = (bqd[])((bqd[])list.toArray(new bqd[list.size()]));
      return atextureanimation1;
   }

   private static bqd[] getTextureAnimations(AC rp) {
      String[] astring = bqN.collectFiles(rp, "mcpatcher/anim/", ".properties", (String[])null);
      if (astring.length <= 0) {
         return null;
      } else {
         List list = new ArrayList();
         ...
         for(int i = 0; i < astring.length; ++i) {
            ...
            try {
               ...
               inputstream.close();
               bqd textureanimation = makeTextureAnimation(properties, resourcelocation);
               if (textureanimation != null) {
                  ResourceLocation resourcelocation1 = new ResourceLocation(textureanimation.getDstTex());
                  if (XH.getDefiningResourcePack(resourcelocation1) != rp) {
                     XH.dbg("Skipped: " + s + ", target texture not loaded from same resource pack");
                  } else {
                     list.add(textureanimation);
                  }
               }
            } catch (FileNotFoundException var10) {
               XH.warn("File not found: " + var10.getMessage());
         ...
         bqd[] atextureanimation = (bqd[])((bqd[])list.toArray(new bqd[list.size()]));
         return atextureanimation;
      }
   }

   private static bqd makeTextureAnimation(Properties props, ResourceLocation propLoc) {
      String s = props.getProperty("from");
      String s1 = props.getProperty("to");
      int i = XH.parseInt(props.getProperty("x"), -1);
      int j = XH.parseInt(props.getProperty("y"), -1);
      int k = XH.parseInt(props.getProperty("w"), -1);
      int l = XH.parseInt(props.getProperty("h"), -1);
      if (s != null && s1 != null) {
         if (i >= 0 && j >= 0 && k >= 0 && l >= 0) {
            s = s.trim();
            s1 = s1.trim();
            String s2 = bqS.getBasePath(propLoc.getPath());
            s = bqS.fixResourcePath(s, s2);
            s1 = bqS.fixResourcePath(s1, s2);
            byte[] abyte = getCustomTextureData(s, k);
            if (abyte == null) {
               XH.warn("TextureAnimation: Source texture not found: " + s1);
               return null;
            } else {
               ...
               int k1 = j1 * k * l;
               if (i1 != k1) {
                  XH.warn("TextureAnimation: Source texture has invalid number of frames: " + s + ", frames: " + (float)i1 / (float)(k * l));
                  return null;
               } else {
                  ...
                  try {
                     ...
                     if (inputstream == null) {
                        XH.warn("TextureAnimation: Target texture not found: " + s1);
                        return null;
                     } else {
                        BufferedImage bufferedimage = readTextureImage(inputstream);
                        if (i + k <= bufferedimage.getWidth() && j + l <= bufferedimage.getHeight()) {
                           bqd textureanimation = new bqd(s, abyte, s1, resourcelocation, i, j, k, l, props);
                           return textureanimation;
                        } else {
                           XH.warn("TextureAnimation: Animation coordinates are outside the target texture: " + s1);
                           return null;
                        }
                     }
            ...
            XH.warn("TextureAnimation: Invalid coordinates");
            return null;
         }
      } else {
         XH.warn("TextureAnimation: Source or target texture not specified");
         return null;
      }
   }
