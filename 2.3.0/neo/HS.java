...
public class HS extends Iu implements IE, Lo {
   private static final Logger LOGGER = LogManager.getLogger();
   public static final Rd<Integer> PHASE;
   public double[][] ringBuffer = new double[64][3];
   public int ringBufferIndex = -1;
   public Lr[] dragonPartArray;
   public Lr dragonPartHead = new Lr(this, "head", 6.0F, 6.0F);
   public Lr dragonPartNeck = new Lr(this, "neck", 6.0F, 6.0F);
   public Lr dragonPartBody = new Lr(this, "body", 8.0F, 8.0F);
   public Lr dragonPartTail1 = new Lr(this, "tail", 4.0F, 4.0F);
   public Lr dragonPartTail2 = new Lr(this, "tail", 4.0F, 4.0F);
   public Lr dragonPartTail3 = new Lr(this, "tail", 4.0F, 4.0F);
   public Lr dragonPartWing1 = new Lr(this, "wing", 4.0F, 4.0F);
   public Lr dragonPartWing2 = new Lr(this, "wing", 4.0F, 4.0F);
   public float prevAnimTime;
   public float animTime;
   public boolean slowed;
   public int deathTicks;
   public IS healingEnderCrystal;
   private final baN fightManager;
   private final HL phaseManager;
   private int growlTime = 200;
   private int sittingDamageReceived;
   private final VR[] pathPoints = new VR[24];
   private final int[] neighbors = new int[24];
   private final VK pathFindQueue = new VK();

   public HS(bij worldIn) {
      super(worldIn);
      this.dragonPartArray = new Lr[]{this.dragonPartHead, this.dragonPartNeck, this.dragonPartBody, this.dragonPartTail1, this.dragonPartTail2, this.dragonPartTail3, this.dragonPartWing1, this.dragonPartWing2};
      ...
      if (!worldIn.isRemote && worldIn.provider instanceof bim) {
         this.fightManager = ((bim)worldIn.provider).getDragonFightManager();
      } else {
         this.fightManager = null;
      }

      this.phaseManager = new HL(this);
   }

   protected void applyEntityAttributes() {
      super.applyEntityAttributes();
      this.getEntityAttribute(Ni.MAX_HEALTH).setBaseValue(200.0);
   }

   protected void entityInit() {
      super.entityInit();
      this.getDataManager().register(PHASE, HK.HOVER.getId());
   }
   ...
   public void onLivingUpdate() {
      ...
      if (this.world.isRemote) {
         ...
         if (!this.isSilent()) {
            ...
            if (f1 <= -0.3F && f11 >= -0.3F) {
               this.world.playSound(this.posX, this.posY, this.posZ, NO.ENTITY_ENDERDRAGON_FLAP, this.getSoundCategory(), 5.0F, 0.8F + this.rand.nextFloat() * 0.3F, false);
            }

            if (!this.phaseManager.getCurrentPhase().getIsStationary() && --this.growlTime < 0) {
               this.world.playSound(this.posX, this.posY, this.posZ, NO.ENTITY_ENDERDRAGON_GROWL, this.getSoundCategory(), 2.5F, 0.8F + this.rand.nextFloat() * 0.3F, false);
      ...
      if (this.getHealth() <= 0.0F) {
         ...
         this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + (double)f11, this.posY + 2.0 + (double)f1, this.posZ + (double)f14, 0.0, 0.0, 0.0);
      } else {
         ...
         if (this.isAIDisabled()) {
            ...
            if (this.world.isRemote) {
               ...
               HB iphase = this.phaseManager.getCurrentPhase();
               ...
               if (vec3d != null) {
                  d6 = vec3d.x - this.posX;
                  d7 = vec3d.y - this.posY;
                  d8 = vec3d.z - this.posZ;
                  ...
                  this.motionY += d7 * 0.10000000149011612;
                  this.rotationYaw = MathHelper.wrapDegrees(this.rotationYaw);
                  ...
                  f23 = 0.06F;
                  this.moveRelative(0.0F, 0.0F, -1.0F, 0.06F * (f6 * f22 + (1.0F - f22)));
                  if (this.slowed) {
                     this.move(Lq.SELF, this.motionX * 0.800000011920929, this.motionY * 0.800000011920929, this.motionZ * 0.800000011920929);
                  } else {
   ...
   private void updateDragonEnderCrystal() {
      ...
      if (this.rand.nextInt(10) == 0) {
         List<IS> list = this.world.getEntitiesWithinAABB(IS.class, this.getEntityBoundingBox().grow(32.0));
         IS entityendercrystal = null;
         double d0 = Double.MAX_VALUE;
         Iterator var5 = list.iterator();

         while(var5.hasNext()) {
            IS entityendercrystal1 = (IS)var5.next();
   ...
   private void collideWithEntities(List<Ig> p_70970_1_) {
      ...
      while(var6.hasNext()) {
         Ig entity = (Ig)var6.next();
         if (entity instanceof Iw) {
            double d2 = entity.posX - d0;
            double d3 = entity.posZ - d1;
            double d4 = d2 * d2 + d3 * d3;
            entity.addVelocity(d2 / d4 * 4.0, 0.20000000298023224, d3 / d4 * 4.0);
            if (!this.phaseManager.getCurrentPhase().getIsStationary() && ((Iw)entity).getRevengeTimer() < entity.ticksExisted - 2) {
   ...
   private void attackEntitiesInList(List<Ig> p_70971_1_) {
      for(int i = 0; i < p_70971_1_.size(); ++i) {
         Ig entity = (Ig)p_70971_1_.get(i);
         if (entity instanceof Iw) {
      ...
      for(int k1 = i; k1 <= l; ++k1) {
         for(int l1 = j; l1 <= i1; ++l1) {
            for(int i2 = k; i2 <= j1; ++i2) {
               ...
               if (iblockstate.getMaterial() != hM.AIR && iblockstate.getMaterial() != hM.FIRE) {
                  if (!this.world.getGameRules().getBoolean("mobGriefing")) {
                     flag = true;
                  } else if (block != Nk.BARRIER && block != Nk.OBSIDIAN && block != Nk.END_STONE && block != Nk.BEDROCK && block != Nk.END_PORTAL && block != Nk.END_PORTAL_FRAME) {
                     if (block != Nk.COMMAND_BLOCK && block != Nk.REPEATING_COMMAND_BLOCK && block != Nk.CHAIN_COMMAND_BLOCK && block != Nk.IRON_BARS && block != Nk.END_GATEWAY) {
   ...
   public boolean attackEntityFromPart(Lr part, DamageSource source, float damage) {
         ...
         if (source.getTrueSource() instanceof ME || source.isExplosion()) {
            ...
            if (this.phaseManager.getCurrentPhase().getIsStationary()) {
               ...
               if ((float)this.sittingDamageReceived > 0.25F * this.getMaxHealth()) {
                  ...
                  this.phaseManager.setPhase(HK.TAKEOFF);
               }
            }
         }
   ...
   protected void onDeathUpdate() {
      ...
      this.move(Lq.SELF, 0.0, 0.10000000149011612, 0.0);
   ...
   private void dropExperience(int p_184668_1_) {
      while(p_184668_1_ > 0) {
         int i = Js.getXPSplit(p_184668_1_);
         p_184668_1_ -= i;
         this.world.spawnEntity(new Js(this.world, this.posX, this.posY, this.posZ, i));
      }
   ...
   public int initPathPoints() {
      if (this.pathPoints[0] == null) {
         for(int i = 0; i < 24; ++i) {
            ...
            this.pathPoints[i] = new VR(l, lvt_3_1_, i1);
         }
   ...
   public int getNearestPpIdx(double x, double y, double z) {
      float f = 10000.0F;
      int i = 0;
      VR pathpoint = new VR(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z));
      int j = 0;
   ...
   @Nullable
   public VI findPath(int startIdx, int finishIdx, @Nullable VR andThen) {
      VR pathpoint5;
      for(int i = 0; i < 24; ++i) {
      ...
      VR pathpoint4 = this.pathPoints[startIdx];
      ...
      while(!this.pathFindQueue.isPathEmpty()) {
         VR pathpoint2 = this.pathFindQueue.dequeue();
         ...
         for(i1 = j; i1 < 24; ++i1) {
            if ((this.neighbors[k] & 1 << i1) > 0) {
               VR pathpoint3 = this.pathPoints[i1];
   ...
   private VI makePath(VR start, VR finish) {
      int i = 1;

      for(VR pathpoint = finish; pathpoint.previous != null; pathpoint = pathpoint.previous) {
         ++i;
      }

      VR[] apathpoint = new VR[i];
      VR pathpoint1 = finish;
      --i;
   ...
   public static void registerFixesDragon(DataFixer fixer) {
      Iu.registerFixesMob(fixer, HS.class);
   }
   ...
   public void readEntityFromNBT(QQ compound) {
      ...
      if (compound.hasKey("DragonPhase")) {
         this.phaseManager.setPhase(HK.getById(compound.getInteger("DragonPhase")));
      }
   ...
   protected SoundEvent getAmbientSound() {
      return NO.ENTITY_ENDERDRAGON_AMBIENT;
   }

   protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
      return NO.ENTITY_ENDERDRAGON_HURT;
   }
   ...
   protected ResourceLocation getLootTable() {
      return bhq.ENTITIES_ENDER_DRAGON;
   }

   public float getHeadPartYOffset(int p_184667_1_, double[] p_184667_2_, double[] p_184667_3_) {
      HB iphase = this.phaseManager.getCurrentPhase();
      HK<? extends HB> phaselist = iphase.getType();
      double d0;
      if (phaselist != HK.LANDING && phaselist != HK.TAKEOFF) {
         ...
         BlockPos blockpos = this.world.getTopSolidOrLiquidBlock(bbD.END_PODIUM_LOCATION);
   ...
   public void onCrystalDestroyed(IS crystal, BlockPos pos, DamageSource dmgSrc) {
      ME entityplayer;
      if (dmgSrc.getTrueSource() instanceof ME) {
         entityplayer = (ME)dmgSrc.getTrueSource();
      } else {
         entityplayer = this.world.getNearestAttackablePlayer(pos, 64.0, 64.0);
      }

      if (crystal == this.healingEnderCrystal) {
         this.attackEntityFromPart(this.dragonPartHead, DamageSource.causeExplosionDamage((Iw)entityplayer), 10.0F);
      }
   ...
   public void notifyDataManagerChange(Rd<?> key) {
      if (PHASE.equals(key) && this.world.isRemote) {
         this.phaseManager.setPhase(HK.getById((Integer)this.getDataManager().get(PHASE)));
      }

      super.notifyDataManagerChange(key);
   }

   public HL getPhaseManager() {
      return this.phaseManager;
   }

   @Nullable
   public baN getFightManager() {
   ...
   static {
      PHASE = Rv.createKey(HS.class, Rt.VARINT);
   }
}
