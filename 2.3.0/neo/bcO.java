...
public class bcO extends bcM {
   protected static final in AIR;

   public bcO() {
   }

   protected void addRoom(long p_180705_1_, int p_180705_3_, int p_180705_4_, ban p_180705_5_, double p_180705_6_, double p_180705_8_, double p_180705_10_) {
   ...
   protected void addTunnel(long p_180704_1_, int p_180704_3_, int p_180704_4_, ban p_180704_5_, double p_180704_6_, double p_180704_8_, double p_180704_10_, float p_180704_12_, float p_180704_13_, float p_180704_14_, int p_180704_15_, int p_180704_16_, double p_180704_17_) {
      ...
      for(boolean flag = random.nextInt(6) == 0; p_180704_15_ < p_180704_16_; ++p_180704_15_) {
         double d2 = 1.5 + (double)(MathHelper.sin((float)p_180704_15_ * 3.1415927F / (float)p_180704_16_) * p_180704_12_);
               ...
               for(i3 = j2; !flag2 && i3 < k; ++i3) {
                  for(int k1 = l2; !flag2 && k1 < i1; ++k1) {
                     for(int l1 = l + 1; !flag2 && l1 >= k2 - 1; --l1) {
                        if (l1 >= 0 && l1 < 128) {
                           in iblockstate = p_180704_5_.getBlockState(i3, l1, k1);
                           if (iblockstate.getBlock() == Nk.FLOWING_LAVA || iblockstate.getBlock() == Nk.LAVA) {
                              flag2 = true;
                           }
                     ...
                     for(int j3 = l2; j3 < i1; ++j3) {
                        ...
                        for(int i2 = l; i2 > k2; --i2) {
                           ...
                           if (d9 > -0.7 && d10 * d10 + d9 * d9 + d8 * d8 < 1.0) {
                              in iblockstate1 = p_180704_5_.getBlockState(i3, i2, j3);
                              if (iblockstate1.getBlock() == Nk.NETHERRACK || iblockstate1.getBlock() == Nk.DIRT || iblockstate1.getBlock() == Nk.GRASS) {
   ...
   protected void recursiveGenerate(bij worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ban chunkPrimerIn) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         for(int l = 0; l < k; ++l) {
            ...
            this.addTunnel(this.rand.nextLong(), originalX, originalZ, chunkPrimerIn, d0, d1, d2, f2 * 2.0F, f, f1, 0, 0, 0.5);
         }
      }
