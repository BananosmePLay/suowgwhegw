...
public class jI {
   ...
   public static void drawGradientRect1(float f8, float sY, double d, double e, int startColor, int endColor) {
      ...
      bufferbuilder.pos(d, (double)sY, 0.0).color(f1, f2, f3, f).endVertex();
      bufferbuilder.pos((double)f8, (double)sY, 0.0).color(f5, f6, f7, f4).endVertex();
      bufferbuilder.pos((double)f8, e, 0.0).color(f5, f6, f7, f4).endVertex();
   ...
   public void drawCenteredString(jH fontRendererIn, String text, int x, int y, int color) {
      fontRendererIn.drawStringWithShadow(text, (float)(x - fontRendererIn.getStringWidth(text) / 2), (float)y, color);
   }

   public void drawString(jH fontRendererIn, String text, int x, int y, int color) {
   ...
   public void drawTexturedModalRect(int xCoord, int yCoord, zd textureSprite, int widthIn, int heightIn) {
      yN tessellator = yN.getInstance();
      tN bufferbuilder = tessellator.getBuffer();
      bufferbuilder.begin(7, zK.POSITION_TEX);
   ...
   public static void drawScaledCustomSizeModalRect(int x, int y, float u, float v, int uWidth, int vHeight, int width, int height, float tileWidth, float tileHeight) {
      ...
      tN bufferbuilder = tessellator.getBuffer();
      bufferbuilder.begin(7, zK.POSITION_TEX);
      bufferbuilder.pos((double)x, (double)(y + height), 0.0).tex((double)(u * f), (double)((v + (float)vHeight) * f1)).endVertex();
      bufferbuilder.pos((double)(x + width), (double)(y + height), 0.0).tex((double)((u + (float)uWidth) * f), (double)((v + (float)vHeight) * f1)).endVertex();
      bufferbuilder.pos((double)(x + width), (double)y, 0.0).tex((double)((u + (float)uWidth) * f), (double)(v * f1)).endVertex();
      bufferbuilder.pos((double)x, (double)y, 0.0).tex((double)(u * f), (double)(v * f1)).endVertex();
      tessellator.draw();
   }
}
