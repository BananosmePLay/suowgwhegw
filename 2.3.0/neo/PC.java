package neo;

import java.util.List;
import javax.annotation.Nullable;
import net.minecraft.util.text.translation.I18n;

public class PC extends OL {
   public PC() {
   }

   public static QH getExplosionTag(Qy stack, String key) {
      if (stack.hasTagCompound()) {
         QQ nbttagcompound = stack.getTagCompound().getCompoundTag("Explosion");
         if (nbttagcompound != null) {
            return nbttagcompound.getTag(key);
         }
      }

      return null;
   }

   public void addInformation(Qy stack, @Nullable bij worldIn, List<String> tooltip, BJ flagIn) {
      if (stack.hasTagCompound()) {
         QQ nbttagcompound = stack.getTagCompound().getCompoundTag("Explosion");
         if (nbttagcompound != null) {
            addExplosionInfo(nbttagcompound, tooltip);
         }
      }

   }

   public static void addExplosionInfo(QQ nbt, List<String> tooltip) {
      byte b0 = nbt.getByte("Type");
      if (b0 >= 0 && b0 <= 4) {
         tooltip.add(I18n.translateToLocal("item.fireworksCharge.type." + b0).trim());
      } else {
         tooltip.add(I18n.translateToLocal("item.fireworksCharge.type").trim());
      }

      int[] aint = nbt.getIntArray("Colors");
      int var8;
      int i;
      if (aint.length > 0) {
         boolean flag = true;
         String s = "";
         int[] var6 = aint;
         int var7 = aint.length;

         for(var8 = 0; var8 < var7; ++var8) {
            i = var6[var8];
            if (!flag) {
               s = s + ", ";
            }

            flag = false;
            boolean flag1 = false;

            for(int j = 0; j < Pq.DYE_COLORS.length; ++j) {
               if (i == Pq.DYE_COLORS[j]) {
                  flag1 = true;
                  s = s + I18n.translateToLocal("item.fireworksCharge." + Om.byDyeDamage(j).getTranslationKey());
                  break;
               }
            }

            if (!flag1) {
               s = s + I18n.translateToLocal("item.fireworksCharge.customColor");
            }
         }

         tooltip.add(s);
      }

      int[] aint1 = nbt.getIntArray("FadeColors");
      boolean flag2;
      if (aint1.length > 0) {
         flag2 = true;
         String s1 = I18n.translateToLocal("item.fireworksCharge.fadeTo") + " ";
         int[] var16 = aint1;
         var8 = aint1.length;

         for(i = 0; i < var8; ++i) {
            int l = var16[i];
            if (!flag2) {
               s1 = s1 + ", ";
            }

            flag2 = false;
            boolean flag5 = false;

            for(int k = 0; k < 16; ++k) {
               if (l == Pq.DYE_COLORS[k]) {
                  flag5 = true;
                  s1 = s1 + I18n.translateToLocal("item.fireworksCharge." + Om.byDyeDamage(k).getTranslationKey());
                  break;
               }
            }

            if (!flag5) {
               s1 = s1 + I18n.translateToLocal("item.fireworksCharge.customColor");
            }
         }

         tooltip.add(s1);
      }

      flag2 = nbt.getBoolean("Trail");
      if (flag2) {
         tooltip.add(I18n.translateToLocal("item.fireworksCharge.trail"));
      }

      boolean flag4 = nbt.getBoolean("Flicker");
      if (flag4) {
         tooltip.add(I18n.translateToLocal("item.fireworksCharge.flicker"));
      }

   }
}
