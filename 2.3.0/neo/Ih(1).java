...
public abstract class Ih extends Ik {
   private static final Rd<Boolean> BABY;
   ...
   public Ih(bij worldIn) {
      super(worldIn);
   }

   @Nullable
   public abstract Ih createChild(Ih var1);
   ...
   public boolean processInteract(ME player, EnumHand hand) {
      ...
      if (itemstack.getItem() == NK.SPAWN_EGG) {
         if (!this.world.isRemote) {
            Class<? extends Ig> oclass = (Class)Ir.REGISTRY.getObject(PX.getNamedIdFrom(itemstack));
            if (oclass != null && this.getClass() == oclass) {
               Ih entityageable = this.createChild(this);
   ...
   protected boolean holdingSpawnEggOfClass(Qy stack, Class<? extends Ig> entityClass) {
      if (stack.getItem() != NK.SPAWN_EGG) {
         return false;
      } else {
         Class<? extends Ig> oclass = (Class)Ir.REGISTRY.getObject(PX.getNamedIdFrom(stack));
   ...
   public int getGrowingAge() {
      if (this.world.isRemote) {
         try {
            Object value = this.dataManager.get(BABY);
   ...
   static {
      BABY = Rv.createKey(Ih.class, Rt.BOOLEAN);
   }
}
