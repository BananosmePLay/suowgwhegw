...
public class bni {
   ...
   public bni(String type) {
      if (type.equals("4")) {
         ...
         XH.warn("NaturalTextures: Unknown type: " + type);
      }
   ...
   public synchronized rK getQuad(rK quadIn, int rotate, boolean flipU) {
      int i = rotate;
      if (flipU) {
         i = rotate | 4;
      }
   ...
   private rK makeQuad(rK quad, int rotate, boolean flipU) {
      ...
      rK bakedquad = new rK(aint, i, enumfacing, textureatlassprite);
      return bakedquad;
   }

   private int[] transformVertexData(int[] vertexData, int rotate, boolean flipU) {
      int[] aint = (int[])((int[])(([I)vertexData).clone());
      int i = 4 - rotate;
      if (flipU) {
         i += 3;
      }
   ...
   private boolean isFullSprite(rK quad) {
      zd textureatlassprite = quad.getSprite();
