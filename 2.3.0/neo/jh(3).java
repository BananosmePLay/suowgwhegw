...
public class jh extends jf {
   public final py connection;
   private final XT statWriter;
   private final XK recipeBook;
   ...
   public jh(nC p_i47378_1_, bij p_i47378_2_, py p_i47378_3_, XT p_i47378_4_, XK p_i47378_5_) {
   ...
   public boolean startRiding(Ig entityIn, boolean force) {
      if (!super.startRiding(entityIn, force)) {
         ...
         if (entityIn instanceof Jc) {
            this.mc.getSoundHandler().playSound(new iI(this, (Jc)entityIn));
         }
   ...
   public void onUpdate() {
      if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
         0bz.method_Qm().method_Qn().post(new 0dm());
         super.onUpdate();
         if (this.isRiding()) {
            this.connection.sendPacket(new SX(this.rotationYaw, this.rotationPitch, this.onGround));
            this.connection.sendPacket(new SS(this.moveStrafing, this.moveForward, this.movementInput.jump, this.movementInput.sneak));
            Ig entity = this.getLowestRidingEntity();
            if (entity != this && entity.canPassengerSteer()) {
               this.connection.sendPacket(new Tq(entity));
            }
         } else {
            this.onUpdateWalkingPlayer();
            if (!0bz.method_Qm().method_Qs().method_bxY(0bQ.class).method_bBh()) {
   ...
   private void onUpdateWalkingPlayer() {
      if (!0bz.method_Qm().method_Qs().method_bxY(0bQ.class).method_bBh()) {
         ...
         if (flag != this.serverSprintState) {
            if (flag) {
               this.connection.sendPacket(new SQ(this, SP.START_SPRINTING));
            } else {
               this.connection.sendPacket(new SQ(this, SP.STOP_SPRINTING));
            }
         ...
         if (flag1 != this.serverSneakState) {
            if (flag1) {
               this.connection.sendPacket(new SQ(this, SP.START_SNEAKING));
            } else {
               this.connection.sendPacket(new SQ(this, SP.STOP_SNEAKING));
            }
         ...
         if (this.isCurrentViewEntity()) {
            ...
            if (this.isRiding()) {
               this.connection.sendPacket(new SW(this.motionX, -999.0, this.motionZ, this.rotationYaw, this.rotationPitch, this.onGround));
               flag2 = false;
            } else if (flag2 && flag3) {
               this.connection.sendPacket(new SW(this.posX, axisalignedbb.minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround));
            } else if (flag2) {
               this.connection.sendPacket(new SV(this.posX, axisalignedbb.minY, this.posZ, this.onGround));
            } else if (flag3) {
               this.connection.sendPacket(new SX(this.rotationYaw, this.rotationPitch, this.onGround));
            } else if (this.prevOnGround != this.onGround) {
               this.connection.sendPacket(new SY(this.onGround));
            }
            ...
            nC var10001 = this.mc;
            this.autoJumpEnabled = nC.gameSettings.autoJump;
         }
   ...
   public IY dropItem(boolean dropAll) {
      Ta cpacketplayerdigging$action = dropAll ? Ta.DROP_ALL_ITEMS : Ta.DROP_ITEM;
      this.connection.sendPacket(new Tb(cpacketplayerdigging$action, BlockPos.ORIGIN, EnumFacing.DOWN));
      return null;
   }
   ...
   public void sendChatMessage(String message) {
      0de eventMessage = new 0de(message);
      0bz.method_Qm().method_Qn().post(eventMessage);
      if (!eventMessage.method_bzU() && !message.startsWith(0bz.method_Qm().method_Qr().method_bxu())) {
         this.connection.sendPacket(new SE(message));
      }
   ...
   public void swingArm(EnumHand hand) {
      super.swingArm(hand);
      this.connection.sendPacket(new SD(hand));
   }

   public void respawnPlayer() {
      this.connection.sendPacket(new SI(SH.PERFORM_RESPAWN));
   }
   ...
   public void closeScreen() {
      this.connection.sendPacket(new SJ(this.openContainer.windowId));
      this.closeScreenAndDropStack();
   }

   public void closeScreenAndDropStack() {
      this.inventory.setItemStack(Qy.EMPTY);
      super.closeScreen();
   ...
   public void sendPlayerAbilities() {
      this.connection.sendPacket(new SZ(this.capabilities));
   }
   ...
   protected void sendHorseJump() {
      this.connection.sendPacket(new SQ(this, SP.START_RIDING_JUMP, MathHelper.floor(this.getHorseJumpPower() * 100.0F)));
   }

   public void sendHorseInventory() {
      this.connection.sendPacket(new SQ(this, SP.OPEN_INVENTORY));
   }
   ...
   public XT getStatFileWriter() {
      return this.statWriter;
   }

   public XK getRecipeBook() {
      return this.recipeBook;
   }

   public void removeRecipeHighlight(NT p_193103_1_) {
      if (this.recipeBook.isNew(p_193103_1_)) {
         this.recipeBook.markSeen(p_193103_1_);
         this.connection.sendPacket(new Tf(p_193103_1_));
      }
   ...
   public boolean isBlocking() {
      return this.isHandActive() && this.activeItemStack.getItem().getItemUseAction(this.activeItemStack) == Ol.BLOCK;
   }

   protected boolean pushOutOfBlocks(double x, double y, double z) {
      if ((!0bz.method_Qm().method_Qs().method_bxY(0bT.class).method_bBh() || !0bT.field_c.method_bna()) && !0bz.method_Qm().method_Qs().method_bxY(0bQ.class).method_bBh()) {
         if (this.noClip) {
            return false;
         } else {
   ...
   public void notifyDataManagerChange(Rd<?> key) {
      ...
      if (FLAGS.equals(key) && this.isElytraFlying() && !this.wasFallFlying) {
         this.mc.getSoundHandler().playSound(new iz(this));
      }
   ...
   public boolean isRidingHorse() {
      ...
      return this.isRiding() && entity instanceof IG && ((IG)entity).canJump();
   }
   ...
   public void openEditSign(YQ signTile) {
      this.mc.displayGuiScreen(new md(signTile));
   }

   public void displayGuiEditCommandCart(XZ commandBlock) {
      this.mc.displayGuiScreen(new mc(commandBlock));
   }

   public void displayGuiCommandBlock(Yq commandBlock) {
      this.mc.displayGuiScreen(new jS(commandBlock));
   }

   public void openEditStructure(YV structure) {
      this.mc.displayGuiScreen(new mf(structure));
   }

   public void openBook(Qy stack, EnumHand hand) {
      OL item = stack.getItem();
      if (item == NK.WRITABLE_BOOK) {
         this.mc.displayGuiScreen(new lk(this, stack, true));
      }

   }

   public void displayGUIChest(IInventory chestInventory) {
      String s = chestInventory instanceof bga ? ((bga)chestInventory).getGuiID() : "minecraft:container";
      if ("minecraft:chest".equals(s)) {
         this.mc.displayGuiScreen(new lT(this.inventory, chestInventory));
      } else if ("minecraft:hopper".equals(s)) {
         this.mc.displayGuiScreen(new kl(this.inventory, chestInventory));
      } else if ("minecraft:furnace".equals(s)) {
         this.mc.displayGuiScreen(new mg(this.inventory, chestInventory));
      } else if ("minecraft:brewing_stand".equals(s)) {
         this.mc.displayGuiScreen(new lS(this.inventory, chestInventory));
      } else if ("minecraft:beacon".equals(s)) {
         this.mc.displayGuiScreen(new lR(this.inventory, chestInventory));
      } else if (!"minecraft:dispenser".equals(s) && !"minecraft:dropper".equals(s)) {
         if ("minecraft:shulker_box".equals(s)) {
            this.mc.displayGuiScreen(new mj(this.inventory, chestInventory));
         } else {
            this.mc.displayGuiScreen(new lT(this.inventory, chestInventory));
         }
      } else {
         this.mc.displayGuiScreen(new ma(this.inventory, chestInventory));
      }

   }

   public void openGuiHorseInventory(Lw horse, IInventory inventoryIn) {
      this.mc.displayGuiScreen(new mi(this.inventory, inventoryIn, horse));
   }

   public void displayGui(bga guiOwner) {
      String s = guiOwner.getGuiID();
      if ("minecraft:crafting_table".equals(s)) {
         this.mc.displayGuiScreen(new lZ(this.inventory, this.world));
      } else if ("minecraft:enchanting_table".equals(s)) {
         this.mc.displayGuiScreen(new kf(this.inventory, this.world, guiOwner));
      } else if ("minecraft:anvil".equals(s)) {
         this.mc.displayGuiScreen(new lc(this.inventory, this.world));
      }

   }

   public void displayVillagerTradeGui(IH villager) {
      this.mc.displayGuiScreen(new kH(this.inventory, villager, this.world));
   }
   ...
   public void onLivingUpdate() {
      ...
      if (this.inPortal) {
         if (this.mc.currentScreen != null && !this.mc.currentScreen.doesGuiPauseGame()) {
            if (this.mc.currentScreen instanceof lU) {
               this.closeScreen();
            }
         ...
         if (this.timeInPortal == 0.0F) {
            this.mc.getSoundHandler().playSound(iN.getMasterRecord(NO.BLOCK_PORTAL_TRIGGER, this.rand.nextFloat() * 0.4F + 0.8F));
         }
         ...
         this.inPortal = false;
      } else if (this.isPotionActive(NL.NAUSEA) && this.getActivePotionEffect(NL.NAUSEA).getDuration() > 60) {
      ...
      if (this.onGround && !flag1 && !flag2 && this.movementInput.moveForward >= 0.8F && !this.isSprinting() && flag4 && !this.isHandActive() && !this.isPotionActive(NL.BLINDNESS)) {
         if (this.sprintToggleTimer <= 0 && !nC.gameSettings.keyBindSprint.isKeyDown()) {
      ...
      if (!this.isSprinting() && this.movementInput.moveForward >= 0.8F && flag4 && !this.isHandActive() && !this.isPotionActive(NL.BLINDNESS) && nC.gameSettings.keyBindSprint.isKeyDown()) {
         this.setSprinting(true);
      }
      ...
      if (this.movementInput.jump && !flag && !this.onGround && this.motionY < 0.0 && !this.isElytraFlying() && !this.capabilities.isFlying) {
         ...
         if (itemstack.getItem() == NK.ELYTRA && Pt.isUsable(itemstack)) {
            this.connection.sendPacket(new SQ(this, SP.START_FALL_FLYING));
         }
      }
      ...
      if (this.isRidingHorse()) {
         IG ijumpingmount = (IG)this.getRidingEntity();
   ...
   public void updateRidden() {
      ...
      if (this.getRidingEntity() instanceof IR) {
         IR entityboat = (IR)this.getRidingEntity();
         ...
         if (vec2f.x != 0.0F || vec2f.y != 0.0F) {
            ...
            if (f13 >= -0.15F) {
               ...
               if (iblockstate.getCollisionBoundingBox(this.world, blockpos) == null) {
                  ...
                  in iblockstate1 = this.world.getBlockState(blockpos);
                  if (iblockstate1.getCollisionBoundingBox(this.world, blockpos) == null) {
                     float f6 = 7.0F;
                     float f7 = 1.2F;
                     if (this.isPotionActive(NL.JUMP_BOOST)) {
                        f7 += (float)(this.getActivePotionEffect(NL.JUMP_BOOST).getAmplifier() + 1) * 0.75F;
                     }
                     ...
                     label85: {
                        ...
                        for(int i = 1; !((float)i >= f7); ++i) {
                           ...
                           in iblockstate2 = this.world.getBlockState(blockpos2);
                           ...
                           if (i > 1) {
                              blockpos = blockpos.up();
                              in iblockstate3 = this.world.getBlockState(blockpos);
