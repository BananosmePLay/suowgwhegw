...
public class Ii extends Ig {
   private static final Rd<Float> RADIUS;
   private static final Rd<Integer> COLOR;
   private static final Rd<Boolean> IGNORE_RADIUS;
   private static final Rd<Integer> PARTICLE;
   private static final Rd<Integer> PARTICLE_PARAM_1;
   private static final Rd<Integer> PARTICLE_PARAM_2;
   private Wf potion;
   private final List<VZ> effects;
   private final Map<Ig, Integer> reapplicationDelayMap;
   ...
   public Ii(bij worldIn, double x, double y, double z) {
      this(worldIn);
   ...
   public void setPotion(Wf potionIn) {
   ...
   private void updateFixedColor() {
      if (this.potion == NN.EMPTY && this.effects.isEmpty()) {
         this.getDataManager().set(COLOR, 0);
      } else {
         this.getDataManager().set(COLOR, Wg.getPotionColorFromEffectList(Wg.mergeEffects(this.potion, this.effects)));
      }
   ...
   public void onUpdate() {
      ...
      if (this.world.isRemote) {
         ...
         if (flag) {
            if (this.rand.nextBoolean()) {
               for(int i = 0; i < 2; ++i) {
                  ...
                  if (enumparticletypes == EnumParticleTypes.SPELL_MOB) {
                     ...
                     this.world.spawnAlwaysVisibleParticle(enumparticletypes.getParticleID(), this.posX + (double)f3, this.posY, this.posZ + (double)f4, 0.0, 0.0, 0.0, aint);
                  }
               }
            }
         } else {
            ...
            for(int k1 = 0; (float)k1 < f5; ++k1) {
               ...
               if (enumparticletypes == EnumParticleTypes.SPELL_MOB) {
                  ...
                  i1 = k >> 8 & 255;
                  int j1 = k & 255;
                  this.world.spawnAlwaysVisibleParticle(EnumParticleTypes.SPELL_MOB.getParticleID(), this.posX + (double)f4, this.posY, this.posZ + (double)f9, (double)((float)l / 255.0F), (double)((float)i1 / 255.0F), (double)((float)j1 / 255.0F));
               } else {
                  this.world.spawnAlwaysVisibleParticle(enumparticletypes.getParticleID(), this.posX + (double)f4, this.posY, this.posZ + (double)f9, (0.5 - this.rand.nextDouble()) * 0.15, 0.009999999776482582, (0.5 - this.rand.nextDouble()) * 0.15, aint);
               }
            }
         }
      } else {
         ...
         if (this.ticksExisted % 5 == 0) {
            Iterator<Map.Entry<Ig, Integer>> iterator = this.reapplicationDelayMap.entrySet().iterator();

            while(iterator.hasNext()) {
               Map.Entry<Ig, Integer> entry = (Map.Entry)iterator.next();
               if (this.ticksExisted >= (Integer)entry.getValue()) {
                  iterator.remove();
               }
            }

            List<VZ> lstPotions = Lists.newArrayList();
            Iterator var23 = this.potion.getEffects().iterator();

            while(var23.hasNext()) {
               VZ potioneffect1 = (VZ)var23.next();
               lstPotions.add(new VZ(potioneffect1.getPotion(), potioneffect1.getDuration() / 4, potioneffect1.getAmplifier(), potioneffect1.getIsAmbient(), potioneffect1.doesShowParticles()));
            }
            ...
            if (lstPotions.isEmpty()) {
               ...
               List<Iw> list = this.world.getEntitiesWithinAABB(Iw.class, this.getEntityBoundingBox());
               if (!list.isEmpty()) {
                     ...
                     do {
                        do {
                           do {
                              ...
                              entitylivingbase = (Iw)var26.next();
   ...
   public void setOwner(@Nullable Iw ownerIn) {
      this.owner = ownerIn;
   ...
   @Nullable
   public Iw getOwner() {
      if (this.owner == null && this.ownerUniqueId != null && this.world instanceof bis) {
         Ig entity = ((bis)this.world).getEntityFromUuid(this.ownerUniqueId);
         if (entity instanceof Iw) {
            this.owner = (Iw)entity;
         }
      }
   ...
   protected void readEntityFromNBT(QQ compound) {
      ...
      if (compound.hasKey("Potion", 8)) {
         this.setPotion(Wg.getPotionTypeFromNBT(compound));
      }

      if (compound.hasKey("Effects", 9)) {
         QW nbttaglist = compound.getTagList("Effects", 10);
         this.effects.clear();

         for(int i = 0; i < nbttaglist.tagCount(); ++i) {
            VZ potioneffect = VZ.readCustomPotionEffectFromNBT(nbttaglist.getCompoundTagAt(i));
   ...
   static {
      RADIUS = Rv.createKey(Ii.class, Rt.FLOAT);
      COLOR = Rv.createKey(Ii.class, Rt.VARINT);
      IGNORE_RADIUS = Rv.createKey(Ii.class, Rt.BOOLEAN);
      PARTICLE = Rv.createKey(Ii.class, Rt.VARINT);
      PARTICLE_PARAM_1 = Rv.createKey(Ii.class, Rt.VARINT);
      PARTICLE_PARAM_2 = Rv.createKey(Ii.class, Rt.VARINT);
   }
}
