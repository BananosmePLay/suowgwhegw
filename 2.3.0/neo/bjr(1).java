...
public class bjr implements bjx {
   public String name = null;
   public String basePath = null;
   private int format = -1;
   private biZ[] matchBlocks = null;
   ...
   public bjr(Properties props, String path, int width, int height, String formatDefault) {
      biS connectedparser = new biS("Colormap");
      ...
      this.color = biS.parseColor(props.getProperty("color"), -1);
   ...
   public boolean isValid(String path) {
      if (this.format != 0 && this.format != 1) {
         ...
         if (this.color < 0) {
            ...
            if (this.format == 1) {
               this.color = this.getColorGrid(Nj.PLAINS, new BlockPos(0, 64, 0));
            }
         }
      }
   ...
   private biZ[] detectMatchBlocks() {
      co block = co.getBlockFromName(this.name);
      if (block != null) {
         return new biZ[]{new biZ(co.getIdFromBlock(block))};
      } else {
         ...
         if (matcher.matches()) {
            ...
            if (i >= 0) {
               return new biZ[]{new biZ(i)};
            }
         }

         biS connectedparser = new biS("Colormap");
         biZ[] amatchblock = connectedparser.parseMatchBlock(this.name);
   ...
   public boolean matchesBlock(ie blockState) {
      return bja.block(blockState, this.matchBlocks);
   }
   ...
   public int getColorRandom() {
      if (this.format == 2) {
         ...
         int i = bjy.random.nextInt(this.colors.length);
         return this.colors[i];
      }
   }

   public int getColor(int index) {
      index = XH.limit(index, 0, this.colors.length - 1);
      return this.colors[index] & 16777215;
   }

   public int getColor(int cx, int cy) {
      cx = XH.limit(cx, 0, this.width - 1);
      cy = XH.limit(cy, 0, this.height - 1);
   ...
   public int getColor(bfZ blockAccess, BlockPos blockPos) {
   ...
   public int getColor(Zi biome, BlockPos blockPos) {
      if (this.format == 0) {
   ...
   public int getColorSmooth(bfZ blockAccess, double x, double y, double z, int radius) {
      if (this.format == 2) {
         return this.color;
      } else {
         ...
         for(l1 = i - radius; l1 <= i + radius; ++l1) {
            for(i2 = k - radius; i2 <= k + radius; ++i2) {
               ...
               j2 = this.getColor((bfZ)blockAccess, blockposm);
               l += j2 >> 16 & 255;
               i1 += j2 >> 8 & 255;
               j1 += j2 & 255;
               ++k1;
   ...
   private int getColorVanilla(Zi biome, BlockPos blockPos) {
   ...
   private int getColorGrid(Zi biome, BlockPos blockPos) {
      int i = Zi.getIdForBiome(biome);
   ...
   public void addMatchBlock(biZ mb) {
      if (this.matchBlocks == null) {
         this.matchBlocks = new biZ[0];
      }

      this.matchBlocks = (biZ[])((biZ[])XH.addObjectToArray(this.matchBlocks, mb));
   }

   public void addMatchBlock(int blockId, int metadata) {
      biZ matchblock = this.getMatchBlock(blockId);
      if (matchblock != null) {
         if (metadata >= 0) {
            matchblock.addMetadata(metadata);
         }
      } else {
         this.addMatchBlock(new biZ(blockId, metadata));
      }

   }

   private biZ getMatchBlock(int blockId) {
      if (this.matchBlocks == null) {
         return null;
      } else {
   ...
   public String toString() {
      return "" + this.basePath + "/" + this.name + ", blocks: " + XH.arrayToString((Object[])this.matchBlocks) + ", source: " + this.source;
   }
}
