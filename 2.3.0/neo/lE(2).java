...
public class lE extends jI {
   ...
   private kW guiResponder;
   private Predicate<String> validator = Predicates.alwaysTrue();

   public lE(int componentId, jH fontrendererObj, int x, int y, int par5Width, int par6Height) {
      this.id = componentId;
   ...
   public void setGuiResponder(kW guiResponderIn) {
   ...
   public boolean textboxKeyTyped(char typedChar, int keyCode) {
      if (!this.isFocused) {
         return false;
      } else if (lg.isKeyComboCtrlA(keyCode)) {
         this.setCursorPositionEnd();
         this.setSelectionPos(0);
         return true;
      } else if (lg.isKeyComboCtrlC(keyCode)) {
         lg.setClipboardString(this.getSelectedText());
         return true;
      } else if (lg.isKeyComboCtrlV(keyCode)) {
         if (this.isEnabled) {
            this.writeText(lg.getClipboardString());
         }

         return true;
      } else if (lg.isKeyComboCtrlX(keyCode)) {
         lg.setClipboardString(this.getSelectedText());
         if (this.isEnabled) {
            this.writeText("");
         }
   ...
   public void drawTextBox() {
      if (this.getVisible()) {
         ...
         if (flag1) {
            if (flag2) {
               jI.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + this.fontRenderer.FONT_HEIGHT, -3092272);
            } else {
   ...
   private void drawSelectionBox(int startX, int startY, int endX, int endY) {
      ...
      yh.color(0.0F, 0.0F, 255.0F, 255.0F);
      yh.disableTexture2D();
      yh.enableColorLogic();
      yh.colorLogicOp(xU.OR_REVERSE);
   ...
   public void setFocused(boolean isFocusedIn) {
      ...
      if (nC.getMinecraft().currentScreen != null) {
         nC.getMinecraft().currentScreen.setFocused(isFocusedIn);
      }
