...
class jl extends jj {
   ...
   public jl(final jq this$0, nC p_i47554_2_) {
      super(this$0, p_i47554_2_);
      this.this$0 = this$0;
      this.statsHolder = Lists.newArrayList();
      Iterator var3 = XV.MINE_BLOCK_STATS.iterator();
      ...
      while(var3.hasNext()) {
         ...
         if (jq.access$100(this$0).readStat(statcrafting) > 0) {
            flag = true;
         } else if (XV.getObjectUseStats(item) != null && jq.access$100(this$0).readStat(XV.getObjectUseStats(item)) > 0) {
            flag = true;
      ...
      this.statSorter = new Comparator<XS>() {
         public int compare(XS p_compare_1_, XS p_compare_2_) {
            ...
            XQ statbase = null;
            XQ statbase1 = null;
            if (jl.this.sortColumn == 2) {
               statbase = XV.getBlockStats(co.getBlockFromItem(item1));
               statbase1 = XV.getBlockStats(co.getBlockFromItem(item2));
            } else if (jl.this.sortColumn == 0) {
               statbase = XV.getCraftStats(item1);
               statbase1 = XV.getCraftStats(item2);
            } else if (jl.this.sortColumn == 1) {
               statbase = XV.getObjectUseStats(item1);
               statbase1 = XV.getObjectUseStats(item2);
            } else if (jl.this.sortColumn == 3) {
               statbase = XV.getObjectsPickedUpStats(item1);
               statbase1 = XV.getObjectsPickedUpStats(item2);
            } else if (jl.this.sortColumn == 4) {
            ...
            if (statbase != null || statbase1 != null) {
               ...
               int i = jq.access$100(jl.this.this$0).readStat(statbase);
               int j = jq.access$100(jl.this.this$0).readStat(statbase1);
               if (i != j) {
                  return (i - j) * jl.this.sortOrder;
               }
            }

            return OL.getIdFromItem(item1) - OL.getIdFromItem(item2);
         }
   ...
   protected void drawListHeader(int insideLeft, int insideTop, yN tessellatorIn) {
      ...
      if (this.headerPressed == 0) {
         jq.access$000(this.this$0, insideLeft + 115 - 18 + 1, insideTop + 1 + 1, 18, 18);
      } else {
         jq.access$000(this.this$0, insideLeft + 115 - 18, insideTop + 1, 18, 18);
      }

      if (this.headerPressed == 1) {
         jq.access$000(this.this$0, insideLeft + 165 - 18 + 1, insideTop + 1 + 1, 36, 18);
      } else {
         jq.access$000(this.this$0, insideLeft + 165 - 18, insideTop + 1, 36, 18);
      }

      if (this.headerPressed == 2) {
         jq.access$000(this.this$0, insideLeft + 215 - 18 + 1, insideTop + 1 + 1, 54, 18);
      } else {
         jq.access$000(this.this$0, insideLeft + 215 - 18, insideTop + 1, 54, 18);
      }
   ...
   protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
      ...
      this.renderStat(XV.getCraftStats(item), xPos + 115, yPos, slotIndex % 2 == 0);
      this.renderStat(XV.getObjectUseStats(item), xPos + 165, yPos, slotIndex % 2 == 0);
