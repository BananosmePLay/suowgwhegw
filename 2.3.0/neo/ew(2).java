...
public abstract class ew extends co {
   public static final hV DECAYABLE = hV.create("decayable");
   public static final hV CHECK_DECAY = hV.create("check_decay");
         ...
         for(int j1 = -1; j1 <= 1; ++j1) {
            for(int k1 = -1; k1 <= 1; ++k1) {
               for(int l1 = -1; l1 <= 1; ++l1) {
                  ...
                  if (iblockstate.getMaterial() == hM.LEAVES && !(Boolean)iblockstate.getValue(CHECK_DECAY)) {
      ...
      if (!worldIn.isRemote && (Boolean)state.getValue(CHECK_DECAY) && (Boolean)state.getValue(DECAYABLE)) {
         ...
         if (worldIn.isAreaLoaded(new BlockPos(k - 5, l - 5, i1 - 5), new BlockPos(k + 5, l + 5, i1 + 5))) {
            ...
            while(true) {
               ...
               for(j3 = -4; j3 <= 4; ++j3) {
                  for(k3 = -4; k3 <= 4; ++k3) {
                     in iblockstate = worldIn.getBlockState(blockpos$mutableblockpos.setPos(k + i3, l + j3, i1 + k3));
                     co block = iblockstate.getBlock();
                     if (block != Nk.LOG && block != Nk.LOG2) {
                        if (iblockstate.getMaterial() == hM.LEAVES) {
   ...
   private void destroy(bij worldIn, BlockPos pos) {
   ...
   public OL getItemDropped(in state, Random rand, int fortune) {
      return OL.getItemFromBlock(Nk.SAPLING);
   }
   ...
   public abstract fk getWoodType(int var1);
