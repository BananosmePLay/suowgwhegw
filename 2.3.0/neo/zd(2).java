...
public class zd {
   ...
   public zd spriteSingle = null;
   public boolean isSpriteSingle = false;
   public int mipmapLevels = 0;
   public zd spriteNormal = null;
   public zd spriteSpecular = null;
   public boolean isShadersSprite = false;
   public boolean isDependencyParent = false;
   public boolean isEmissive = false;
   public zd spriteEmissive = null;
   private int animationIndex = -1;
   private boolean animationActive = false;

   private zd(String p_i10_1_, boolean p_i10_2_) {
      this.iconName = p_i10_1_;
      this.isSpriteSingle = p_i10_2_;
   }

   public zd(String spriteName) {
      this.iconName = spriteName;
      if (XH.isMultiTexture()) {
         this.spriteSingle = new zd(this.getIconName() + ".spriteSingle", true);
      }

   }

   protected static zd makeAtlasSprite(ResourceLocation spriteResourceLocation) {
      return new zd(spriteResourceLocation.toString());
   }
   ...
   public void copyFrom(zd atlasSpirit) {
      ...
      if (atlasSpirit != XH.getTextureMap().getMissingSprite()) {
   ...
   public void updateAnimation() {
      if (this.animationMetadata != null) {
         this.animationActive = bpW.isActive() ? bpW.isSpriteRendered(this.animationIndex) : true;
         ++this.tickCounter;
         if (this.tickCounter >= this.animationMetadata.getFrameTimeSingle(this.frameCounter)) {
            ...
            if (i != k && k >= 0 && k < this.framesTextureData.size()) {
               zk.uploadTextureMipmap((int[][])this.framesTextureData.get(k), this.width, this.height, this.originX, this.originY, flag, flag1);
            }
   ...
   private void updateAnimationInterpolated() {
      ...
      if (i != k && k >= 0 && k < this.framesTextureData.size()) {
         ...
         zk.uploadTextureMipmap(this.interpolatedFrameData, this.width, this.height, this.originX, this.originY, false, false);
      }
   ...
   public void loadSprite(yX sizeInfo, boolean p_188538_2_) throws IOException {
      this.resetSprite();
   ...
   public void loadSpriteFrames(Az resource, int mipmaplevels) throws IOException {
      BufferedImage bufferedimage = zk.readBufferedImage(resource.getInputStream());
      if (this.width != bufferedimage.getWidth()) {
         bufferedimage = bqS.scaleImage(bufferedimage, this.width);
      }

      zY animationmetadatasection = (zY)resource.getMetadata("animation");
      ...
      if (animationmetadatasection == null) {
         ...
         if (animationmetadatasection.getFrameCount() > 0) {
            ...
            List<zX> list = Lists.newArrayList();

            for(i1 = 0; i1 < k; ++i1) {
               this.framesTextureData.add(getFrameTextureData(aint, this.width, this.width, i1));
               list.add(new zX(i1, -1));
            }

            this.animationMetadata = new zY(list, this.width, this.height, animationmetadatasection.getFrameTime(), animationmetadatasection.isInterpolate());
         }
      }
      ...
      if (!this.isShadersSprite) {
         ...
         if (this.spriteSingle != null) {
            Az iresource = XH.getResourceManager().getResource(resource.getResourceLocation());
   ...
   public void generateMipmaps(int level) {
      ...
      for(int i = 0; i < this.framesTextureData.size(); ++i) {
         ...
         if (aint != null) {
            try {
               list.add(zk.generateMipmapData(level, this.width, aint));
            } catch (Throwable var8) {
               Throwable throwable = var8;
               Er crashreport = Er.makeCrashReport(throwable, "Generating mipmaps for frame");
               Ey crashreportcategory = crashreport.makeCategory("Frame being iterated");
               crashreportcategory.addCrashSection("Frame index", i);
               crashreportcategory.addDetail("Frame sizes", new Ez<String>() {
   ...
   public boolean hasCustomLoader(AA p_hasCustomLoader_1_, ResourceLocation p_hasCustomLoader_2_) {
      return false;
   }

   public boolean load(AA p_load_1_, ResourceLocation p_load_2_, Function<ResourceLocation, zd> p_load_3_) {
      return true;
   }
   ...
   public void updateIndexInMap(bqn p_updateIndexInMap_1_) {
   ...
   public void bindSpriteTexture() {
      if (this.glSpriteTextureId < 0) {
         this.glSpriteTextureId = zk.glGenTextures();
         zk.allocateTextureImpl(this.glSpriteTextureId, this.mipmapLevels, this.width, this.height);
         bqS.applyAnisotropicLevel();
      }

      bqS.bindTexture(this.glSpriteTextureId);
   }

   public void deleteSpriteTexture() {
      if (this.glSpriteTextureId >= 0) {
         zk.deleteTexture(this.glSpriteTextureId);
   ...
   public zY getAnimationMetadata() {
      return this.animationMetadata;
   }

   public void setAnimationMetadata(zY p_setAnimationMetadata_1_) {
   ...
   private void loadShadersSprites() {
      ...
      if (bpq.configNormalMap) {
         ...
         resourcelocation1 = XH.getTextureMap().completeResourceLocation(resourcelocation1);
         if (XH.hasResource(resourcelocation1)) {
            this.spriteNormal = new zd(s1);
            this.spriteNormal.isShadersSprite = true;
            this.spriteNormal.copyFrom(this);
            XH.getTextureMap().generateMipmaps(XH.getResourceManager(), this.spriteNormal);
         }
      }
      ...
      if (bpq.configSpecularMap) {
         ...
         resourcelocation1 = XH.getTextureMap().completeResourceLocation(resourcelocation1);
         if (XH.hasResource(resourcelocation1)) {
            this.spriteSpecular = new zd(s1);
            this.spriteSpecular.isShadersSprite = true;
            this.spriteSpecular.copyFrom(this);
            XH.getTextureMap().generateMipmaps(XH.getResourceManager(), this.spriteSpecular);
         }
      }
