package neo;

import com.google.common.collect.Lists;
import java.util.Iterator;
import java.util.List;
import java.util.Queue;
import java.util.Random;
import net.minecraft.util.EnumFacing;
import net.minecraft.util.EnumParticleTypes;
import net.minecraft.util.NonNullList;
import net.minecraft.util.Tuple;
import net.minecraft.util.math.BlockPos;
import net.minecraft.util.text.translation.I18n;

public class gM extends co {
   public static final hV WET = hV.create("wet");

   protected gM() {
      super(hM.SPONGE);
      this.setDefaultState(this.blockState.getBaseState().withProperty(WET, false));
      this.setCreativeTab(EN.BUILDING_BLOCKS);
   }

   public String getLocalizedName() {
      return I18n.translateToLocal(this.getTranslationKey() + ".dry.name");
   }

   public int damageDropped(in state) {
      return (Boolean)state.getValue(WET) ? 1 : 0;
   }

   public void onBlockAdded(bij worldIn, BlockPos pos, in state) {
      this.tryAbsorb(worldIn, pos, state);
   }

   public void neighborChanged(in state, bij worldIn, BlockPos pos, co blockIn, BlockPos fromPos) {
      this.tryAbsorb(worldIn, pos, state);
      super.neighborChanged(state, worldIn, pos, blockIn, fromPos);
   }

   protected void tryAbsorb(bij worldIn, BlockPos pos, in state) {
      if (!(Boolean)state.getValue(WET) && this.absorb(worldIn, pos)) {
         worldIn.setBlockState(pos, state.withProperty(WET, true), 2);
         worldIn.playEvent(2001, pos, co.getIdFromBlock(Nk.WATER));
      }

   }

   private boolean absorb(bij worldIn, BlockPos pos) {
      Queue<Tuple<BlockPos, Integer>> queue = Lists.newLinkedList();
      List<BlockPos> list = Lists.newArrayList();
      queue.add(new Tuple(pos, 0));
      int i = 0;

      BlockPos blockpos;
      while(!queue.isEmpty()) {
         Tuple<BlockPos, Integer> tuple = (Tuple)queue.poll();
         blockpos = (BlockPos)tuple.getFirst();
         int j = (Integer)tuple.getSecond();
         EnumFacing[] var9 = EnumFacing.values();
         int var10 = var9.length;

         for(int var11 = 0; var11 < var10; ++var11) {
            EnumFacing enumfacing = var9[var11];
            BlockPos blockpos1 = blockpos.offset(enumfacing);
            if (worldIn.getBlockState(blockpos1).getMaterial() == hM.WATER) {
               worldIn.setBlockState(blockpos1, Nk.AIR.getDefaultState(), 2);
               list.add(blockpos1);
               ++i;
               if (j < 6) {
                  queue.add(new Tuple(blockpos1, j + 1));
               }
            }
         }

         if (i > 64) {
            break;
         }
      }

      Iterator var14 = list.iterator();

      while(var14.hasNext()) {
         blockpos = (BlockPos)var14.next();
         worldIn.notifyNeighborsOfStateChange(blockpos, Nk.AIR, false);
      }

      return i > 0;
   }

   public void getSubBlocks(EN itemIn, NonNullList<Qy> items) {
      items.add(new Qy(this, 1, 0));
      items.add(new Qy(this, 1, 1));
   }

   public in getStateFromMeta(int meta) {
      return this.getDefaultState().withProperty(WET, (meta & 1) == 1);
   }

   public int getMetaFromState(in state) {
      return (Boolean)state.getValue(WET) ? 1 : 0;
   }

   protected ii createBlockState() {
      return new ii(this, new hT[]{WET});
   }

   public void randomDisplayTick(in stateIn, bij worldIn, BlockPos pos, Random rand) {
      if ((Boolean)stateIn.getValue(WET)) {
         EnumFacing enumfacing = EnumFacing.random(rand);
         if (enumfacing != EnumFacing.UP && !worldIn.getBlockState(pos.offset(enumfacing)).isTopSolid()) {
            double d0 = (double)pos.getX();
            double d1 = (double)pos.getY();
            double d2 = (double)pos.getZ();
            if (enumfacing == EnumFacing.DOWN) {
               d1 -= 0.05;
               d0 += rand.nextDouble();
               d2 += rand.nextDouble();
            } else {
               d1 += rand.nextDouble() * 0.8;
               if (enumfacing.getAxis() == EnumFacing.Axis.X) {
                  d2 += rand.nextDouble();
                  if (enumfacing == EnumFacing.EAST) {
                     ++d0;
                  } else {
                     d0 += 0.05;
                  }
               } else {
                  d0 += rand.nextDouble();
                  if (enumfacing == EnumFacing.SOUTH) {
                     ++d2;
                  } else {
                     d2 += 0.05;
                  }
               }
            }

            worldIn.spawnParticle(EnumParticleTypes.DRIP_WATER, d0, d1, d2, 0.0, 0.0, 0.0);
         }
      }

   }
}
