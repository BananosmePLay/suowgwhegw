...
public class SA extends ByteBuf {
   private final ByteBuf buf;

   public SA(ByteBuf wrapped) {
      this.buf = wrapped;
   }
   ...
   public SA writeByteArray(byte[] array) {
   ...
   public SA writeVarIntArray(int[] array) {
      this.writeVarInt(array.length);
      int[] var2 = array;
   ...
   public SA writeLongArray(long[] array) {
      this.writeVarInt(array.length);
      long[] var2 = array;
      int var3 = array.length;
   ...
   public SA writeBlockPos(BlockPos pos) {
   ...
   public SA writeTextComponent(ITextComponent component) {
   ...
   public SA writeEnumValue(Enum<?> value) {
   ...
   public SA writeUniqueId(UUID uuid) {
   ...
   public SA writeVarInt(int input) {
      while((input & -128) != 0) {
   ...
   public SA writeVarLong(long value) {
      while((value & -128L) != 0L) {
   ...
   public SA writeCompoundTag(@Nullable QQ nbt) {
      if (nbt == null) {
         this.writeByte(0);
      } else {
         try {
            QF.write(nbt, (DataOutput)(new ByteBufOutputStream(this)));
   ...
   @Nullable
   public QQ readCompoundTag() throws IOException {
      ...
      if (b0 == 0) {
         ...
         try {
            return QF.read(new ByteBufInputStream(this), new QL(2097152L));
   ...
   public SA writeItemStack(Qy stack) {
      if (stack.isEmpty()) {
         this.writeShort(-1);
      } else {
         this.writeShort(OL.getIdFromItem(stack.getItem()));
   ...
   public Qy readItemStack() throws IOException {
      ...
      if (i < 0) {
         ...
         Qy itemstack = new Qy(OL.getItemById(i), j, k);
   ...
   public SA writeString(String string) {
   ...
   public SA writeResourceLocation(ResourceLocation resourceLocationIn) {
