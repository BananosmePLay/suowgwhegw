...
public class bmX {
   private bjr lightMapRgb = null;
   private float[][] sunRgbs = new float[16][3];
   private float[][] torchRgbs = new float[16][3];

   public bmX(bjr lightMapRgb) {
      this.lightMapRgb = lightMapRgb;
   }

   public bjr getColormap() {
      return this.lightMapRgb;
   }

   public boolean updateLightmap(bij world, float torchFlickerX, int[] lmColors, boolean nightvision) {
      if (this.lightMapRgb == null) {
         ...
         if (nightvision && i < 64) {
            ...
            if (j < 16) {
               ...
               float f2 = XH.limitTo1(torchFlickerX + 0.5F) * (float)(j - 1);
               float f3 = XH.limitTo1(XH.getGameSettings().gammaSetting);
               ...
               for(int l = 0; l < 16; ++l) {
                  for(int i1 = 0; i1 < 16; ++i1) {
                     int j1;
                     for(j1 = 0; j1 < 3; ++j1) {
                        float f4 = XH.limitTo1(this.sunRgbs[l][j1] + this.torchRgbs[i1][j1]);
                        if (flag) {
                           float f5 = 1.0F - f4;
                           f5 = 1.0F - f5 * f5 * f5 * f5;
                           f4 = f3 * f5 + (1.0F - f3) * f4;
