...
public class Wk {
   ...
   public void startSection(String name) {
      ...
      if (bmV.isActive()) {
         ...
         if (j == HASH_SCHEDULED_EXECUTABLES && name.equals("scheduledExecutables")) {
            bmV.timerScheduledExecutables.start();
         } else if (j == HASH_TICK && name.equals("tick") && XH.isMinecraftThread()) {
            bmV.timerScheduledExecutables.end();
            bmV.timerTick.start();
   ...
   public List<Wj> getProfilingData(String profilerName) {
      if (!this.profilingEnabled) {
         ...
         List<Wj> list = Lists.newArrayList();
         ...
         if ((float)k > f) {
            list.add(new Wj("unspecified", (double)((float)k - f) * 100.0 / (double)k, (double)((float)k - f) * 100.0 / (double)i));
         }

         Collections.sort(list);
         list.add(0, new Wj(profilerName, 100.0, (double)k * 100.0 / (double)i));
         return list;
      }
   }
