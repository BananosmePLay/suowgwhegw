...
public class boF {
   ...
   public static boq parseLine(String line) {
      Matcher matcher = PATTERN_UNIFORM.matcher(line);
      if (matcher.matches()) {
         return new boq(1, matcher.group(1), "", line);
      } else {
         Matcher matcher1 = PATTERN_ATTRIBUTE.matcher(line);
         if (matcher1.matches()) {
            return new boq(2, matcher1.group(1), "", line);
         } else {
            Matcher matcher2 = PATTERN_PROPERTY.matcher(line);
            if (matcher2.matches()) {
               return new boq(6, matcher2.group(2), matcher2.group(3), line);
            } else {
               Matcher matcher3 = PATTERN_CONST_INT.matcher(line);
               if (matcher3.matches()) {
                  return new boq(3, matcher3.group(1), matcher3.group(2), line);
               } else {
                  Matcher matcher4 = PATTERN_CONST_FLOAT.matcher(line);
                  if (matcher4.matches()) {
                     return new boq(4, matcher4.group(1), matcher4.group(2), line);
                  } else {
                     Matcher matcher5 = PATTERN_CONST_BOOL.matcher(line);
                     if (matcher5.matches()) {
                        return new boq(5, matcher5.group(1), matcher5.group(2), line);
                     } else {
                        Matcher matcher6 = PATTERN_EXTENSION.matcher(line);
                        if (matcher6.matches()) {
                           return new boq(7, matcher6.group(1), matcher6.group(2), line);
                        } else {
                           Matcher matcher7 = PATTERN_CONST_VEC4.matcher(line);
                           return matcher7.matches() ? new boq(8, matcher7.group(1), matcher7.group(2), line) : null;
                        }
                     }
                  }
               }
