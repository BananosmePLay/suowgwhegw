...
public class ic {
   ...
   public ic(bij worldIn, BlockPos posIn, EnumFacing pistonFacing, boolean extending) {
      this.world = worldIn;
   ...
   public boolean canMove() {
      ...
      in iblockstate = this.world.getBlockState(this.blockToMove);
      if (!ff.canPush(iblockstate, this.world, this.blockToMove, this.moveDirection, false, this.moveDirection)) {
         if (iblockstate.getPushReaction() == hJ.DESTROY) {
         ...
         for(int i = 0; i < this.toMove.size(); ++i) {
            ...
            if (this.world.getBlockState(blockpos).getBlock() == Nk.SLIME_BLOCK && !this.addBranchingBlocks(blockpos)) {
               return false;
            }
         }
   ...
   private boolean addBlockLine(BlockPos origin, EnumFacing p_177251_2_) {
      in iblockstate = this.world.getBlockState(origin);
      co block = iblockstate.getBlock();
      if (iblockstate.getMaterial() == hM.AIR) {
         return true;
      } else if (!ff.canPush(iblockstate, this.world, origin, this.moveDirection, false, p_177251_2_)) {
         return true;
         ...
         if (i + this.toMove.size() > 12) {
            ...
            while(block == Nk.SLIME_BLOCK) {
               BlockPos blockpos = origin.offset(this.moveDirection.getOpposite(), i);
               iblockstate = this.world.getBlockState(blockpos);
               block = iblockstate.getBlock();
               if (iblockstate.getMaterial() == hM.AIR || !ff.canPush(iblockstate, this.world, blockpos, this.moveDirection, false, this.moveDirection.getOpposite()) || blockpos.equals(this.pistonPos)) {
                  break;
               }
            ...
            while(true) {
               ...
               if (k > -1) {
                  ...
                  for(int l = 0; l <= k + i1; ++l) {
                     ...
                     if (this.world.getBlockState(blockpos2).getBlock() == Nk.SLIME_BLOCK && !this.addBranchingBlocks(blockpos2)) {
                        return false;
                     }
                  }
               ...
               if (!ff.canPush(iblockstate, this.world, blockpos1, this.moveDirection, true, this.moveDirection) || blockpos1.equals(this.pistonPos)) {
                  return false;
               }
