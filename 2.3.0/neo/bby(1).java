...
public class bby extends bbE {
   private static final iv IS_SAND;
   private final in sandSlab;
   private final in sandstone;
   private final in water;

   public bby() {
      this.sandSlab = Nk.STONE_SLAB.getDefaultState().withProperty(hd.VARIANT, hc.SAND).withProperty(gG.HALF, gF.BOTTOM);
      this.sandstone = Nk.SANDSTONE.getDefaultState();
      this.water = Nk.FLOWING_WATER.getDefaultState();
   }
   ...
   static {
      IS_SAND = iv.forBlock(Nk.SAND).where(gk.VARIANT, Predicates.equalTo(gj.SAND));
   }
}
