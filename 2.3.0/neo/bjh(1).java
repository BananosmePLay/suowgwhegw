...
public class bjh {
   ...
   public int faces = 63;
   public Zi[] biomes = null;
   public bje heights = null;
   ...
   public bjb nbtName = null;
   public int[] sumWeights = null;
   public int sumAllWeights = 1;
   public zd[] matchTileIcons = null;
   public zd[] tileIcons = null;
   public biZ[] connectBlocks = null;
   public String[] connectTiles = null;
   public zd[] connectTileIcons = null;
   public int tintIndex = -1;
   public in tintBlockState;
   ...
   public bjh(Properties props, String path) {
      this.tintBlockState = Nk.AIR.getDefaultState();
      this.layer = null;
      biS connectedparser = new biS("ConnectedTextures");
      ...
      if (this.heights == null) {
         ...
         if (i != -1 || j != 1024) {
            this.heights = new bje(new bjd(i, j));
         }
      }
      ...
      this.tintBlockState = connectedparser.parseBlockState(props.getProperty("tintBlock"), Nk.AIR.getDefaultState());
               ...
               while(true) {
                  ...
                  if (i >= 0 && i <= 46) {
                     ...
                     if (j >= 0 && j < this.tiles.length) {
                        ...
                        XH.warn("Invalid CTM tile index: " + s3);
                     }
                  } else {
                     XH.warn("Invalid CTM index: " + s2);
                  }
               }
            }
         }
   ...
   private String[] parseMatchTiles(String str) {
      if (str == null) {
         ...
         for(int i = 0; i < astring.length; ++i) {
            ...
            s = bqS.fixResourcePath(s, this.basePath);
            astring[i] = s;
         }
   ...
   private String[] parseTileNames(String str) {
      if (str == null) {
         ...
         for(int i = 0; i < astring.length; ++i) {
            ...
            if (s.contains("-")) {
               ...
               if (astring1.length == 2) {
                  int j = XH.parseInt(astring1[0], -1);
                  int k = XH.parseInt(astring1[1], -1);
                  if (j >= 0 && k >= 0) {
                     if (j > k) {
                        XH.warn("Invalid interval: " + s + ", when parsing: " + str);
                        continue;
                     }
         ...
         for(int i1 = 0; i1 < astring2.length; ++i1) {
            ...
            s1 = bqS.fixResourcePath(s1, this.basePath);
   ...
   private static int parseConnect(String str) {
      if (str == null) {
         ...
         if (str.equals("block")) {
            ...
            XH.warn("Unknown connect: " + str);
            return 128;
         }
      }
   }

   public static hT getProperty(String key, Collection properties) {
      ...
      do {
         ...
         iproperty = var2.next();
      } while(!key.equals(((hT)iproperty).getName()));
      ...
      if (this.name != null && this.name.length() > 0) {
         if (this.basePath == null) {
            ...
            if (this.matchBlocks == null && this.matchTiles == null) {
               XH.warn("No matchBlocks or matchTiles specified: " + path);
               return false;
            } else if (this.method == 0) {
               XH.warn("No method: " + path);
               return false;
               ...
               if (this.connect == 128) {
                  XH.warn("Invalid connect in: " + path);
                  return false;
               } else if (this.renderPass > 0) {
                  XH.warn("Render pass not supported: " + this.renderPass);
                  return false;
               } else if ((this.faces & 128) != 0) {
                  XH.warn("Invalid faces in: " + path);
                  return false;
               } else if ((this.symmetry & 128) != 0) {
                  XH.warn("Invalid symmetry in: " + path);
                  return false;
               } else {
                  switch (this.method) {
                     case 1:
                     ...
                     default:
                        XH.warn("Unknown method: " + path);
                        return false;
                  }
               }
            } else {
               XH.warn("No tiles specified: " + path);
               return false;
            }
         }
      } else {
   ...
   private biZ[] detectMatchBlocks() {
      int[] aint = this.detectMatchBlockIds();
      if (aint == null) {
         return null;
      } else {
         biZ[] amatchblock = new biZ[aint.length];

         for(int i = 0; i < amatchblock.length; ++i) {
            amatchblock[i] = new biZ(aint[i]);
         }
   ...
   private String[] detectMatchTiles() {
      zd textureatlassprite = getIcon(this.name);
      return textureatlassprite == null ? null : new String[]{this.name};
   }

   private static zd getIcon(String iconName) {
      zj texturemap = nC.getMinecraft().getTextureMapBlocks();
      zd textureatlassprite = texturemap.getSpriteSafe(iconName);
   ...
   private boolean isValidCtm(String path) {
      ...
      if (this.tiles.length < 47) {
         XH.warn("Invalid tiles, must be at least 47: " + path);
         return false;
      } else {
         return true;
      }
   ...
   private boolean isValidCtmCompact(String path) {
      ...
      if (this.tiles.length < 5) {
         XH.warn("Invalid tiles, must be at least 5: " + path);
         return false;
      } else {
         return true;
      }
   ...
   private boolean isValidOverlay(String path) {
      ...
      if (this.tiles.length < 17) {
         XH.warn("Invalid tiles, must be at least 17: " + path);
         return false;
      } else if (this.layer != null && this.layer != BlockRenderLayer.SOLID) {
         return true;
      } else {
         XH.warn("Invalid overlay layer: " + this.layer);
         return false;
      }
   }
   ...
   private boolean isValidHorizontal(String path) {
      ...
      if (this.tiles.length != 4) {
         XH.warn("Invalid tiles, must be exactly 4: " + path);
         return false;
      } else {
         return true;
      }
   ...
   private boolean isValidVertical(String path) {
      if (this.tiles == null) {
         XH.warn("No tiles defined for vertical: " + path);
         return false;
   ...
   private boolean isValidHorizontalVertical(String path) {
      if (this.tiles == null) {
         XH.warn("No tiles defined for horizontal+vertical: " + path);
         return false;
      } else if (this.tiles.length != 7) {
         XH.warn("Invalid tiles, must be exactly 7: " + path);
         return false;
      } else {
         return true;
      }
   ...
   private boolean isValidVerticalHorizontal(String path) {
      if (this.tiles == null) {
         XH.warn("No tiles defined for vertical+horizontal: " + path);
         return false;
      } else if (this.tiles.length != 7) {
         XH.warn("Invalid tiles, must be exactly 7: " + path);
         return false;
      ...
      if (this.tiles != null && this.tiles.length > 0) {
         if (this.weights != null) {
            ...
            if (this.weights.length > this.tiles.length) {
               XH.warn("More weights defined than tiles, trimming weights: " + path);
            ...
            int l;
            if (this.weights.length < this.tiles.length) {
               XH.warn("Less weights defined than tiles, expanding weights: " + path);
               aint1 = new int[this.tiles.length];
               System.arraycopy(this.weights, 0, aint1, 0, this.weights.length);
               l = bqD.getAverage(this.weights);
         ...
         if (this.randomLoops >= 0 && this.randomLoops <= 9) {
            ...
            XH.warn("Invalid randomLoops: " + this.randomLoops);
            return false;
         }
      } else {
         XH.warn("Tiles not defined: " + path);
         return false;
      }
   }
   ...
   private boolean isValidRepeat(String path) {
      if (this.tiles == null) {
         ...
         XH.warn("Invalid width: " + path);
         return false;
      } else if (this.height <= 0) {
         XH.warn("Invalid height: " + path);
         return false;
      } else if (this.tiles.length != this.width * this.height) {
         XH.warn("Number of tiles does not equal width x height: " + path);
         return false;
      } else {
         return true;
      }
   ...
   private boolean isValidFixed(String path) {
      if (this.tiles == null) {
         ...
         XH.warn("Number of tiles should be 1 for method: fixed.");
         return false;
      } else {
         return true;
      }
   ...
   private boolean isValidTop(String path) {
      ...
      if (this.tiles.length != 1) {
         XH.warn("Invalid tiles, must be exactly 1: " + path);
         return false;
      } else {
         return true;
      }
   ...
   private static zd[] registerIcons(String[] tileNames, zj textureMap, boolean skipTiles, boolean defaultTiles) {
      if (tileNames == null) {
         return null;
      } else {
         List list = new ArrayList();
         ...
         zd[] atextureatlassprite = (zd[])((zd[])list.toArray(new zd[list.size()]));
         return atextureatlassprite;
      }
   }

   public boolean matchesBlockId(int blockId) {
      return bja.blockId(blockId, this.matchBlocks);
   }

   public boolean matchesBlock(int blockId, int metadata) {
      return !bja.block(blockId, metadata, this.matchBlocks) ? false : bja.metadata(metadata, this.metadatas);
   }

   public boolean matchesIcon(zd icon) {
      return bja.sprite(icon, this.matchTileIcons);
   }

   public String toString() {
      return "CTM name: " + this.name + ", basePath: " + this.basePath + ", matchBlocks: " + XH.arrayToString((Object[])this.matchBlocks) + ", matchTiles: " + XH.arrayToString((Object[])this.matchTiles);
   }

   public boolean matchesBiome(Zi biome) {
      return bja.biome(biome, this.biomes);
   }
   ...
   public int getMetadataMax() {
      ...
      if (this.matchBlocks != null) {
         for(int j = 0; j < this.matchBlocks.length; ++j) {
            biZ matchblock = this.matchBlocks[j];
