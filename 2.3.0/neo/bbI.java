...
public class bbI extends bbE {
   ...
   public boolean generate(bij worldIn, Random rand, BlockPos position) {
      ...
      bfK template = templatemanager.getTemplate(minecraftserver, FOSSILS[i]);
      bfK template1 = templatemanager.getTemplate(minecraftserver, FOSSILS_COAL[i]);
      ChunkPos chunkpos = new ChunkPos(position);
      bdy structureboundingbox = new bdy(chunkpos.getXStart(), 0, chunkpos.getZStart(), chunkpos.getXEnd(), 256, chunkpos.getZEnd());
      bfD placementsettings = (new bfD()).setRotation(rotation).setBoundingBox(structureboundingbox).setRandom(random);
