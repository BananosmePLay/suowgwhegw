...
public class mA {
   private List<ms> buttons = Lists.newArrayListWithCapacity(20);
   private ms hoveredButton;
   private mw overlay = new mw();
   private nC minecraft;
   private List<my> listeners = Lists.newArrayList();
   private List<mB> recipeLists;
   private jN forwardButton;
   private jN backButton;
   private int totalPages;
   private int currentPage;
   private XK recipeBook;
   private NT lastClickedRecipe;
   private mB lastClickedRecipeList;

   public mA() {
      for(int i = 0; i < 20; ++i) {
         this.buttons.add(new ms());
      }

   }

   public void init(nC p_194194_1_, int p_194194_2_, int p_194194_3_) {
      this.minecraft = p_194194_1_;
      this.recipeBook = nC.player.getRecipeBook();

      for(int i = 0; i < this.buttons.size(); ++i) {
         ((ms)this.buttons.get(i)).setPosition(p_194194_2_ + 11 + 25 * (i % 5), p_194194_3_ + 31 + 25 * (i / 5));
      }

      this.forwardButton = new jN(0, p_194194_2_ + 93, p_194194_3_ + 137, 12, 17, false);
      this.forwardButton.initTextureValues(1, 208, 13, 18, mu.RECIPE_BOOK);
      this.backButton = new jN(0, p_194194_2_ + 38, p_194194_3_ + 137, 12, 17, true);
      this.backButton.initTextureValues(1, 208, 13, 18, mu.RECIPE_BOOK);
   }

   public void addListener(mu p_193732_1_) {
      this.listeners.remove(p_193732_1_);
      this.listeners.add(p_193732_1_);
   }

   public void updateLists(List<mB> p_194192_1_, boolean p_194192_2_) {
   ...
   private void updateButtonsForPage() {
      ...
      for(int j = 0; j < this.buttons.size(); ++j) {
         ms guibuttonrecipe = (ms)this.buttons.get(j);
         if (i + j < this.recipeLists.size()) {
            mB recipelist = (mB)this.recipeLists.get(i + j);
   ...
   public void render(int p_194191_1_, int p_194191_2_, int p_194191_3_, int p_194191_4_, float p_194191_5_) {
      ...
      while(var8.hasNext()) {
         ms guibuttonrecipe = (ms)var8.next();
   ...
   @Nullable
   public NT getLastClickedRecipe() {
      return this.lastClickedRecipe;
   }

   @Nullable
   public mB getLastClickedRecipeList() {
   ...
   public void recipesShown(List<NT> p_194195_1_) {
      Iterator var2 = this.listeners.iterator();

      while(var2.hasNext()) {
         my irecipeupdatelistener = (my)var2.next();
