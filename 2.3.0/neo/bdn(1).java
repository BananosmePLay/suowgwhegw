...
public class bdn extends bdr {
   private final List<Zi> allowedBiomes;
   ...
   public bdn() {
      ...
      Iterator var1 = Zi.REGISTRY.iterator();

      while(var1.hasNext()) {
         Zi biome = (Zi)var1.next();
   ...
   public bdn(Map<String, String> p_i2068_1_) {
      this();
      ...
      while(var2.hasNext()) {
         ...
         if (((String)entry.getKey()).equals("distance")) {
            this.distance = MathHelper.getDouble((String)entry.getValue(), this.distance, 1.0);
   ...
   private void generatePositions() {
      ...
      while(lvt_2_1_.hasNext()) {
         beM structurestart = (beM)lvt_2_1_.next();
      ...
      if (l < this.structureCoords.length) {
         for(int i1 = 0; i1 < this.structureCoords.length; ++i1) {
            ...
            d1 += 6.283185307179586 / (double)this.spread;
            ++k;
            if (k == this.spread) {
               ++j;
               k = 0;
   ...
   protected beM getStructureStart(int chunkX, int chunkZ) {
      bdm mapgenstronghold$start;
      for(mapgenstronghold$start = new bdm(this.world, this.rand, chunkX, chunkZ); mapgenstronghold$start.getComponents().isEmpty() || ((bfb)mapgenstronghold$start.getComponents().get(0)).strongholdPortalRoom == null; mapgenstronghold$start = new bdm(this.world, this.rand, chunkX, chunkZ)) {
      }
