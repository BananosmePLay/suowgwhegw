...
public class qH extends pM {
   private final Ig item;
   private final Ig target;
   private int age;
   private final int maxAge;
   private final float yOffset;
   private final wC renderManager = nC.getMinecraft().getRenderManager();

   public qH(bij worldIn, Ig p_i1233_2_, Ig p_i1233_3_, float p_i1233_4_) {
   ...
   public void renderParticle(tN buffer, Ig entityIn, float partialTicks, float rotationX, float rotationZ, float rotationYZ, float rotationXY, float rotationXZ) {
      bpg program = null;
      if (XH.isShaders()) {
         program = bpq.activeProgram;
         bpq.nextEntity(this.item);
      }
      ...
      if (XH.isShaders()) {
         bpq.setEntityId((Ig)null);
         bpq.useProgram(program);
      }
