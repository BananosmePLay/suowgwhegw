package neo;

public class bcJ extends bco {
   public bcJ(long p_i2134_1_, bco p_i2134_3_) {
      super(p_i2134_1_);
      super.parent = p_i2134_3_;
   }

   public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
      int i = areaX >> 1;
      int j = areaY >> 1;
      int k = (areaWidth >> 1) + 2;
      int l = (areaHeight >> 1) + 2;
      int[] aint = this.parent.getInts(i, j, k, l);
      int i1 = k - 1 << 1;
      int j1 = l - 1 << 1;
      int[] aint1 = bcK.getIntCache(i1 * j1);

      int j3;
      for(int k1 = 0; k1 < l - 1; ++k1) {
         j3 = (k1 << 1) * i1;
         int i2 = 0;
         int j2 = aint[i2 + 0 + (k1 + 0) * k];

         for(int k2 = aint[i2 + 0 + (k1 + 1) * k]; i2 < k - 1; ++i2) {
            this.initChunkSeed((long)(i2 + i << 1), (long)(k1 + j << 1));
            int l2 = aint[i2 + 1 + (k1 + 0) * k];
            int i3 = aint[i2 + 1 + (k1 + 1) * k];
            aint1[j3] = j2;
            aint1[j3++ + i1] = this.selectRandom2(j2, k2);
            aint1[j3] = this.selectRandom2(j2, l2);
            aint1[j3++ + i1] = this.selectModeOrRandom(j2, l2, k2, i3);
            j2 = l2;
            k2 = i3;
         }
      }

      int[] aint2 = bcK.getIntCache(areaWidth * areaHeight);

      for(j3 = 0; j3 < areaHeight; ++j3) {
         System.arraycopy(aint1, (j3 + (areaY & 1)) * i1 + (areaX & 1), aint2, j3 * areaWidth, areaWidth);
      }

      return aint2;
   }

   public static bco magnify(long p_75915_0_, bco p_75915_2_, int p_75915_3_) {
      bco genlayer = p_75915_2_;

      for(int i = 0; i < p_75915_3_; ++i) {
         genlayer = new bcJ(p_75915_0_ + (long)i, (bco)genlayer);
      }

      return (bco)genlayer;
   }

   protected int selectRandom2(int p_selectRandom2_1_, int p_selectRandom2_2_) {
      int i = this.nextInt(2);
      return i == 0 ? p_selectRandom2_1_ : p_selectRandom2_2_;
   }
}
