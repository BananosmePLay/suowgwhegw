...
public class XU extends XT {
   ...
   private final Set<XQ> dirty = Sets.newHashSet();
   private int lastStatRequest = -300;

   public XU(Xx serverIn, File statsFileIn) {
   ...
   private Set<XQ> getDirty() {
      Set<XQ> set = Sets.newHashSet(this.dirty);
      this.dirty.clear();
      return set;
   }

   public Map<XQ, TupleIntJsonSerializable> parseJson(String p_150881_1_) {
      JsonElement jsonelement = (new JsonParser()).parse(p_150881_1_);
      if (!jsonelement.isJsonObject()) {
         return Maps.newHashMap();
      } else {
         JsonObject jsonobject = jsonelement.getAsJsonObject();
         Map<XQ, TupleIntJsonSerializable> map = Maps.newHashMap();
         Iterator var5 = jsonobject.entrySet().iterator();

         while(true) {
            while(var5.hasNext()) {
               Map.Entry<String, JsonElement> entry = (Map.Entry)var5.next();
               XQ statbase = XV.getOneShotStat((String)entry.getKey());
               if (statbase != null) {
   ...
   public static String dumpJson(Map<XQ, TupleIntJsonSerializable> p_150880_0_) {
      JsonObject jsonobject = new JsonObject();
      Iterator var2 = p_150880_0_.entrySet().iterator();

      while(var2.hasNext()) {
         Map.Entry<XQ, TupleIntJsonSerializable> entry = (Map.Entry)var2.next();
         if (((TupleIntJsonSerializable)entry.getValue()).getJsonSerializableValue() != null) {
            ...
            try {
               ...
               LOGGER.warn("Couldn't save statistic {}: error serializing progress", ((XQ)entry.getKey()).getStatName(), throwable);
            }

            jsonobject.add(((XQ)entry.getKey()).statId, jsonobject1);
         } else {
            jsonobject.addProperty(((XQ)entry.getKey()).statId, ((TupleIntJsonSerializable)entry.getValue()).getIntegerValue());
         }
      }
