...
public class mf extends lg {
   ...
   private lE posYEdit;
   private lE posZEdit;
   private lE sizeXEdit;
   private lE sizeYEdit;
   private lE sizeZEdit;
   private lE integrityEdit;
   private lE seedEdit;
   private lE dataEdit;
   private jK doneButton;
   private jK cancelButton;
   private jK saveButton;
   private jK loadButton;
   private jK rotateZeroDegreesButton;
   private jK rotateNinetyDegreesButton;
   private jK rotate180DegreesButton;
   private jK rotate270DegressButton;
   private jK modeButton;
   private jK detectSizeButton;
   private jK showEntitiesButton;
   private jK mirrorButton;
   private jK showAirButton;
   private jK showBoundingBoxButton;
   private final List<lE> tabOrder;
   private final DecimalFormat decimalFormat;

   public mf(YV p_i47142_1_) {
   ...
   public void initGui() {
      ...
      this.doneButton = this.addButton(new jK(0, this.width / 2 - 4 - 150, 210, 150, 20, Ax.format("gui.done")));
      this.cancelButton = this.addButton(new jK(1, this.width / 2 + 4, 210, 150, 20, Ax.format("gui.cancel")));
      this.saveButton = this.addButton(new jK(9, this.width / 2 + 4 + 100, 185, 50, 20, Ax.format("structure_block.button.save")));
      this.loadButton = this.addButton(new jK(10, this.width / 2 + 4 + 100, 185, 50, 20, Ax.format("structure_block.button.load")));
      this.modeButton = this.addButton(new jK(18, this.width / 2 - 4 - 150, 185, 50, 20, "MODE"));
      this.detectSizeButton = this.addButton(new jK(19, this.width / 2 + 4 + 100, 120, 50, 20, Ax.format("structure_block.button.detect_size")));
      this.showEntitiesButton = this.addButton(new jK(20, this.width / 2 + 4 + 100, 160, 50, 20, "ENTITIES"));
      this.mirrorButton = this.addButton(new jK(21, this.width / 2 - 20, 185, 40, 20, "MIRROR"));
      this.showAirButton = this.addButton(new jK(22, this.width / 2 + 4 + 100, 80, 50, 20, "SHOWAIR"));
      this.showBoundingBoxButton = this.addButton(new jK(23, this.width / 2 + 4 + 100, 80, 50, 20, "SHOWBB"));
      this.rotateZeroDegreesButton = this.addButton(new jK(11, this.width / 2 - 1 - 40 - 1 - 40 - 20, 185, 40, 20, "0"));
      this.rotateNinetyDegreesButton = this.addButton(new jK(12, this.width / 2 - 1 - 40 - 20, 185, 40, 20, "90"));
      this.rotate180DegreesButton = this.addButton(new jK(13, this.width / 2 + 1 + 20, 185, 40, 20, "180"));
      this.rotate270DegressButton = this.addButton(new jK(14, this.width / 2 + 1 + 40 + 1 + 20, 185, 40, 20, "270"));
      this.nameEdit = new lE(2, this.fontRenderer, this.width / 2 - 152, 40, 300, 20);
      ...
      this.sizeXEdit = new lE(6, this.fontRenderer, this.width / 2 - 152, 120, 80, 20);
      ...
      this.sizeYEdit = new lE(7, this.fontRenderer, this.width / 2 - 72, 120, 80, 20);
      ...
      this.sizeZEdit = new lE(8, this.fontRenderer, this.width / 2 + 8, 120, 80, 20);
      ...
      this.integrityEdit = new lE(15, this.fontRenderer, this.width / 2 - 152, 120, 80, 20);
      ...
      this.seedEdit = new lE(16, this.fontRenderer, this.width / 2 - 72, 120, 80, 20);
      ...
      this.dataEdit = new lE(17, this.fontRenderer, this.width / 2 - 152, 120, 240, 20);
   ...
   protected void actionPerformed(jK button) throws IOException {
      if (button.enabled) {
         if (button.id == 1) {
            ...
            if (this.tileStructure.getMode() == YU.SAVE) {
               this.sendToServer(2);
               this.mc.displayGuiScreen((lg)null);
            }
         } else if (button.id == 10) {
            if (this.tileStructure.getMode() == YU.LOAD) {
               this.sendToServer(3);
               this.mc.displayGuiScreen((lg)null);
            }
   ...
   private void updateEntitiesButton() {
      ...
      if (flag) {
         this.showEntitiesButton.displayString = Ax.format("options.on");
      } else {
         this.showEntitiesButton.displayString = Ax.format("options.off");
      }
   ...
   private void updateToggleAirButton() {
      ...
      if (flag) {
         this.showAirButton.displayString = Ax.format("options.on");
      } else {
         this.showAirButton.displayString = Ax.format("options.off");
      }
   ...
   private void updateToggleBoundingBox() {
      ...
      if (flag) {
         this.showBoundingBoxButton.displayString = Ax.format("options.on");
      } else {
         this.showBoundingBoxButton.displayString = Ax.format("options.off");
      }
   ...
   private void updateMode() {
      ...
      this.modeButton.displayString = Ax.format("structure_block.mode." + this.tileStructure.getMode().getName());
   }
   ...
   private boolean sendToServer(int p_189820_1_) {
      try {
         ...
         this.mc.getConnection().sendPacket(new SN("MC|Struct", packetbuffer));
         return true;
   ...
   protected void keyTyped(char typedChar, int keyCode) throws IOException {
      ...
      if (keyCode == 15) {
         lE guitextfield = null;
         lE guitextfield1 = null;
         Iterator var5 = this.tabOrder.iterator();

         lE guitextfield3;
         while(var5.hasNext()) {
            guitextfield3 = (lE)var5.next();
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      ...
      YU tileentitystructure$mode = this.tileStructure.getMode();
      this.drawCenteredString(this.fontRenderer, Ax.format("tile.structureBlock.name"), this.width / 2, 10, 16777215);
      if (tileentitystructure$mode != YU.DATA) {
         this.drawString(this.fontRenderer, Ax.format("structure_block.structure_name"), this.width / 2 - 153, 30, 10526880);
         this.nameEdit.drawTextBox();
      }

      String s4;
      int l;
      if (tileentitystructure$mode == YU.LOAD || tileentitystructure$mode == YU.SAVE) {
         this.drawString(this.fontRenderer, Ax.format("structure_block.position"), this.width / 2 - 153, 70, 10526880);
         this.posXEdit.drawTextBox();
         this.posYEdit.drawTextBox();
         this.posZEdit.drawTextBox();
         s4 = Ax.format("structure_block.include_entities");
         l = this.fontRenderer.getStringWidth(s4);
         this.drawString(this.fontRenderer, s4, this.width / 2 + 154 - l, 150, 10526880);
      }

      if (tileentitystructure$mode == YU.SAVE) {
         this.drawString(this.fontRenderer, Ax.format("structure_block.size"), this.width / 2 - 153, 110, 10526880);
         this.sizeXEdit.drawTextBox();
         this.sizeYEdit.drawTextBox();
         this.sizeZEdit.drawTextBox();
         s4 = Ax.format("structure_block.detect_size");
         l = this.fontRenderer.getStringWidth(s4);
         this.drawString(this.fontRenderer, s4, this.width / 2 + 154 - l, 110, 10526880);
         String s1 = Ax.format("structure_block.show_air");
         int j = this.fontRenderer.getStringWidth(s1);
         this.drawString(this.fontRenderer, s1, this.width / 2 + 154 - j, 70, 10526880);
      }

      if (tileentitystructure$mode == YU.LOAD) {
         this.drawString(this.fontRenderer, Ax.format("structure_block.integrity"), this.width / 2 - 153, 110, 10526880);
         this.integrityEdit.drawTextBox();
         this.seedEdit.drawTextBox();
         s4 = Ax.format("structure_block.show_boundingbox");
         l = this.fontRenderer.getStringWidth(s4);
         this.drawString(this.fontRenderer, s4, this.width / 2 + 154 - l, 70, 10526880);
      }

      if (tileentitystructure$mode == YU.DATA) {
         this.drawString(this.fontRenderer, Ax.format("structure_block.custom_data"), this.width / 2 - 153, 110, 10526880);
         this.dataEdit.drawTextBox();
      }

      s4 = "structure_block.mode_info." + tileentitystructure$mode.getName();
      this.drawString(this.fontRenderer, Ax.format(s4), this.width / 2 - 153, 174, 10526880);
