...
public class bok {
   ...
   private boolean eval(String str) {
      ...
      if (i >= 100) {
         XH.warn("Too many iterations: " + i + ", when resolving: " + str);
         return true;
      } else {
         try {
            blZ iexpressionresolver = new boi(this.mapMacroValues);
            blM expressionparser = new blM(iexpressionresolver);
            blU iexpression = expressionparser.parse(str);
            if (iexpression.getExpressionType() == blN.BOOL) {
               blV iexpressionbool = (blV)iexpression;
               boolean flag1 = iexpressionbool.eval();
               return flag1;
            } else if (iexpression.getExpressionType() == blN.FLOAT) {
               blX iexpressionfloat = (blX)iexpression;
               float f = iexpressionfloat.eval();
               boolean flag2 = f != 0.0F;
               return flag2;
            } else {
               throw new bmd("Not a boolean or float expression: " + iexpression.getExpressionType());
            }
         } catch (bmd var12) {
            XH.warn("Invalid macro expression: " + str);
            XH.warn("Error: " + var12.getMessage());
            return false;
         }
      }
   }
