...
public class Hb extends Gi {
   private final Lw horseHost;
   private final double speed;
   private double targetX;
   private double targetY;
   private double targetZ;

   public Hb(Lw horse, double speedIn) {
   ...
   public boolean shouldExecute() {
      if (!this.horseHost.isTame() && this.horseHost.isBeingRidden()) {
         Vec3d vec3d = HA.findRandomTarget(this.horseHost, 5, 4);
         if (vec3d == null) {
            return false;
         } else {
   ...
   public void updateTask() {
      if (!this.horseHost.isTame() && this.horseHost.getRNG().nextInt(50) == 0) {
         Ig entity = (Ig)this.horseHost.getPassengers().get(0);
         if (entity == null) {
            return;
         }
         ...
         if (entity instanceof ME) {
            ...
            if (j > 0 && this.horseHost.getRNG().nextInt(j) < i) {
               this.horseHost.setTamedBy((ME)entity);
               return;
            }
