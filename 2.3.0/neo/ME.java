...
public abstract class ME extends Iw {
   private static final Rd<Float> ABSORPTION;
   private static final Rd<Integer> PLAYER_SCORE;
   protected static final Rd<Byte> PLAYER_MODEL_FLAG;
   protected static final Rd<Byte> MAIN_HAND;
   protected static final Rd<QQ> LEFT_SHOULDER_ENTITY;
   protected static final Rd<QQ> RIGHT_SHOULDER_ENTITY;
   public MJ inventory = new MJ(this);
   ...
   @Nullable
   public MU fishEntity;

   protected CooldownTracker createCooldownTracker() {
      return new CooldownTracker();
   }

   public ME(bij worldIn, GameProfile gameProfileIn) {
      super(worldIn);
   ...
   protected void applyEntityAttributes() {
      ...
      this.getAttributeMap().registerAttribute(Ni.ATTACK_DAMAGE).setBaseValue(1.0);
      this.getEntityAttribute(Ni.MOVEMENT_SPEED).setBaseValue(0.10000000149011612);
      this.getAttributeMap().registerAttribute(Ni.ATTACK_SPEED);
      this.getAttributeMap().registerAttribute(Ni.LUCK);
   }
   ...
   protected void entityInit() {
      ...
      this.dataManager.register(LEFT_SHOULDER_ENTITY, new QQ());
      this.dataManager.register(RIGHT_SHOULDER_ENTITY, new QQ());
   }

   public void onUpdate() {
      this.noClip = 0bz.method_Qm().method_Qs().method_bxY(0bQ.class).method_bBh() || this.isSpectator();
      if (this.isSpectator()) {
         this.onGround = false;
      }
      ...
      if (!this.world.isRemote) {
         ...
         this.addStat(XV.PLAY_ONE_MINUTE);
         if (this.isEntityAlive()) {
            this.addStat(XV.TIME_SINCE_DEATH);
         }

         if (this.isSneaking()) {
            this.addStat(XV.SNEAK_TIME);
         }
      }
      ...
      if (!Qy.areItemStacksEqual(this.itemStackMainHand, itemstack)) {
         if (!Qy.areItemsEqualIgnoreDurability(this.itemStackMainHand, itemstack)) {
            this.resetCooldown();
         }

         this.itemStackMainHand = itemstack.isEmpty() ? Qy.EMPTY : itemstack.copy();
      }
   ...
   protected SoundEvent getSwimSound() {
      return NO.ENTITY_PLAYER_SWIM;
   }

   protected SoundEvent getSplashSound() {
      return NO.ENTITY_PLAYER_SPLASH;
   }
   ...
   public void updateRidden() {
      if (!this.world.isRemote && this.isSneaking() && this.isRiding()) {
         ...
         if (this.getRidingEntity() instanceof LQ) {
            this.rotationPitch = f1;
            this.rotationYaw = f;
            this.renderYawOffset = ((LQ)this.getRidingEntity()).renderYawOffset;
         }
      }
   ...
   public void onLivingUpdate() {
      ...
      if (this.world.getDifficulty() == baV.PEACEFUL && this.world.getGameRules().getBoolean("naturalRegeneration")) {
      ...
      if (this.getHealth() > 0.0F && !this.isSpectator()) {
         ...
         List<Ig> list = this.world.getEntitiesWithinAABBExcludingEntity(this, axisalignedbb);
   ...
   private void playShoulderEntityAmbientSound(@Nullable QQ p_192028_1_) {
      if (p_192028_1_ != null && !p_192028_1_.hasKey("Silent") || !p_192028_1_.getBoolean("Silent")) {
         String s = p_192028_1_.getString("id");
         if (s.equals(Ir.getKey(LP.class).toString())) {
            LP.playAmbientSound(this.world, this);
         }
      }

   }

   private void collideWithPlayer(Ig entityIn) {
   ...
   public void onDeath(DamageSource cause) {
      ...
      if ("Notch".equals(this.getName())) {
         this.dropItem(new Qy(NK.APPLE, 1), true, false);
      }
   ...
   protected void destroyVanishingCursedItems() {
      for(int i = 0; i < this.inventory.getSizeInventory(); ++i) {
         ...
         if (!itemstack.isEmpty() && Ft.hasVanishingCurse(itemstack)) {
   ...
   protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
      if (damageSourceIn == DamageSource.ON_FIRE) {
         return NO.ENTITY_PLAYER_HURT_ON_FIRE;
      } else {
         return damageSourceIn == DamageSource.DROWN ? NO.ENTITY_PLAYER_HURT_DROWN : NO.ENTITY_PLAYER_HURT;
      }
   }

   protected SoundEvent getDeathSound() {
      return NO.ENTITY_PLAYER_DEATH;
   }

   @Nullable
   public IY dropItem(boolean dropAll) {
   ...
   @Nullable
   public IY dropItem(Qy itemStackIn, boolean unused) {
      return this.dropItem(itemStackIn, false, unused);
   }

   @Nullable
   public IY dropItem(Qy droppedItem, boolean dropAround, boolean traceItem) {
      if (droppedItem.isEmpty()) {
         return null;
      } else {
         double d0 = this.posY - 0.30000001192092896 + (double)this.getEyeHeight();
         IY entityitem = new IY(this.world, this.posX, d0, this.posZ, droppedItem);
         ...
         Qy itemstack = this.dropItemAndGetStack(entityitem);
         if (traceItem) {
            if (!itemstack.isEmpty()) {
               this.addStat(XV.getDroppedObjectStats(itemstack.getItem()), droppedItem.getCount());
            }

            this.addStat(XV.DROP);
         }
   ...
   public Qy getEquipmentInSlot(int p_71124_1_) {
      return p_71124_1_ == 0 ? this.inventory.getCurrentItem() : (Qy)this.inventory.armorInventory.get(p_71124_1_ - 1);
   }

   protected Qy dropItemAndGetStack(IY p_184816_1_) {
      this.world.spawnEntity(p_184816_1_);
      return p_184816_1_.getItem();
   }

   public float getDigSpeed(in state) {
      float f = this.inventory.getDestroySpeed(state);
      if (f > 1.0F) {
         int i = Ft.getEfficiencyModifier(this);
         Qy itemstack = this.getHeldItemMainhand();
      ...
      if (this.isPotionActive(NL.HASTE)) {
         f *= 1.0F + (float)(this.getActivePotionEffect(NL.HASTE).getAmplifier() + 1) * 0.2F;
      }

      if (this.isPotionActive(NL.MINING_FATIGUE)) {
         float f1;
         switch (this.getActivePotionEffect(NL.MINING_FATIGUE).getAmplifier()) {
            case 0:
               f1 = 0.3F;
               break;
            case 1:
      ...
      if (this.isInsideOfMaterial(hM.WATER) && !Ft.getAquaAffinityModifier(this)) {
         f /= 5.0F;
      }
   ...
   public void readEntityFromNBT(QQ compound) {
      ...
      if (compound.hasKey("EnderItems", 9)) {
         QW nbttaglist1 = compound.getTagList("EnderItems", 10);
   ...
   public void writeEntityToNBT(QQ compound) {
      ...
      compound.setTag("Inventory", this.inventory.writeToNBT(new QW()));
      ...
      if (this.isEntityInvulnerable(source)) {
         ...
         if (this.getHealth() <= 0.0F) {
            ...
            if (source.isDifficultyScaled()) {
               ...
               if (this.world.getDifficulty() == baV.EASY) {
   ...
   protected void blockUsingShield(Iw p_190629_1_) {
      ...
      if (p_190629_1_.getHeldItemMainhand().getItem() instanceof OU) {
   ...
   protected void damageShield(float damage) {
      if (damage >= 3.0F && this.activeItemStack.getItem() == NK.SHIELD) {
         ...
         if (this.activeItemStack.isEmpty()) {
            ...
            if (enumhand == EnumHand.MAIN_HAND) {
               ...
               this.setItemStackToSlot(EntityEquipmentSlot.OFFHAND, Qy.EMPTY);
            }
      ...
      if (!this.isEntityInvulnerable(damageSrc)) {
         ...
         if (damageAmount != 0.0F) {
            ...
            if (damageAmount < 3.4028235E37F) {
               this.addStat(XV.DAMAGE_TAKEN, Math.round(damageAmount * 10.0F));
            }
         }
      }
   ...
   public void displayGuiEditCommandCart(XZ commandBlock) {
   }

   public void displayGuiCommandBlock(Yq commandBlock) {
   }

   public void openEditStructure(YV structure) {
   }
   ...
   public EnumActionResult interactOn(Ig entityToInteractOn, EnumHand hand) {
      if (this.isSpectator()) {
         ...
         Qy itemstack1 = itemstack.isEmpty() ? Qy.EMPTY : itemstack.copy();
         if (entityToInteractOn.processInitialInteract(this, hand)) {
            ...
            if (!itemstack.isEmpty() && entityToInteractOn instanceof Iw) {
               if (this.capabilities.isCreativeMode) {
                  itemstack = itemstack1;
               }

               if (itemstack.interactWithEntity(this, (Iw)entityToInteractOn, hand)) {
                  if (itemstack.isEmpty() && !this.capabilities.isCreativeMode) {
                     this.setHeldItem(hand, Qy.EMPTY);
                  }
   ...
   public void attackTargetEntityWithCurrentItem(Ig targetEntity) {
      if (targetEntity.canBeAttackedWithItem() && !targetEntity.hitByEntity(this)) {
         ...
         float f1;
         if (targetEntity instanceof Iw) {
            f1 = Ft.getModifierForCreature(this.getHeldItemMainhand(), ((Iw)targetEntity).getCreatureAttribute());
         } else {
            f1 = Ft.getModifierForCreature(this.getHeldItemMainhand(), IB.UNDEFINED);
         }
         ...
         if (f > 0.0F || f1 > 0.0F) {
            ...
            if (this.isSprinting() && flag) {
               this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_ATTACK_KNOCKBACK, this.getSoundCategory(), 1.0F, 1.0F);
               ++i;
               flag1 = true;
            }

            boolean flag2 = flag && this.fallDistance > 0.0F && !this.onGround && !this.isOnLadder() && !this.isInWater() && !this.isPotionActive(NL.BLINDNESS) && !this.isRiding() && targetEntity instanceof Iw;
            ...
            if (flag && !flag2 && !flag1 && this.onGround && d0 < (double)this.getAIMoveSpeed()) {
               Qy itemstack = this.getHeldItem(EnumHand.MAIN_HAND);
               if (itemstack.getItem() instanceof Qz) {
                  flag3 = true;
               }
            }
            ...
            if (flag5) {
               if (i > 0) {
                  if (targetEntity instanceof Iw) {
                     ((Iw)targetEntity).knockBack(this, (float)i * 0.5F, (double)MathHelper.sin(this.rotationYaw * 0.017453292F), (double)(-MathHelper.cos(this.rotationYaw * 0.017453292F)));
                  } else {
                     targetEntity.addVelocity((double)(-MathHelper.sin(this.rotationYaw * 0.017453292F) * (float)i * 0.5F), 0.1, (double)(MathHelper.cos(this.rotationYaw * 0.017453292F) * (float)i * 0.5F));
                  }
               ...
               if (flag3) {
                  float f3 = 1.0F + Ft.getSweepingDamageRatio(this) * f;
                  Iterator var23 = this.world.getEntitiesWithinAABB(Iw.class, targetEntity.getEntityBoundingBox().grow(1.0, 0.25, 1.0)).iterator();

                  while(var23.hasNext()) {
                     Iw entitylivingbase = (Iw)var23.next();
                  ...
                  this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_ATTACK_SWEEP, this.getSoundCategory(), 1.0F, 1.0F);
                  this.spawnSweepParticles();
               }

               if (targetEntity instanceof MG && targetEntity.velocityChanged) {
                  ((MG)targetEntity).connection.sendPacket(new Uf(targetEntity));
               ...
               if (flag2) {
                  this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_ATTACK_CRIT, this.getSoundCategory(), 1.0F, 1.0F);
                  this.onCriticalHit(targetEntity);
               }

               if (!flag2 && !flag3) {
                  if (flag) {
                     this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_ATTACK_STRONG, this.getSoundCategory(), 1.0F, 1.0F);
                  } else {
                     this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_ATTACK_WEAK, this.getSoundCategory(), 1.0F, 1.0F);
                  }
               }
               ...
               if (targetEntity instanceof Iw) {
                  Ft.applyThornEnchantments((Iw)targetEntity, this);
               }

               Ft.applyArthropodEnchantments(this, targetEntity);
               Qy itemstack1 = this.getHeldItemMainhand();
               Ig entity = targetEntity;
               if (targetEntity instanceof Lr) {
                  IE ientitymultipart = ((Lr)targetEntity).parent;
                  if (ientitymultipart instanceof Iw) {
                     entity = (Iw)ientitymultipart;
                  }
               }

               if (!itemstack1.isEmpty() && entity instanceof Iw) {
                  itemstack1.hitEntity((Iw)entity, this);
                  if (itemstack1.isEmpty()) {
                     this.setHeldItem(EnumHand.MAIN_HAND, Qy.EMPTY);
                  }
               }

               if (targetEntity instanceof Iw) {
                  float f5 = f4 - ((Iw)targetEntity).getHealth();
                  this.addStat(XV.DAMAGE_DEALT, Math.round(f5 * 10.0F));
                  if (j > 0) {
                     targetEntity.setFire(j * 4);
                  }

                  if (this.world instanceof bis && f5 > 2.0F) {
                     int k = (int)((double)f5 * 0.5);
                     ((bis)this.world).spawnParticle(EnumParticleTypes.DAMAGE_INDICATOR, targetEntity.posX, targetEntity.posY + (double)(targetEntity.height * 0.5F), targetEntity.posZ, k, 0.1, 0.0, 0.1, 0.2);
                  }
               }

               this.addExhaustion(0.1F);
            } else {
               this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_ATTACK_NODAMAGE, this.getSoundCategory(), 1.0F, 1.0F);
               if (flag4) {
   ...
   public void disableShield(boolean p_190777_1_) {
      float f = 0.25F + (float)Ft.getEfficiencyModifier(this) * 0.05F;
      if (p_190777_1_) {
         f += 0.75F;
      }

      if (this.rand.nextFloat() < f) {
         this.getCooldownTracker().setCooldown(NK.SHIELD, 100);
   ...
   public void spawnSweepParticles() {
      ...
      if (this.world instanceof bis) {
         ((bis)this.world).spawnParticle(EnumParticleTypes.SWEEP_ATTACK, this.posX + d0, this.posY + (double)this.height * 0.5, this.posZ + d1, 0, d0, 0.0, d1, 0.0);
      }
   ...
   public MD trySleep(BlockPos bedLocation) {
      EnumFacing enumfacing = (EnumFacing)this.world.getBlockState(bedLocation).getValue(en.FACING);
      if (!this.world.isRemote) {
         if (this.isPlayerSleeping() || !this.isEntityAlive()) {
            return MD.OTHER_PROBLEM;
         }

         if (!this.world.provider.isSurfaceWorld()) {
            return MD.NOT_POSSIBLE_HERE;
         }

         if (this.world.isDaytime()) {
            return MD.NOT_POSSIBLE_NOW;
         }

         if (!this.bedInRange(bedLocation, enumfacing)) {
            return MD.TOO_FAR_AWAY;
         }

         double d0 = 8.0;
         double d1 = 5.0;
         List<Kl> list = this.world.getEntitiesWithinAABB(Kl.class, new AxisAlignedBB((double)bedLocation.getX() - 8.0, (double)bedLocation.getY() - 5.0, (double)bedLocation.getZ() - 8.0, (double)bedLocation.getX() + 8.0, (double)bedLocation.getY() + 5.0, (double)bedLocation.getZ() + 8.0), new MC(this));
         if (!list.isEmpty()) {
            return MD.NOT_SAFE;
         }
      }
   ...
   public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn) {
      this.setSize(0.6F, 1.8F);
      in iblockstate = this.world.getBlockState(this.bedLocation);
      if (this.bedLocation != null && iblockstate.getBlock() == Nk.BED) {
         this.world.setBlockState(this.bedLocation, iblockstate.withProperty(cC.OCCUPIED, false), 4);
         BlockPos blockpos = cC.getSafeExitLocation(this.world, this.bedLocation, 0);
         if (blockpos == null) {
   ...
   private boolean isInBed() {
      return this.world.getBlockState(this.bedLocation).getBlock() == Nk.BED;
   }

   @Nullable
   public static BlockPos getBedSpawnLocation(bij worldIn, BlockPos bedLocation, boolean forceSpawn) {
      co block = worldIn.getBlockState(bedLocation).getBlock();
      if (block != Nk.BED) {
         if (!forceSpawn) {
            return null;
         } else {
         ...
         return cC.getSafeExitLocation(worldIn, bedLocation, 0);
      }
   }

   public float getBedOrientationInDegrees() {
      if (this.bedLocation != null) {
         EnumFacing enumfacing = (EnumFacing)this.world.getBlockState(this.bedLocation).getValue(en.FACING);
         switch (enumfacing) {
            case SOUTH:
               return 90.0F;
            case WEST:
   ...
   public void jump() {
      if (this == nC.player) {
         0bz.method_Qm().method_Qn().post(new 0dl());
      }

      super.jump();
      this.addStat(XV.JUMP);
      if (this.isSprinting()) {
   ...
   public float getAIMoveSpeed() {
      return (float)this.getEntityAttribute(Ni.MOVEMENT_SPEED).getAttributeValue();
   }
   ...
   public void addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
      if (!this.isRiding()) {
         ...
         if (this.isInsideOfMaterial(hM.WATER)) {
            ...
            if (k > 0) {
               this.addStat(XV.SWIM_ONE_CM, k);
               this.addExhaustion(0.01F * (float)k * 0.01F);
            }
         } else if (this.isOnLadder()) {
            if (p_71000_3_ > 0.0) {
               this.addStat(XV.CLIMB_ONE_CM, (int)Math.round(p_71000_3_ * 100.0));
            }
         } else if (this.onGround) {
            k = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0F);
            if (k > 0) {
               if (this.isSprinting()) {
                  this.addStat(XV.SPRINT_ONE_CM, k);
                  this.addExhaustion(0.1F * (float)k * 0.01F);
               } else if (this.isSneaking()) {
                  this.addStat(XV.CROUCH_ONE_CM, k);
                  this.addExhaustion(0.0F * (float)k * 0.01F);
               } else {
                  this.addStat(XV.WALK_ONE_CM, k);
                  this.addExhaustion(0.0F * (float)k * 0.01F);
               }
            }
         } else if (this.isElytraFlying()) {
            k = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_) * 100.0F);
            this.addStat(XV.AVIATE_ONE_CM, k);
         } else {
            k = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0F);
            if (k > 25) {
               this.addStat(XV.FLY_ONE_CM, k);
            }
         }
      }
   ...
   private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
      if (this.isRiding()) {
         ...
         if (i > 0) {
            if (this.getRidingEntity() instanceof Jc) {
               this.addStat(XV.MINECART_ONE_CM, i);
            } else if (this.getRidingEntity() instanceof IR) {
               this.addStat(XV.BOAT_ONE_CM, i);
            } else if (this.getRidingEntity() instanceof LQ) {
               this.addStat(XV.PIG_ONE_CM, i);
            } else if (this.getRidingEntity() instanceof Lw) {
               this.addStat(XV.HORSE_ONE_CM, i);
            }
         }
      }
   ...
   public void fall(float distance, float damageMultiplier) {
      if (!this.capabilities.allowFlying) {
         if (distance >= 2.0F) {
            this.addStat(XV.FALL_ONE_CM, (int)Math.round((double)distance * 100.0));
         }
   ...
   protected SoundEvent getFallSound(int heightIn) {
      return heightIn > 4 ? NO.ENTITY_PLAYER_BIG_FALL : NO.ENTITY_PLAYER_SMALL_FALL;
   }

   public void onKillEntity(Iw entityLivingIn) {
      Iq entitylist$entityegginfo = (Iq)Ir.ENTITY_EGGS.get(Ir.getKey((Ig)entityLivingIn));
   ...
   public void addExperienceLevel(int levels) {
      ...
      if (levels > 0 && this.experienceLevel % 5 == 0 && (float)this.lastXPSound < (float)this.ticksExisted - 100.0F) {
         ...
         this.world.playSound((ME)null, this.posX, this.posY, this.posZ, NO.ENTITY_PLAYER_LEVELUP, this.getSoundCategory(), f * 0.75F, 1.0F);
   ...
   public boolean canPlayerEdit(BlockPos pos, EnumFacing facing, Qy stack) {
   ...
   public Qy getItemStackFromSlot(EntityEquipmentSlot slotIn) {
      if (slotIn == EntityEquipmentSlot.MAINHAND) {
         ...
         return (Qy)this.inventory.offHandInventory.get(0);
      } else {
         return slotIn.getSlotType() == EntityEquipmentSlot.Type.ARMOR ? (Qy)this.inventory.armorInventory.get(slotIn.getIndex()) : Qy.EMPTY;
      }
   }
   ...
   public boolean addItemStackToInventory(Qy p_191521_1_) {
   ...
   public Iterable<Qy> getHeldEquipment() {
      return Lists.newArrayList(new Qy[]{this.getHeldItemMainhand(), this.getHeldItemOffhand()});
   }
   ...
   public boolean addShoulderEntity(QQ p_192027_1_) {
   ...
   protected void spawnShoulderEntities() {
      ...
      this.setLeftShoulderEntity(new QQ());
      this.spawnShoulderEntity(this.getRightShoulderEntity());
      this.setRightShoulderEntity(new QQ());
   }

   private void spawnShoulderEntity(@Nullable QQ p_192026_1_) {
      if (!this.world.isRemote && !p_192026_1_.isEmpty()) {
         Ig entity = Ir.createEntityFromNBT(p_192026_1_, this.world);
         if (entity instanceof Mg) {
            ((Mg)entity).setOwnerId(this.entityUniqueID);
         }
   ...
   public boolean isInvisibleToPlayer(ME player) {
      if (!this.isInvisible()) {
         return false;
   ...
   public boolean isPushedByWater() {
      if (0bz.method_Qm().method_Qs().method_bxY(0bT.class).method_bBh() && 0bT.field_b.method_bna()) {
         return false;
      } else {
         return !this.capabilities.isFlying;
      }
   ...
   public ITextComponent getDisplayName() {
      ITextComponent itextcomponent = new TextComponentString(WA.formatPlayerName(this.getTeam(), this.getName()));
   ...
   public boolean canOpen(bge code) {
      if (code.isEmpty()) {
         return true;
      } else {
         Qy itemstack = this.getHeldItemMainhand();
   ...
   public boolean isWearing(MH part) {
      ...
      if (inventorySlot >= 0 && inventorySlot < this.inventory.mainInventory.size()) {
         ...
         if (inventorySlot == 98) {
            ...
            if (!itemStackIn.isEmpty()) {
               if (!(itemStackIn.getItem() instanceof OR) && !(itemStackIn.getItem() instanceof Pt)) {
                  if (entityequipmentslot != EntityEquipmentSlot.HEAD) {
                     return false;
                  }
               } else if (Iu.getSlotForItemStack(itemStackIn) != entityequipmentslot) {
                  return false;
               }
            }
   ...
   public QQ getLeftShoulderEntity() {
      return (QQ)this.dataManager.get(LEFT_SHOULDER_ENTITY);
   }

   protected void setLeftShoulderEntity(QQ tag) {
      this.dataManager.set(LEFT_SHOULDER_ENTITY, tag);
   }

   public QQ getRightShoulderEntity() {
      return (QQ)this.dataManager.get(RIGHT_SHOULDER_ENTITY);
   }

   protected void setRightShoulderEntity(QQ tag) {
      this.dataManager.set(RIGHT_SHOULDER_ENTITY, tag);
   }

   public float getCooldownPeriod() {
      return (float)(1.0 / this.getEntityAttribute(Ni.ATTACK_SPEED).getAttributeValue() * 20.0);
   }
   ...
   public float getLuck() {
      return (float)this.getEntityAttribute(Ni.LUCK).getAttributeValue();
   }
   ...
   static {
      ABSORPTION = Rv.createKey(ME.class, Rt.FLOAT);
      PLAYER_SCORE = Rv.createKey(ME.class, Rt.VARINT);
      PLAYER_MODEL_FLAG = Rv.createKey(ME.class, Rt.BYTE);
      MAIN_HAND = Rv.createKey(ME.class, Rt.BYTE);
      LEFT_SHOULDER_ENTITY = Rv.createKey(ME.class, Rt.COMPOUND_TAG);
      RIGHT_SHOULDER_ENTITY = Rv.createKey(ME.class, Rt.COMPOUND_TAG);
   }
}
