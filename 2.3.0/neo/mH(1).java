...
public class mH implements kx {
   ...
   private final nC mc;
   private final pf server;
   private final ResourceLocation serverIcon;
   private String lastIconB64;
   private yP icon;
   private long lastClickTime;

   protected mH(kI ownerIn, pf serverIn) {
      this.owner = ownerIn;
      this.server = serverIn;
      this.mc = nC.getMinecraft();
      this.serverIcon = new ResourceLocation("servers/" + serverIn.serverIP + "/icon");
      this.icon = (yP)this.mc.getTextureManager().getTexture(this.serverIcon);
   }
   ...
   public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
      if (!this.server.pinged) {
         ...
         EXECUTOR.submit(new Runnable() {
            public void run() {
               try {
                  mH.this.owner.getOldServerPinger().ping(mH.this.server, false);
               } catch (UnknownHostException var2) {
                  mH.this.server.pingToServer = -1L;
                  mH.this.server.serverMOTD = TextFormatting.DARK_RED + Ax.format("multiplayer.status.cannot_resolve");
               } catch (Exception var3) {
                  mH.this.server.pingToServer = -1L;
                  mH.this.server.serverMOTD = TextFormatting.DARK_RED + Ax.format("multiplayer.status.cannot_connect");
               }
      ...
      if (flag2) {
         l = 5;
         s1 = Ax.format(flag ? "multiplayer.status.client_out_of_date" : "multiplayer.status.server_out_of_date");
         ...
         if (this.server.pingToServer < 0L) {
            s1 = Ax.format("multiplayer.status.no_connection");
         } else {
         ...
         k = 1;
         l = (int)(nC.getSystemTime() / 100L + (long)(slotIndex * 2) & 7L);
         if (l > 4) {
            l = 8 - l;
         }

         s1 = Ax.format("multiplayer.status.pinging");
      }

      yh.color(1.0F, 1.0F, 1.0F, 1.0F);
      this.mc.getTextureManager().bindTexture(jI.ICONS);
      jI.drawModalRectWithCustomSizedTexture(x + listWidth - 15, y, (float)(k * 10), (float)(176 + l * 8), 10, 8, 256.0F, 256.0F);
   ...
   protected void drawTextureAt(int p_178012_1_, int p_178012_2_, ResourceLocation p_178012_3_) {
      ...
      yh.enableBlend();
      jI.drawModalRectWithCustomSizedTexture(p_178012_1_, p_178012_2_, 0.0F, 0.0F, 32, 32, 32.0F, 32.0F);
      yh.disableBlend();
   }
      ...
      if (this.server.getBase64EncodedIconData() == null) {
         ...
         label99: {
            try {
               ...
               bufferedimage = zk.readBufferedImage(new ByteBufInputStream(bytebuf1));
   ...
   public boolean mousePressed(int slotIndex, int mouseX, int mouseY, int mouseEvent, int relativeX, int relativeY) {
      if (relativeX <= 32) {
         ...
         if (relativeX < 16 && relativeY < 16 && this.owner.canMoveUp(this, slotIndex)) {
            this.owner.moveServerUp(this, slotIndex, lg.isShiftKeyDown());
            return true;
         }

         if (relativeX < 16 && relativeY > 16 && this.owner.canMoveDown(this, slotIndex)) {
            this.owner.moveServerDown(this, slotIndex, lg.isShiftKeyDown());
            return true;
         }
      }
