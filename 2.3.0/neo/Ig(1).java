...
public abstract class Ig implements DB {
   private static final Logger LOGGER = LogManager.getLogger();
   private static final List<Qy> EMPTY_EQUIPMENT = Collections.emptyList();
   ...
   private final List<Ig> riddenByEntities;
   protected int rideCooldown;
   private Ig ridingEntity;
   ...
   protected Rv dataManager;
   protected static final Rd<Byte> FLAGS;
   private static final Rd<Integer> AIR;
   private static final Rd<String> CUSTOM_NAME;
   private static final Rd<Boolean> CUSTOM_NAME_VISIBLE;
   private static final Rd<Boolean> SILENT;
   private static final Rd<Boolean> NO_GRAVITY;
   ...
   private final CL cmdResultStats;
   ...
   public Rv getDataManager() {
      return this.dataManager;
   }

   public boolean equals(Object p_equals_1_) {
      if (p_equals_1_ instanceof Ig) {
         return ((Ig)p_equals_1_).entityId == this.entityId;
      } else {
         return false;
      }
   }
   ...
   protected void setSize(float width, float height) {
      if (width != this.width || height != this.height) {
         ...
         if (this.width > f && !this.firstUpdate && !this.world.isRemote) {
            this.move(Lq.SELF, (double)(f - this.width), 0.0, (double)(f - this.width));
         }
      }
   ...
   public void setFire(int seconds) {
      ...
      if (this instanceof Iw) {
         i = Fz.getFireTimeForEntity((Iw)this, i);
      }
   ...
   public void move(Lq type, double x, double y, double z) {
      if (this.noClip) {
         ...
         double d3 = y;
         double d4 = z;
         if ((type == Lq.SELF || type == Lq.PLAYER) && this.onGround && this.isSneaking() && this instanceof ME) {
         ...
         if (iblockstate.getMaterial() == hM.AIR) {
            ...
            if (block1 instanceof dL || block1 instanceof hz || block1 instanceof dM) {
         ...
         if (this.canTriggerWalking() && (!this.onGround || !this.isSneaking() || !(this instanceof ME)) && !this.isRiding()) {
            ...
            if (this.distanceWalkedOnStepModified > (float)this.nextStepDistance && iblockstate.getMaterial() != hM.AIR) {
               this.nextStepDistance = (int)this.distanceWalkedOnStepModified + 1;
               if (!this.isInWater()) {
                  this.playStepSound(blockpos, block);
               } else {
                  Ig entity = this.isBeingRidden() && this.getControllingPassenger() != null ? this.getControllingPassenger() : this;
            ...
            } else if (this.distanceWalkedOnStepModified > this.nextFlap && this.makeFlySound() && iblockstate.getMaterial() == hM.AIR) {
         ...
         try {
            ...
            Er crashreport = Er.makeCrashReport(var58, "Checking entity block collision");
            Ey crashreportcategory = crashreport.makeCategory("Entity being checked for collision");
         ...
         if (flag1 && this.isBurning()) {
            this.playSound(NO.ENTITY_GENERIC_EXTINGUISH_FIRE, 0.7F, 1.6F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F);
            this.fire = -this.getFireImmuneTicks();
         }
   ...
   protected SoundEvent getSwimSound() {
      return NO.ENTITY_GENERIC_SWIM;
   }

   protected SoundEvent getSplashSound() {
      return NO.ENTITY_GENERIC_SPLASH;
   }
         ...
         for(int i = blockpos$pooledmutableblockpos.getX(); i <= blockpos$pooledmutableblockpos1.getX(); ++i) {
            for(int j = blockpos$pooledmutableblockpos.getY(); j <= blockpos$pooledmutableblockpos1.getY(); ++j) {
               for(int k = blockpos$pooledmutableblockpos.getZ(); k <= blockpos$pooledmutableblockpos1.getZ(); ++k) {
                  ...
                  in iblockstate = this.world.getBlockState(blockpos$pooledmutableblockpos2);
                  ...
                  try {
                     ...
                     Er crashreport = Er.makeCrashReport(throwable, "Colliding entity with block");
                     Ey crashreportcategory = crashreport.makeCategory("Block being collided with");
                     Ey.addBlockInfo(crashreportcategory, blockpos$pooledmutableblockpos2, iblockstate);
                     throw new ReportedException(crashreport);
                  }
               }
            }
   ...
   public void playSound(SoundEvent soundIn, float volume, float pitch) {
      if (!this.isSilent()) {
         this.world.playSound((ME)null, this.posX, this.posY, this.posZ, soundIn, this.getSoundCategory(), volume, pitch);
      }
   ...
   public boolean isOverWater() {
      return this.world.handleMaterialAcceleration(this.getEntityBoundingBox().grow(0.0, -20.0, 0.0).shrink(0.001), hM.WATER, this);
   }

   public boolean handleWaterMovement() {
      if (this.getRidingEntity() instanceof IR) {
         this.inWater = false;
      } else if (this.world.handleMaterialAcceleration(this.getEntityBoundingBox().grow(0.0, -0.4000000059604645, 0.0).shrink(0.001), hM.WATER, this)) {
   ...
   public double getDistanceSqToEntity(Ig entityIn) {
   ...
   protected void createRunningParticles() {
      ...
      if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE) {
         this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5) * (double)this.width, this.getEntityBoundingBox().minY + 0.1, this.posZ + ((double)this.rand.nextFloat() - 0.5) * (double)this.width, -this.motionX * 4.0, 1.5, -this.motionZ * 4.0, co.getStateId(iblockstate));
      }

   }

   public boolean isInsideOfMaterial(hM materialIn) {
      if (this.getRidingEntity() instanceof IR) {
         return false;
      } else {
         ...
         if (iblockstate.getMaterial() == materialIn) {
            float f = eB.getLiquidHeightPercent(iblockstate.getBlock().getMetaFromState(iblockstate)) - 0.11111111F;
            float f1 = (float)(blockpos.getY() + 1) - f;
            boolean flag = d0 < (double)f1;
            return !flag && this instanceof ME ? false : flag;
         } else {
            return false;
         }
      }
   ...
   public boolean isInLava() {
      return this.world.isMaterialInBB(this.getEntityBoundingBox().grow(-0.10000000149011612, -0.4000000059604645, -0.10000000149011612), hM.LAVA);
   }
   ...
   public float getDistance(Ig entityIn) {
   ...
   public double getDistanceSq(Ig entityIn) {
      double d0 = this.posX - entityIn.posX;
      double d1 = this.posY - entityIn.posY;
      double d2 = this.posZ - entityIn.posZ;
      return d0 * d0 + d1 * d1 + d2 * d2;
   ...
   public void awardKillScore(Ig p_191956_1_, int p_191956_2_, DamageSource p_191956_3_) {
      if (p_191956_1_ instanceof MG) {
         bY.ENTITY_KILLED_PLAYER.trigger((MG)p_191956_1_, this, p_191956_3_);
      }
   ...
   public boolean writeToNBTAtomically(QQ compound) {
   ...
   public QQ writeToNBT(QQ compound) {
      try {
         ...
         if (this.isBeingRidden()) {
            nbttaglist1 = new QW();
            var8 = this.getPassengers().iterator();
         ...
         Er crashreport = Er.makeCrashReport(throwable, "Saving entity NBT");
         Ey crashreportcategory = crashreport.makeCategory("Entity being saved");
         this.addEntityCrashInfo(crashreportcategory);
         throw new ReportedException(crashreport);
      }
   }
   ...
   public void readFromNBT(QQ compound) {
      try {
         QW nbttaglist = compound.getTagList("Pos", 6);
         QW nbttaglist2 = compound.getTagList("Motion", 6);
         QW nbttaglist3 = compound.getTagList("Rotation", 5);
         ...
         if (compound.hasKey("Tags", 9)) {
            this.tags.clear();
            QW nbttaglist1 = compound.getTagList("Tags", 8);
         ...
         Er crashreport = Er.makeCrashReport(throwable, "Loading entity NBT");
         Ey crashreportcategory = crashreport.makeCategory("Entity being loaded");
         this.addEntityCrashInfo(crashreportcategory);
         throw new ReportedException(crashreport);
      }
   }
   ...
   protected final String getEntityString() {
      ResourceLocation resourcelocation = Ir.getKey(this);
      return resourcelocation == null ? null : resourcelocation.toString();
   }

   protected abstract void readEntityFromNBT(QQ var1);

   protected abstract void writeEntityToNBT(QQ var1);

   protected QW newDoubleNBTList(double... numbers) {
      ...
      for(int var5 = 0; var5 < var4; ++var5) {
         double d0 = var3[var5];
         nbttaglist.appendTag(new QR(d0));
      }

      return nbttaglist;
   }

   protected QW newFloatNBTList(float... numbers) {
      QW nbttaglist = new QW();
      float[] var3 = numbers;
      int var4 = numbers.length;
   ...
   @Nullable
   public IY dropItem(OL itemIn, int size) {
      return this.dropItemWithOffset(itemIn, size, 0.0F);
   }

   @Nullable
   public IY dropItemWithOffset(OL itemIn, int size, float offsetY) {
      return this.entityDropItem(new Qy(itemIn, size, 0), offsetY);
   }
   ...
   public IY entityDropItem(Qy stack, float offsetY) {
      if (stack.isEmpty()) {
         ...
         IY entityitem = new IY(this.world, this.posX, this.posY + (double)offsetY, this.posZ, stack);
   ...
   public boolean isEntityInsideOpaqueBlock() {
      if (this.noClip) {
         return false;
      } else if (0bT.field_c.method_bna()) {
         return false;
      } else {
   ...
   public boolean startRiding(Ig entityIn) {
      return this.startRiding(entityIn, false);
   }

   public boolean startRiding(Ig entityIn, boolean force) {
      for(Ig entity = entityIn; entity.ridingEntity != null; entity = entity.ridingEntity) {
   ...
   public void removePassengers() {
      for(int i = this.riddenByEntities.size() - 1; i >= 0; --i) {
         ((Ig)this.riddenByEntities.get(i)).dismountRidingEntity();
      }
   ...
   protected void addPassenger(Ig passenger) {
      if (passenger.getRidingEntity() != this) {
         throw new IllegalStateException("Use x.startRiding(y), not y.addPassenger(x)");
      } else {
         if (!this.world.isRemote && passenger instanceof ME && !(this.getControllingPassenger() instanceof ME)) {
   ...
   protected void removePassenger(Ig passenger) {
      if (passenger.getRidingEntity() == this) {
   ...
   public void setPortal(BlockPos pos) {
      if (this.timeUntilPortal > 0) {
         ...
         if (!this.world.isRemote && !pos.equals(this.lastPortalPos)) {
            ...
            is blockpattern$patternhelper = Nk.PORTAL.createPatternHelper(this.world, this.lastPortalPos);
   ...
   public Iterable<Qy> getEquipmentAndArmor() {
   ...
   public boolean isOnScoreboardTeam(WE teamIn) {
   ...
   public String getName() {
      if (this.hasCustomName()) {
         ...
         String s = Ir.getEntityString(this);
         if (s == null) {
            s = "generic";
         }
   ...
   public void copyLocationAndAnglesFrom(Ig entityIn) {
      this.setLocationAndAngles(entityIn.posX, entityIn.posY, entityIn.posZ, entityIn.rotationYaw, entityIn.rotationPitch);
   }

   private void copyDataFromOld(Ig entityIn) {
      QQ nbttagcompound = entityIn.writeToNBT(new QQ());
   ...
   public Ig changeDimension(int dimensionIn) {
      if (!this.world.isRemote && !this.isDead) {
         ...
         bis worldserver = minecraftserver.getWorld(i);
         bis worldserver1 = minecraftserver.getWorld(dimensionIn);
         ...
         if (dimensionIn == 1) {
            ...
            bid teleporter = worldserver1.getDefaultTeleporter();
         ...
         Ig entity = Ir.newEntity(this.getClass(), worldserver1);
         if (entity != null) {
   ...
   public void addEntityCrashInfo(Ey category) {
      category.addDetail("Entity Type", new Ez<String>() {
         public String call() throws Exception {
            return Ir.getKey(Ig.this) + " (" + Ig.this.getClass().getCanonicalName() + ")";
         }
      ...
      category.addDetail("Entity Name", new Ez<String>() {
         public String call() throws Exception {
            return Ig.this.getName();
         }
      ...
      category.addCrashSection("Entity's Block location", Ey.getCoordinateInfo(MathHelper.floor(this.posX), MathHelper.floor(this.posY), MathHelper.floor(this.posZ)));
      category.addCrashSection("Entity's Momentum", String.format("%.2f, %.2f, %.2f", this.motionX, this.motionY, this.motionZ));
      category.addDetail("Entity's Passengers", new Ez<String>() {
         public String call() throws Exception {
            return Ig.this.getPassengers().toString();
         }
      ...
      category.addDetail("Entity's Vehicle", new Ez<String>() {
         public String call() throws Exception {
            return Ig.this.getRidingEntity().toString();
         }
   ...
   public ITextComponent getDisplayName() {
      TextComponentString textcomponentstring = new TextComponentString(WA.formatPlayerName(this.getTeam(), this.getName()));
   ...
   public CL getCommandStats() {
      return this.cmdResultStats;
   }

   public void setCommandStats(Ig entityIn) {
      this.cmdResultStats.addAllStats(entityIn.getCommandStats());
   }

   public EnumActionResult applyPlayerInteraction(ME player, Vec3d vec, EnumHand hand) {
   ...
   protected void applyEnchantments(Iw entityLivingBaseIn, Ig entityIn) {
      if (entityIn instanceof Iw) {
         Ft.applyThornEnchantments((Iw)entityIn, entityLivingBaseIn);
      }

      Ft.applyArthropodEnchantments(entityLivingBaseIn, entityIn);
   }
   ...
   public Collection<Ig> getRecursivePassengers() {
      Set<Ig> set = Sets.newHashSet();
      this.getRecursivePassengersByType(Ig.class, set);
      return set;
   }

   public <T extends Ig> Collection<T> getRecursivePassengersByType(Class<T> entityClass) {
      Set<T> set = Sets.newHashSet();
      this.getRecursivePassengersByType(entityClass, set);
      return set;
   }

   private <T extends Ig> void getRecursivePassengersByType(Class<T> entityClass, Set<T> theSet) {
      Ig entity;
   ...
   public Ig getLowestRidingEntity() {
      Ig entity;
   ...
   public boolean isRidingSameEntity(Ig entityIn) {
      return this.getLowestRidingEntity() == entityIn.getLowestRidingEntity();
   }

   public boolean isRidingOrBeingRiddenBy(Ig entityIn) {
      Iterator var2 = this.getPassengers().iterator();
   ...
   static {
      FLAGS = Rv.createKey(Ig.class, Rt.BYTE);
      AIR = Rv.createKey(Ig.class, Rt.VARINT);
      CUSTOM_NAME = Rv.createKey(Ig.class, Rt.STRING);
      CUSTOM_NAME_VISIBLE = Rv.createKey(Ig.class, Rt.BOOLEAN);
      SILENT = Rv.createKey(Ig.class, Rt.BOOLEAN);
      NO_GRAVITY = Rv.createKey(Ig.class, Rt.BOOLEAN);
   }
}
