...
public class baX {
   ...
   private final Ig exploder;
   private final float size;
   private final List<BlockPos> affectedBlockPositions;
   private final Map<ME, Vec3d> playerKnockbackMap;

   public baX(bij worldIn, Ig entityIn, double x, double y, double z, float size, List<BlockPos> affectedPositions) {
      this(worldIn, entityIn, x, y, z, size, false, true, affectedPositions);
   }

   public baX(bij worldIn, Ig entityIn, double x, double y, double z, float size, boolean causesFire, boolean damagesTerrain, List<BlockPos> affectedPositions) {
      this(worldIn, entityIn, x, y, z, size, causesFire, damagesTerrain);
      this.affectedBlockPositions.addAll(affectedPositions);
   }

   public baX(bij worldIn, Ig entityIn, double x, double y, double z, float size, boolean causesFire, boolean damagesTerrain) {
   ...
   public void doExplosionA() {
               ...
               if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
                  ...
                  for(float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
                     ...
                     if (iblockstate.getMaterial() != hM.AIR) {
                        float f2 = this.exploder != null ? this.exploder.getExplosionResistance(this, this.world, blockpos, iblockstate) : iblockstate.getBlock().getExplosionResistance((Ig)null);
                        f -= (f2 + 0.3F) * 0.3F;
                     }
      ...
      k = MathHelper.floor(this.x - (double)f3 - 1.0);
      l = MathHelper.floor(this.x + (double)f3 + 1.0);
      ...
      List<Ig> list = this.world.getEntitiesWithinAABBExcludingEntity(this.exploder, new AxisAlignedBB((double)k, (double)i2, (double)j2, (double)l, (double)i1, (double)j1));
      Vec3d vec3d = new Vec3d(this.x, this.y, this.z);

      for(int k2 = 0; k2 < list.size(); ++k2) {
         Ig entity = (Ig)list.get(k2);
            ...
            if (d12 <= 1.0) {
               ...
               if (d13 != 0.0) {
                  ...
                  if (entity instanceof Iw) {
                     d11 = Fz.getBlastDamageReduction((Iw)entity, d10);
                  }
   ...
   public void doExplosionB(boolean spawnParticles) {
      this.world.playSound((ME)null, this.x, this.y, this.z, NO.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F, (1.0F + (this.world.rand.nextFloat() - this.world.rand.nextFloat()) * 0.2F) * 0.7F);
      ...
      if (this.damagesTerrain) {
         ...
         while(var2.hasNext()) {
            ...
            if (spawnParticles) {
               ...
               d4 *= d7;
               d5 *= d7;
               this.world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, (d0 + this.x) / 2.0, (d1 + this.y) / 2.0, (d2 + this.z) / 2.0, d3, d4, d5);
            ...
            if (iblockstate.getMaterial() != hM.AIR) {
               ...
               this.world.setBlockState(blockpos, Nk.AIR.getDefaultState(), 3);
      ...
      if (this.causesFire) {
         ...
         while(var2.hasNext()) {
            blockpos = (BlockPos)var2.next();
            if (this.world.getBlockState(blockpos).getMaterial() == hM.AIR && this.world.getBlockState(blockpos.down()).isFullBlock() && this.random.nextInt(3) == 0) {
   ...
   public Map<ME, Vec3d> getPlayerKnockbackMap() {
      return this.playerKnockbackMap;
   }

   @Nullable
   public Iw getExplosivePlacedBy() {
      if (this.exploder == null) {
         return null;
      } else if (this.exploder instanceof Jr) {
         return ((Jr)this.exploder).getTntPlacedBy();
      } else {
         return this.exploder instanceof Iw ? (Iw)this.exploder : null;
      }
   }
