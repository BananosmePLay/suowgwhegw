...
public class bqN {
   ...
   public static String[] collectFiles(AC rp, String prefix, String suffix, String[] defaultPaths) {
      return collectFiles(rp, new String[]{prefix}, new String[]{suffix}, defaultPaths);
   }

   public static String[] collectFiles(AC rp, String[] prefixes, String[] suffixes) {
      return collectFiles(rp, prefixes, suffixes, (String[])null);
   }

   public static String[] collectFiles(AC rp, String[] prefixes, String[] suffixes, String[] defaultPaths) {
      if (rp instanceof Aq) {
         return collectFilesFixed(rp, defaultPaths);
      } else {
         if (rp instanceof AG) {
            AC iresourcepack = (AC)bnK.getFieldValue(rp, bnK.LegacyV2Adapter_pack);
            if (iresourcepack == null) {
               XH.warn("LegacyV2Adapter base resource pack not found: " + rp);
               return new String[0];
            }
         ...
         if (!(rp instanceof zW)) {
            XH.warn("Unknown resource pack type: " + rp);
            return new String[0];
         } else {
            zW abstractresourcepack = (zW)rp;
            ...
            if (file1 == null) {
               ...
               XH.warn("Unknown resource pack file: " + file1);
               return new String[0];
            }
         }
      }
   ...
   private static String[] collectFilesFixed(AC rp, String[] paths) {
      if (paths == null) {
         return new String[0];
      } else {
         ...
         for(int i = 0; i < paths.length; ++i) {
            String s = paths[i];
            if (!isLowercase(s)) {
               XH.warn("Skipping non-lowercase path: " + s);
            } else {
         ...
         for(int i = 0; i < afile.length; ++i) {
            ...
            if (file1.isFile()) {
               ...
               if (s3.startsWith(s)) {
                  ...
                  if (bqP.startsWith(s3, prefixes) && bqP.endsWith(s3, suffixes)) {
                     if (!isLowercase(s3)) {
                        XH.warn("Skipping non-lowercase path: " + s3);
                     } else {
                        list.add(s3);
                     }
                  }
      ...
      try {
         ...
         while(enumeration.hasMoreElements()) {
            ...
            if (s1.startsWith(s)) {
               ...
               if (bqP.startsWith(s1, prefixes) && bqP.endsWith(s1, suffixes)) {
                  if (!isLowercase(s1)) {
                     XH.warn("Skipping non-lowercase path: " + s1);
                  } else {
                     list.add(s1);
                  }
               }
   ...
   public static Properties readProperties(String path, String module) {
      ...
      try {
         ...
         if (inputstream == null) {
            ...
            inputstream.close();
            XH.dbg("" + module + ": Loading " + path);
            return properties;
         }
      } catch (FileNotFoundException var5) {
         return null;
      } catch (IOException var6) {
         XH.warn("" + module + ": Error reading " + path);
         return null;
      }
   }
