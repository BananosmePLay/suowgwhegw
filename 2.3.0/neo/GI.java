...
public class GI extends Gi {
   private final Ly animal;
   private final Class<? extends Ly> mateClass;
   bij world;
   private Ly targetMate;
   int spawnBabyDelay;
   double moveSpeed;

   public GI(Ly animal, double speedIn) {
      this(animal, speedIn, animal.getClass());
   }

   public GI(Ly p_i47306_1_, double p_i47306_2_, Class<? extends Ly> p_i47306_4_) {
   ...
   private Ly getNearbyMate() {
      List<Ly> list = this.world.getEntitiesWithinAABB(this.mateClass, this.animal.getEntityBoundingBox().grow(8.0));
   ...
   private void spawnBaby() {
      Ih entityageable = this.animal.createChild(this.targetMate);
      if (entityageable != null) {
         MG entityplayermp = this.animal.getLoveCause();
         if (entityplayermp == null && this.targetMate.getLoveCause() != null) {
            entityplayermp = this.targetMate.getLoveCause();
         }

         if (entityplayermp != null) {
            entityplayermp.addStat(XV.ANIMALS_BRED);
            bY.BRED_ANIMALS.trigger(entityplayermp, this.animal, this.targetMate, entityageable);
         }
         ...
         if (this.world.getGameRules().getBoolean("doMobLoot")) {
            this.world.spawnEntity(new Js(this.world, this.animal.posX, this.animal.posY, this.animal.posZ, random.nextInt(7) + 1));
         }
      }
