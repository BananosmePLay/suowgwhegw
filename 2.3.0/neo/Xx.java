...
public abstract class Xx implements DB, Runnable, IThreadListener, Wh {
   private static final Logger LOGGER = LogManager.getLogger();
   public static final File USER_CACHE_FILE = new File("usercache.json");
   private final bgl anvilConverterForAnvilFile;
   private final Wm usageSnooper = new Wm("server", this, getCurrentTimeMillis());
   private final File anvilFile;
   private final List<ITickable> tickables = Lists.newArrayList();
   public final DA commandManager;
   public final Wk profiler = new Wk();
   private final Sx networkSystem;
   private final Vu statusResponse = new Vu();
   private final Random random = new Random();
   private final DataFixer dataFixer;
   private int serverPort = -1;
   public bis[] worlds;
   private WY playerList;
   ...
   public Xx(File anvilFileIn, Proxy proxyIn, DataFixer dataFixerIn, YggdrasilAuthenticationService authServiceIn, MinecraftSessionService sessionServiceIn, GameProfileRepository profileRepoIn, Xd profileCacheIn) {
      ...
      this.networkSystem = new Sx(this);
      this.commandManager = this.createCommandManager();
      this.anvilConverterForAnvilFile = new bax(anvilFileIn, dataFixerIn);
         ...
         this.getActiveAnvilConverter().convertMapFormat(worldNameIn, new IProgressUpdate() {
            ...
            public void setLoadingProgress(int progress) {
               if (System.currentTimeMillis() - this.startTime >= 1000L) {
                  ...
                  Xx.LOGGER.info("Converting... {}%", progress);
               }
   ...
   public void loadAllWorlds(String saveName, String worldNameIn, long seed, bix type, String generatorOptions) {
      ...
      bgm isavehandler = this.anvilConverterForAnvilFile.getSaveLoader(saveName, true);
      this.setResourcePackFromWorld(this.getFolderName(), isavehandler);
      bhY worldinfo = isavehandler.loadWorldInfo();
      biw worldsettings;
      if (worldinfo == null) {
         if (this.isDemo()) {
            worldsettings = bit.DEMO_WORLD_SETTINGS;
         } else {
            worldsettings = new biw(seed, this.getGameType(), this.canStructuresSpawn(), this.isHardcore(), type);
         ...
         worldinfo = new bhY(worldsettings, worldNameIn);
      } else {
         worldinfo.setWorldName(worldNameIn);
         worldsettings = new biw(worldinfo);
      }
      ...
      for(int i = 0; i < this.worlds.length; ++i) {
         ...
         if (i == 0) {
            if (this.isDemo()) {
               this.worlds[i] = (bis)(new bit(this, isavehandler, worldinfo, j, this.profiler)).init();
            } else {
               this.worlds[i] = (bis)(new bis(this, isavehandler, worldinfo, j, this.profiler)).init();
            }

            this.worlds[i].initialize(worldsettings);
         } else {
            this.worlds[i] = (bis)(new biv(this, isavehandler, j, this.worlds[0], this.profiler)).init();
         }

         this.worlds[i].addEventListener(new bgi(this, this.worlds[i]));
   ...
   public void setResourcePackFromWorld(String worldNameIn, bgm saveHandlerIn) {
   ...
   public abstract bbb getGameType();

   public abstract baV getDifficulty();
   ...
   public void run() {
      try {
         if (this.init()) {
            ...
            this.statusResponse.setVersion(new Vt("1.12.2", 340));
            ...
            this.finalTick((Er)null);
         }
      } catch (Throwable var46) {
         Throwable throwable1 = var46;
         LOGGER.error("Encountered an unexpected exception", throwable1);
         Er crashreport = null;
         if (throwable1 instanceof ReportedException) {
            crashreport = this.addServerInfoToCrashReport(((ReportedException)throwable1).getCrashReport());
         } else {
            crashreport = this.addServerInfoToCrashReport(new Er("Exception in server tick loop", throwable1));
         }
   ...
   public void applyServerIconToResponse(Vu response) {
   ...
   public void tick() {
      ...
      if (i - this.nanoTimeSinceStatusRefresh >= 5000000000L) {
         ...
         this.statusResponse.setPlayers(new Vq(this.getMaxPlayers(), this.getCurrentPlayerCount()));
         GameProfile[] agameprofile = new GameProfile[Math.min(this.getCurrentPlayerCount(), 12)];
         int j = MathHelper.getInt((Random)this.random, 0, this.getCurrentPlayerCount() - agameprofile.length);

         for(int k = 0; k < agameprofile.length; ++k) {
            agameprofile[k] = ((MG)this.playerList.getPlayers().get(j + k)).getGameProfile();
         }
   ...
   public void updateTimeLightAndEntities() {
      ...
      int k;
      for(k = 0; k < this.worlds.length; ++k) {
         long i = System.nanoTime();
         if (k == 0 || this.getAllowNether()) {
            bis worldserver = this.worlds[k];
            this.profiler.func_194340_a(() -> {
               return worldserver.getWorldInfo().getWorldName();
            });
            if (this.tickCounter % 20 == 0) {
               this.profiler.startSection("timeSync");
               this.playerList.sendPacketToAllPlayersInDimension(new UW(worldserver.getTotalWorldTime(), worldserver.getWorldTime(), worldserver.getGameRules().getBoolean("doDaylightCycle")), worldserver.provider.getDimensionType().getId());
               this.profiler.endSection();
            }
            ...
            try {
               ...
               throwable = var8;
               crashreport1 = Er.makeCrashReport(throwable, "Exception ticking world");
            ...
            try {
               ...
               throwable = var7;
               crashreport1 = Er.makeCrashReport(throwable, "Exception ticking world entities");
               worldserver.addWorldInfoToCrashReport(crashreport1);
               throw new ReportedException(crashreport1);
            }
   ...
   public bis getWorld(int dimension) {
      if (dimension == -1) {
   ...
   public Er addServerInfoToCrashReport(Er report) {
      report.getCategory().addDetail("Profiler Position", new Ez<String>() {
         public String call() throws Exception {
            return Xx.this.profiler.profilingEnabled ? Xx.this.profiler.getNameOfLastSection() : "N/A (disabled)";
         }
      ...
      if (this.playerList != null) {
         report.getCategory().addDetail("Player Count", new Ez<String>() {
            public String call() {
               return Xx.this.playerList.getCurrentPlayerCount() + " / " + Xx.this.playerList.getMaxPlayers() + "; " + Xx.this.playerList.getPlayers();
            }
   ...
   public List<String> getTabCompletions(DB sender, String input, @Nullable BlockPos pos, boolean hasTargetBlock) {
      ...
      if (!flag && !hasTargetBlock) {
         ...
         String[] var15 = this.playerList.getOnlinePlayerNames();
         int var16 = var15.length;

         for(int var11 = 0; var11 < var16; ++var11) {
            String s1 = var15[var11];
            if (Ch.doesStringStartWith(s2, s1)) {
               list.add(s1);
            }
         }
   ...
   public void setDifficultyForAllWorlds(baV difficulty) {
      ...
      for(int var4 = 0; var4 < var3; ++var4) {
         bis worldserver1 = var2[var4];
         if (worldserver1 != null) {
            if (worldserver1.getWorldInfo().isHardcoreModeEnabled()) {
               worldserver1.getWorldInfo().setDifficulty(baV.HARD);
               worldserver1.setAllowedSpawnTypes(true, true);
            } else if (this.isSinglePlayer()) {
               worldserver1.getWorldInfo().setDifficulty(difficulty);
               worldserver1.setAllowedSpawnTypes(worldserver1.getDifficulty() != baV.PEACEFUL, true);
            } else {
               worldserver1.getWorldInfo().setDifficulty(difficulty);
   ...
   public bgl getActiveAnvilConverter() {
   ...
   public void addServerStatsToSnooper(Wm playerSnooper) {
      ...
      int l = 0;
      if (this.worlds != null) {
         bis[] var3 = this.worlds;
         int var4 = var3.length;

         for(int var5 = 0; var5 < var4; ++var5) {
            bis worldserver1 = var3[var5];
            if (worldserver1 != null) {
               bhY worldinfo = worldserver1.getWorldInfo();
   ...
   public WY getPlayerList() {
      return this.playerList;
   }

   public void setPlayerList(WY list) {
   ...
   public abstract String shareToLAN(bbb var1, boolean var2);
   ...
   public boolean isBlockProtected(bij worldIn, BlockPos pos, ME playerIn) {
      return false;
   }
   ...
   public Xd getPlayerProfileCache() {
      return this.profileCache;
   }

   public Vu getServerStatusResponse() {
   ...
   @Nullable
   public Ig getEntityFromUuid(UUID uuid) {
      bis[] var2 = this.worlds;
      int var3 = var2.length;
      ...
      for(int var4 = 0; var4 < var3; ++var4) {
         bis worldserver1 = var2[var4];
         if (worldserver1 != null) {
            Ig entity = worldserver1.getEntityFromUuid(uuid);
            if (entity != null) {
               return entity;
            }
         }
   ...
   public int getSpawnRadius(@Nullable bis worldIn) {
      return worldIn != null ? worldIn.getGameRules().getInt("spawnRadius") : 10;
   }

   public f getAdvancementManager() {
      return this.worlds[0].getAdvancementManager();
   }

   public cf getFunctionManager() {
