...
public class VK {
   private VR[] pathPoints = new VR[128];
   private int count;

   public VK() {
   }

   public VR addPoint(VR point) {
      if (point.index >= 0) {
         throw new IllegalStateException("OW KNOWS!");
      } else {
         if (this.count == this.pathPoints.length) {
            VR[] apathpoint = new VR[this.count << 1];
   ...
   public VR dequeue() {
      VR pathpoint = this.pathPoints[0];
   ...
   public void changeDistance(VR point, float distance) {
   ...
   private void sortBack(int index) {
      VR pathpoint = this.pathPoints[index];

      int i;
      for(float f = pathpoint.distanceToTarget; index > 0; index = i) {
         i = index - 1 >> 1;
         VR pathpoint1 = this.pathPoints[i];
