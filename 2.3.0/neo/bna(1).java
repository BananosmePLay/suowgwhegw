...
public class bna {
   ...
   public bna(String iconName, int width, int height, int[] data, boolean direct) {
   ...
   public static Dimension[] makeMipmapDimensions(int width, int height, String iconName) {
      int i = bqS.ceilPowerOfTwo(width);
      int j = bqS.ceilPowerOfTwo(height);
      if (i == width && j == height) {
         ...
         XH.warn("Mipmaps not possible (power of 2 dimensions needed), texture: " + iconName + ", dim: " + width + "x" + height);
   ...
   public static IntBuffer[] makeMipmapBuffers(Dimension[] mipmapDimensions, int[][] mipmapDatas) {
      if (mipmapDimensions == null) {
         ...
         for(int i = 0; i < mipmapDimensions.length; ++i) {
            ...
            IntBuffer intbuffer = xE.createDirectIntBuffer(j);
