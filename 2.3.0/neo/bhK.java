...
public class bhK implements bgl {
   private static final Logger LOGGER = LogManager.getLogger();
   protected final File savesDirectory;
   protected final DataFixer dataFixer;

   public bhK(File savesDirectoryIn, DataFixer dataFixerIn) {
   ...
   public List<bib> getSaveList() throws iy {
      ...
      for(int i = 0; i < 5; ++i) {
         ...
         if (worldinfo != null) {
            list.add(new bib(worldinfo, s, "", worldinfo.getSizeOnDisk(), false));
         }
      }
   ...
   @Nullable
   public bhY getWorldInfo(String saveName) {
      ...
      if (!file1.exists()) {
         ...
         if (file2.exists()) {
            bhY worldinfo = getWorldData(file2, this.dataFixer);
            if (worldinfo != null) {
               return worldinfo;
            }
         }
   ...
   @Nullable
   public static bhY getWorldData(File p_186353_0_, DataFixer dataFixerIn) {
      try {
         QQ nbttagcompound = QF.readCompressed(new FileInputStream(p_186353_0_));
         QQ nbttagcompound1 = nbttagcompound.getCompoundTag("Data");
         return new bhY(dataFixerIn.process(FixTypes.LEVEL, nbttagcompound1));
   ...
   public void renameWorld(String dirName, String newName) {
      ...
      if (file1.exists()) {
         ...
         if (file2.exists()) {
            try {
               QQ nbttagcompound = QF.readCompressed(new FileInputStream(file2));
               QQ nbttagcompound1 = nbttagcompound.getCompoundTag("Data");
               nbttagcompound1.setString("LevelName", newName);
               QF.writeCompressed(nbttagcompound, new FileOutputStream(file2));
   ...
   public bgm getSaveLoader(String saveName, boolean storePlayerdata) {
      return new bhL(this.savesDirectory, saveName, storePlayerdata, this.dataFixer);
   }
