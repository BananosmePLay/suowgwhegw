...
public class bkQ extends bkn {
   ...
   public bkQ() {
      super(LF.class, "horse", 0.75F);
   }

   protected bkQ(Class entityClass, String name, float shadowSize) {
   ...
   public ow getModelRenderer(nH model, String modelPart) {
      if (!(model instanceof oe)) {
         return null;
      } else {
         oe modelhorse = (oe)model;
         Map<String, Integer> map = getMapPartFields();
         if (map.containsKey(modelPart)) {
            int i = (Integer)map.get(modelPart);
            return (ow)bnK.getFieldValue(modelhorse, bnK.ModelHorse_ModelRenderers, i);
         } else {
            return null;
         }
      }
   ...
   public bkm makeEntityRender(nH modelBase, float shadowSize) {
      ...
      wo renderhorse = new wo(rendermanager);
