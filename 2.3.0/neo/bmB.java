...
public class bmB implements bmz {
   public bmB() {
   }
   ...
   public String[] getTooltipLines(jK btn, int width) {
      if (!(btn instanceof bmx)) {
         return null;
      } else {
         bmx ioptioncontrol = (bmx)btn;
         Bi gamesettings$options = ioptioncontrol.getOption();
   ...
   public static String[] getTooltipLines(String key) {
      ...
      for(int i = 0; i < 10; ++i) {
         ...
         String s1 = bmW.get(s, (String)null);
         if (s1 == null) {
            break;
         }
