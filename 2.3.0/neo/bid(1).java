...
public class bid {
   ...
   private final Long2ObjectMap<bic> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);

   public bid(bis worldIn) {
      this.world = worldIn;
      this.random = new Random(worldIn.getSeed());
   }

   public void placeInPortal(Ig entityIn, float rotationYaw) {
         ...
         for(int j1 = -2; j1 <= 2; ++j1) {
            for(int k1 = -2; k1 <= 2; ++k1) {
               for(int l1 = -1; l1 < 3; ++l1) {
                  ...
                  int k2 = k + k1 * 0 - j1 * 1;
                  boolean flag = l1 < 0;
                  this.world.setBlockState(new BlockPos(i2, j2, k2), flag ? Nk.OBSIDIAN.getDefaultState() : Nk.AIR.getDefaultState());
               }
            }
         }
   ...
   public boolean placeInExistingPortal(Ig entityIn, float rotationYaw) {
      int i = true;
      double d0 = -1.0;
      ...
      if (this.destinationCoordinateCache.containsKey(l)) {
         bic teleporter$portalposition = (bic)this.destinationCoordinateCache.get(l);
         d0 = 0.0;
         ...
         for(int i1 = -128; i1 <= 128; ++i1) {
            ...
            for(int j1 = -128; j1 <= 128; ++j1) {
               for(BlockPos blockpos1 = blockpos3.add(i1, this.world.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
                  ...
                  if (this.world.getBlockState(blockpos1).getBlock() == Nk.PORTAL) {
                     for(blockpos2 = blockpos1.down(); this.world.getBlockState(blockpos2).getBlock() == Nk.PORTAL; blockpos2 = blockpos2.down()) {
                        blockpos1 = blockpos2;
                     }
      ...
      if (d0 >= 0.0) {
         if (flag) {
            this.destinationCoordinateCache.put(l, new bic(this, (BlockPos)blockpos, this.world.getTotalWorldTime()));
         }

         double d5 = (double)((BlockPos)blockpos).getX() + 0.5;
         double d7 = (double)((BlockPos)blockpos).getZ() + 0.5;
         is blockpattern$patternhelper = Nk.PORTAL.createPatternHelper(this.world, (BlockPos)blockpos);
         ...
         if (entityIn instanceof MG) {
            ((MG)entityIn).connection.setPlayerLocation(d5, d6, d7, entityIn.rotationYaw, entityIn.rotationPitch);
         } else {
   ...
   public boolean makePortal(Ig entityIn) {
      int i = true;
      double d0 = -1.0;
      int j = MathHelper.floor(entityIn.posX);
      ...
      for(l5 = j - 16; l5 <= j + 16; ++l5) {
         d3 = (double)l5 + 0.5 - entityIn.posX;

         for(j6 = l - 16; j6 <= l + 16; ++j6) {
            d4 = (double)j6 + 0.5 - entityIn.posZ;
            ...
            for(i7 = this.world.getActualHeight() - 1; i7 >= 0; --i7) {
               if (this.world.isAirBlock(blockpos$mutableblockpos.setPos(l5, i7, j6))) {
                  ...
                  for(k7 = i2; k7 < i2 + 4; ++k7) {
                     ...
                     d6 = (double)i7 + 0.5 - entityIn.posY;
         ...
         for(int j7 = -1; j7 <= 1; ++j7) {
            for(i7 = 1; i7 < 3; ++i7) {
               for(k7 = -1; k7 < 3; ++k7) {
                  ...
                  boolean flag = k7 < 0;
                  this.world.setBlockState(new BlockPos(j8, j9, j10), flag ? Nk.OBSIDIAN.getDefaultState() : Nk.AIR.getDefaultState());
               }
            }
         }
      }

      in iblockstate = Nk.PORTAL.getDefaultState().withProperty(fo.AXIS, j6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
      ...
      for(i7 = 0; i7 < 4; ++i7) {
         for(k7 = 0; k7 < 4; ++k7) {
            for(j8 = -1; j8 < 4; ++j8) {
               ...
               this.world.setBlockState(new BlockPos(j9, j10, j11), flag1 ? Nk.OBSIDIAN.getDefaultState() : iblockstate, 2);
            }
         }
   ...
   public void removeStalePortalLocations(long worldTime) {
      if (worldTime % 100L == 0L) {
         ...
         ObjectIterator<bic> objectiterator = this.destinationCoordinateCache.values().iterator();

         while(true) {
            bic teleporter$portalposition;
            do {
               if (!objectiterator.hasNext()) {
                  return;
               }

               teleporter$portalposition = (bic)objectiterator.next();
