...
public class bbq extends bbn {
   ...
   List<bbp> foliageCoords;

   public bbq(boolean notify) {
      super(notify);
   ...
   void generateLeafNodeList() {
      ...
      this.foliageCoords.add(new bbp(this.basePos.up(k), j));
            ...
            for(int l = 0; l < i; ++l) {
               ...
               if (this.checkBlockLine(blockpos, blockpos1) == -1) {
                  ...
                  if (this.checkBlockLine(blockpos2, blockpos) == -1) {
                     this.foliageCoords.add(new bbp(blockpos, blockpos2.getY()));
                  }
               }
            }
         }
   ...
   void crosSection(BlockPos pos, float p_181631_2_, in p_181631_3_) {
      ...
      for(int j = -i; j <= i; ++j) {
         for(int k = -i; k <= i; ++k) {
            if (Math.pow((double)Math.abs(j) + 0.5, 2.0) + Math.pow((double)Math.abs(k) + 0.5, 2.0) <= (double)(p_181631_2_ * p_181631_2_)) {
               ...
               hM material = this.world.getBlockState(blockpos).getMaterial();
   ...
   void generateLeafNode(BlockPos pos) {
      for(int i = 0; i < this.leafDistanceLimit; ++i) {
         this.crosSection(pos.up(i), this.leafSize(i), Nk.LEAVES.getDefaultState().withProperty(ew.CHECK_DECAY, false));
      }

   }

   void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, co p_175937_3_) {
      ...
      for(int j = 0; j <= i; ++j) {
         ...
         eD blocklog$enumaxis = this.getLogAxis(p_175937_1_, blockpos1);
         this.setBlockAndNotifyAdequately(this.world, blockpos1, p_175937_3_.getDefaultState().withProperty(eE.LOG_AXIS, blocklog$enumaxis));
      }
   ...
   private eD getLogAxis(BlockPos p_175938_1_, BlockPos p_175938_2_) {
      eD blocklog$enumaxis = eD.Y;
      int i = Math.abs(p_175938_2_.getX() - p_175938_1_.getX());
      int j = Math.abs(p_175938_2_.getZ() - p_175938_1_.getZ());
      int k = Math.max(i, j);
      if (k > 0) {
         if (i == k) {
            blocklog$enumaxis = eD.X;
         } else if (j == k) {
            blocklog$enumaxis = eD.Z;
         }
      }
   ...
   void generateLeaves() {
      ...
      while(var1.hasNext()) {
         bbp worldgenbigtree$foliagecoordinates = (bbp)var1.next();
   ...
   void generateLeafNodeBases() {
      ...
      while(var1.hasNext()) {
         ...
         if (!blockpos.equals(worldgenbigtree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY())) {
            this.limb(blockpos, worldgenbigtree$foliagecoordinates, Nk.LOG);
         }
      }
   ...
   private boolean validTreeLocation() {
      co block = this.world.getBlockState(this.basePos.down()).getBlock();
      if (block != Nk.DIRT && block != Nk.GRASS && block != Nk.FARMLAND) {
         return false;
      } else {
