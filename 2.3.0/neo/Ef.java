...
public class Ef extends Ch {
   ...
   public void execute(Xx server, DB sender, String[] args) throws Ct {
      if (args.length < 1) {
         throw new Ej("commands.whitelist.usage", new Object[0]);
      } else {
         if ("on".equals(args[0])) {
            ...
            if ("add".equals(args[0])) {
               if (args.length < 2) {
                  throw new Ej("commands.whitelist.add.usage", new Object[0]);
               }

               gameprofile1 = server.getPlayerProfileCache().getGameProfileForUsername(args[1]);
               if (gameprofile1 == null) {
                  throw new Ct("commands.whitelist.add.failed", new Object[]{args[1]});
               }
               ...
               if (args.length < 2) {
                  throw new Ej("commands.whitelist.remove.usage", new Object[0]);
               }

               gameprofile1 = server.getPlayerList().getWhitelistedPlayers().getByName(args[1]);
               if (gameprofile1 == null) {
                  throw new Ct("commands.whitelist.remove.failed", new Object[]{args[1]});
               }
