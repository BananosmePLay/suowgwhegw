...
public class VO extends VL {
   private boolean shouldAvoidSun;

   public VO(Iu entitylivingIn, bij worldIn) {
      super(entitylivingIn, worldIn);
   }

   protected VJ getPathFinder() {
      this.nodeProcessor = new VU();
      this.nodeProcessor.setCanEnterDoors(true);
      return new VJ(this.nodeProcessor);
   }
   ...
   public VI getPathToPos(BlockPos pos) {
      BlockPos blockpos1;
      if (this.world.getBlockState(pos).getMaterial() == hM.AIR) {
         for(blockpos1 = pos.down(); blockpos1.getY() > 0 && this.world.getBlockState(blockpos1).getMaterial() == hM.AIR; blockpos1 = blockpos1.down()) {
         }
         ...
         while(blockpos1.getY() < this.world.getHeight() && this.world.getBlockState(blockpos1).getMaterial() == hM.AIR) {
   ...
   private int getPathablePosY() {
      if (this.entity.isInWater() && this.getCanSwim()) {
         ...
         co block = this.world.getBlockState(new BlockPos(MathHelper.floor(this.entity.posX), i, MathHelper.floor(this.entity.posZ))).getBlock();
         int j = 0;
         ...
         for(int k = i; k < i + sizeX; ++k) {
            for(int l = j; l < j + sizeZ; ++l) {
               ...
               if (d0 * p_179683_8_ + d1 * p_179683_10_ >= 0.0) {
                  VQ pathnodetype = this.nodeProcessor.getPathNodeType(this.world, k, y - 1, l, this.entity, sizeX, sizeY, sizeZ, true, true);
                  if (pathnodetype == VQ.WATER) {
                     return false;
                  }

                  if (pathnodetype == VQ.LAVA) {
                     return false;
                  }
                  ...
                  if (pathnodetype == VQ.DAMAGE_FIRE || pathnodetype == VQ.DANGER_FIRE || pathnodetype == VQ.DAMAGE_OTHER) {
                     return false;
                  }
               }
            }
   ...
   private boolean isPositionClear(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3d p_179692_7_, double p_179692_8_, double p_179692_10_) {
      Iterator var12 = BlockPos.getAllInBox(new BlockPos(x, y, z), new BlockPos(x + sizeX - 1, y + sizeY - 1, z + sizeZ - 1)).iterator();
