...
public class bnG implements bnH {
   ...
   public bnG(Class cls, Class[] preTypes, Class type, Class[] postTypes, String errorName) {
      ...
      if (l < 0) {
         bmZ.log("(Reflector) Field not found: " + errorName);
      } else {
         int j = Collections.indexOfSubList(list.subList(l + 1, list.size()), list1);
         if (j >= 0) {
            bmZ.log("(Reflector) More than one match found for field: " + errorName);
         } else {
