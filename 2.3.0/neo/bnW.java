...
public class bnW {
   ...
   private static bij worldLast = null;
   ...
   public static int getMaxChunkY(bij world, Ig viewEntity, int renderDistanceChunks) {
      ...
      for(int i2 = l; i2 < i1; ++i2) {
         for(int j2 = j1; j2 < k1; ++j2) {
            bam chunk1 = world.getChunk(i2, j2);
            if (!chunk1.isEmpty()) {
               baB[] aextendedblockstorage = chunk1.getBlockStorageArray();

               for(int k2 = aextendedblockstorage.length - 1; k2 > l1; --k2) {
                  baB extendedblockstorage = aextendedblockstorage[k2];
               ...
               try {
                  Map<BlockPos, Yg> map = chunk1.getTileEntityMap();
                  if (!map.isEmpty()) {
               ...
               ClassInheritanceMultiMap<Ig>[] classinheritancemultimap = chunk1.getEntityLists();

               for(int i3 = classinheritancemultimap.length - 1; i3 > l1; --i3) {
                  ClassInheritanceMultiMap<Ig> classinheritancemultimap1 = classinheritancemultimap[i3];
   ...
   private static EnumFacing[][] makeEnumFacingArrays(boolean opposite) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         EnumFacing[] aenumfacing1 = (EnumFacing[])((EnumFacing[])list.toArray(new EnumFacing[list.size()]));
