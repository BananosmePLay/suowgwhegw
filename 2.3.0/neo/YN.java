...
public class YN extends YD implements ITickable, ISidedInventory {
   private static final int[] SLOTS = new int[27];
   private NonNullList<Qy> items;
   private boolean hasBeenCleared;
   private int openCount;
   private YM animationStatus;
   ...
   public YN(@Nullable Om colorIn) {
      this.items = NonNullList.withSize(27, Qy.EMPTY);
      this.animationStatus = YM.CLOSED;
      this.color = colorIn;
   }

   public void update() {
      this.updateAnimation();
      if (this.animationStatus == YM.OPENING || this.animationStatus == YM.CLOSING) {
      ...
      switch (this.animationStatus) {
         ...
         case OPENING:
            ...
            if (this.progress >= 1.0F) {
               this.moveCollidedEntities();
               this.animationStatus = YM.OPENED;
               this.progress = 1.0F;
            }
            break;
         case CLOSING:
   ...
   public YM getAnimationStatus() {
      return this.animationStatus;
   }

   public AxisAlignedBB getBoundingBox(in p_190584_1_) {
      return this.getBoundingBox((EnumFacing)p_190584_1_.getValue(gr.FACING));
   }

   public AxisAlignedBB getBoundingBox(EnumFacing p_190587_1_) {
      return co.FULL_BLOCK_AABB.expand((double)(0.5F * this.getProgress(1.0F) * (float)p_190587_1_.getXOffset()), (double)(0.5F * this.getProgress(1.0F) * (float)p_190587_1_.getYOffset()), (double)(0.5F * this.getProgress(1.0F) * (float)p_190587_1_.getZOffset()));
   }
   ...
   private void moveCollidedEntities() {
      ...
      if (iblockstate.getBlock() instanceof gr) {
         EnumFacing enumfacing = (EnumFacing)iblockstate.getValue(gr.FACING);
         AxisAlignedBB axisalignedbb = this.getTopBoundingBox(enumfacing).offset(this.pos);
         List<Ig> list = this.world.getEntitiesWithinAABBExcludingEntity((Ig)null, axisalignedbb);
         if (!list.isEmpty()) {
            for(int i = 0; i < list.size(); ++i) {
               ...
               if (entity.getPushReaction() != hJ.IGNORE) {
                  ...
                  entity.move(Lq.SHULKER_BOX, d0 * (double)enumfacing.getXOffset(), d1 * (double)enumfacing.getYOffset(), d2 * (double)enumfacing.getZOffset());
               }
            }
         }
      }
   ...
   public boolean receiveClientEvent(int id, int type) {
      if (id == 1) {
         this.openCount = type;
         if (type == 0) {
            this.animationStatus = YM.CLOSING;
         }

         if (type == 1) {
            this.animationStatus = YM.OPENING;
         }
   ...
   public void openInventory(ME player) {
      if (!player.isSpectator()) {
         ...
         if (this.openCount == 1) {
            this.world.playSound((ME)null, this.pos, NO.BLOCK_SHULKER_BOX_OPEN, SoundCategory.BLOCKS, 0.5F, this.world.rand.nextFloat() * 0.1F + 0.9F);
         }
      }
   ...
   public void closeInventory(ME player) {
      if (!player.isSpectator()) {
         ...
         this.world.addBlockEvent(this.pos, this.getBlockType(), 1, this.openCount);
         if (this.openCount <= 0) {
            this.world.playSound((ME)null, this.pos, NO.BLOCK_SHULKER_BOX_CLOSE, SoundCategory.BLOCKS, 0.5F, this.world.rand.nextFloat() * 0.1F + 0.9F);
         }
      }
   ...
   public void loadFromNbt(QQ compound) {
      this.items = NonNullList.withSize(this.getSizeInventory(), Qy.EMPTY);
   ...
   public QQ saveToNbt(QQ compound) {
   ...
   public boolean canInsertItem(int index, Qy itemStackIn, EnumFacing direction) {
      return !(co.getBlockFromItem(itemStackIn.getItem()) instanceof gr);
   }
   ...
   public Om getColor() {
      if (this.color == null) {
         this.color = gr.getColorFromBlock(this.getBlockType());
      }
   ...
   public Vg getUpdatePacket() {
      return new Vg(this.pos, 10, this.getUpdateTag());
   }
