...
public class bps {
   ...
   public static Map<Integer, bpf> multiTexMap;
   ...
   public static bpf getMultiTexID(yO tex) {
      bpf multitexid = tex.multiTex;
      if (multitexid == null) {
         int i = tex.getGlTextureId();
         multitexid = (bpf)multiTexMap.get(i);
         if (multitexid == null) {
            multitexid = new bpf(i, GL11.glGenTextures(), GL11.glGenTextures());
   ...
   public static void deleteTextures(yO atex, int texid) {
      bpf multitexid = atex.multiTex;
      if (multitexid != null) {
         atex.multiTex = null;
         multiTexMap.remove(multitexid.base);
         yh.deleteTexture(multitexid.norm);
         yh.deleteTexture(multitexid.spec);
         if (multitexid.base != texid) {
            bpx.warning("Error : MultiTexID.base mismatch: " + multitexid.base + ", texid: " + texid);
            yh.deleteTexture(multitexid.base);
         }
      }
   ...
   public static void bindNSTextures(int normTex, int specTex) {
      if (bpq.isRenderingWorld && yh.getActiveTextureUnit() == 33984) {
         yh.setActiveTexture(33986);
         yh.bindTexture(normTex);
         yh.setActiveTexture(33987);
         yh.bindTexture(specTex);
         yh.setActiveTexture(33984);
      }

   }

   public static void bindNSTextures(bpf multiTex) {
   ...
   public static void bindTextures(int baseTex, int normTex, int specTex) {
      ...
      yh.bindTexture(baseTex);
   }

   public static void bindTextures(bpf multiTex) {
      if (bpq.isRenderingWorld && yh.getActiveTextureUnit() == 33984) {
         if (bpq.configNormalMap) {
            yh.setActiveTexture(33986);
            yh.bindTexture(multiTex.norm);
         }

         if (bpq.configSpecularMap) {
            yh.setActiveTexture(33987);
            yh.bindTexture(multiTex.spec);
         }

         yh.setActiveTexture(33984);
      }

      yh.bindTexture(multiTex.base);
   }

   public static void bindTexture(yR tex) {
      int i = tex.getGlTextureId();
      bindTextures(tex.getMultiTexID());
      if (yh.getActiveTextureUnit() == 33984) {
         int j = bpq.atlasSizeX;
         int k = bpq.atlasSizeY;
         if (tex instanceof zj) {
            bpq.atlasSizeX = ((zj)tex).atlasWidth;
            bpq.atlasSizeY = ((zj)tex).atlasHeight;
         } else {
            bpq.atlasSizeX = 0;
            bpq.atlasSizeY = 0;
         }

         if (bpq.atlasSizeX != j || bpq.atlasSizeY != k) {
            bpq.uniform_atlasSize.setValue(bpq.atlasSizeX, bpq.atlasSizeY);
         }
      }
   ...
   public static void bindTextures(int baseTex) {
      bpf multitexid = (bpf)multiTexMap.get(baseTex);
      bindTextures(multitexid);
   }

   public static void initDynamicTexture(int texID, int width, int height, yP tex) {
      bpf multitexid = tex.getMultiTexID();
      int[] aint = tex.getTextureData();
      int i = width * height;
      Arrays.fill(aint, i, i * 2, -8421377);
      Arrays.fill(aint, i * 2, i * 3, 0);
      zk.allocateTexture(multitexid.base, width, height);
      zk.setTextureBlurMipmap(false, false);
      zk.setTextureClamped(false);
      zk.allocateTexture(multitexid.norm, width, height);
      zk.setTextureBlurMipmap(false, false);
      zk.setTextureClamped(false);
      zk.allocateTexture(multitexid.spec, width, height);
      zk.setTextureBlurMipmap(false, false);
      zk.setTextureClamped(false);
      yh.bindTexture(multitexid.base);
   }

   public static void updateDynamicTexture(int texID, int[] src, int width, int height, yP tex) {
      bpf multitexid = tex.getMultiTexID();
      yh.bindTexture(multitexid.base);
      updateDynTexSubImage1(src, width, height, 0, 0, 0);
      yh.bindTexture(multitexid.norm);
      updateDynTexSubImage1(src, width, height, 0, 0, 1);
      yh.bindTexture(multitexid.spec);
   ...
   public static yR createDefaultTexture() {
      yP dynamictexture = new yP(1, 1);
      dynamictexture.getTextureData()[0] = -1;
      dynamictexture.updateDynamicTexture();
      return dynamictexture;
   }

   public static void allocateTextureMap(int texID, int mipmapLevels, int width, int height, zb stitcher, zj tex) {
      bpx.info("allocateTextureMap " + mipmapLevels + " " + width + " " + height + " ");
      tex.atlasWidth = width;
      tex.atlasHeight = height;
      bpf multitexid = getMultiTexID(tex);
      zk.allocateTextureImpl(multitexid.base, mipmapLevels, width, height);
      if (bpq.configNormalMap) {
         zk.allocateTextureImpl(multitexid.norm, mipmapLevels, width, height);
      }

      if (bpq.configSpecularMap) {
         zk.allocateTextureImpl(multitexid.spec, mipmapLevels, width, height);
      }

      yh.bindTexture(texID);
   }

   public static void uploadTexSubForLoadAtlas(zj textureMap, String iconName, int[][] data, int width, int height, int xoffset, int yoffset, boolean linear, boolean clamp) {
      bpf multitexid = textureMap.multiTex;
      zk.uploadTextureMipmap(data, width, height, xoffset, yoffset, linear, clamp);
      boolean flag = false;
      int[][] aint1;
      if (bpq.configNormalMap) {
         aint1 = readImageAndMipmaps(textureMap, iconName + "_n", width, height, data.length, flag, -8421377);
         yh.bindTexture(multitexid.norm);
         zk.uploadTextureMipmap(aint1, width, height, xoffset, yoffset, linear, clamp);
      }
   ...
   public static int[][] readImageAndMipmaps(zj updatingTextureMap, String name, int width, int height, int numLevels, boolean border, int defColor) {
      bpf multitexid = updatingTextureMap.multiTex;
   ...
   public static BufferedImage readImage(ResourceLocation resLoc) {
      try {
         if (!XH.hasResource(resLoc)) {
            return null;
         } else {
            InputStream inputstream = XH.getResourceStream(resLoc);
            if (inputstream == null) {
               return null;
            } else {
   ...
   public static void setupTexture(bpf multiTex, int[] src, int width, int height, boolean linear, boolean clamp) {
   ...
   public static void updateSubImage(bpf multiTex, int[] src, int width, int height, int posX, int posY, boolean linear, boolean clamp) {
      int i = width * height;
      IntBuffer intbuffer = getIntBuffer(i);
      intbuffer.clear();
   ...
   public static void loadNSMap(AA manager, ResourceLocation location, int width, int height, int[] aint) {
      if (bpq.configNormalMap) {
   ...
   private static void loadNSMap1(AA manager, ResourceLocation location, int width, int height, int[] aint, int offset, int defaultColor) {
   ...
   private static boolean loadNSMapFile(AA manager, ResourceLocation location, int width, int height, int[] aint, int offset) {
      if (location == null) {
         return false;
      } else {
         try {
            Az iresource = manager.getResource(location);
   ...
   public static int loadSimpleTexture(int textureID, BufferedImage bufferedimage, boolean linear, boolean clamp, AA resourceManager, ResourceLocation location, bpf multiTex) {
      int i = bufferedimage.getWidth();
      int j = bufferedimage.getHeight();
      int k = i * j;
   ...
   public static void loadLayeredTexture(yV tex, AA manager, List<String> list) {
      int i = 0;
      int j = 0;
      int k = 0;
      int[] aint = null;
   ...
   public static void updateTextureMinMagFilter() {
      ...
      yR itextureobject = texturemanager.getTexture(zj.LOCATION_BLOCKS_TEXTURE);
      if (itextureobject != null) {
         bpf multitexid = itextureobject.getMultiTexID();
         yh.bindTexture(multitexid.base);
         GL11.glTexParameteri(3553, 10241, bpq.texMinFilValue[bpq.configTexMinFilB]);
         GL11.glTexParameteri(3553, 10240, bpq.texMagFilValue[bpq.configTexMagFilB]);
         yh.bindTexture(multitexid.norm);
         GL11.glTexParameteri(3553, 10241, bpq.texMinFilValue[bpq.configTexMinFilN]);
         GL11.glTexParameteri(3553, 10240, bpq.texMagFilValue[bpq.configTexMagFilN]);
         yh.bindTexture(multitexid.spec);
         GL11.glTexParameteri(3553, 10241, bpq.texMinFilValue[bpq.configTexMinFilS]);
         GL11.glTexParameteri(3553, 10240, bpq.texMagFilValue[bpq.configTexMagFilS]);
         yh.bindTexture(0);
      }
   ...
   public static int[][] prepareAF(zd tas, int[][] src, int width, int height) {
      boolean flag = true;
      return src;
   }

   public static void fixTransparentColor(zd tas, int[] aint) {
   }
