...
public class CF extends Ch {
   ...
   public void execute(Xx server, DB sender, String[] args) throws Ct {
      if (args.length < 8) {
         throw new Ej("commands.particle.usage", new Object[0]);
      } else {
         boolean flag = false;
         EnumParticleTypes enumparticletypes = EnumParticleTypes.getByName(args[0]);
         if (enumparticletypes == null) {
            throw new Ct("commands.particle.notFound", new Object[]{args[0]});
         } else {
            String s = args[0];
            ...
            for(int j = 0; j < aint.length; ++j) {
               if (args.length > 11 + j) {
                  try {
                     ...
                     throw new Ct("commands.particle.invalidParam", new Object[]{args[11 + j]});
                  }
               }
            }

            bij world = sender.getEntityWorld();
            if (world instanceof bis) {
               bis worldserver = (bis)world;
